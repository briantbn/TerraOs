...
# Antes en 40m -- subido a 60m para reducir la cantidad de celdas de la
# grilla (menos nodos = menos costo por paso interno del esquema explícito
# de LISFLOOD-FP). No cambia el sentido del resultado (misma tormenta,
# mismo radio), solo el detalle espacial por celda -- sigue siendo
# suficiente para estimar superficie/volumen inundado a escala de campo.
# Ver TIMEOUT_SUBPROCESO_SEG más abajo: en terreno muy plano (llanura,
# ej. Mesopotamia) esto solo, ya de por sí, puede no alcanzar del todo.
RESOLUCION_M = 60.0
...
# Subido de 160s a 220s: junto con RESOLUCION_M=60m (antes 40m), le da más
# margen real a terreno muy plano. Sigue siendo un parche de paciencia, no
# una solución de fondo -- si un punto en particular sigue sin alcanzar,
# la palanca que realmente ataca la causa es reducir horas_total (menos
# tiempo simulado = menos pasos internos), no otro segundo más de timeout.
# IMPORTANTE: si subís este valor, subí también en Render (Start Command)
# el --timeout de gunicorn por encima de este número, y en el HTML el
# AbortSignal.timeout por encima de ambos -- los tres deben quedar en ese
# orden: TIMEOUT_SUBPROCESO_SEG < --timeout gunicorn < AbortSignal frontend.
TIMEOUT_SUBPROCESO_SEG = 220
