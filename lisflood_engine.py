...
# Techo DURO para la simulación dinámica — pase lo que pase desde afuera
# (frontend, un slider compartido con otro panel de la UI, un query param
# manual), nunca se corre LISFLOOD-FP con más radio que esto. Con
# TIMEOUT_SUBPROCESO_SEG=60s el margen es chico: 3km/30m ya medía ~99s en
# pruebas (por encima del timeout actual), así que 2.5km es el límite
# razonable para dejar margen en terreno plano. Para áreas más grandes hay
# que usar el simulador estático (HAND, /inundacion_local), no este motor.
RADIO_MAXIMO_KM = 2.5
...
TIMEOUT_SUBPROCESO_SEG = 60  # ver nota abajo: en terreno muy plano (esteros/bañados)
# LISFLOOD-FP puede necesitar pasos de tiempo carísimamente chicos y tardar
# mucho más que en terreno con pendiente normal — mejor fallar rápido con
# un mensaje claro que colgar el único worker del servidor varios minutos.
...
def simular_crecida_dinamica(lat, lon, mm_totales, horas_lluvia, horas_total,
    ...
    if radio_km > RADIO_MAXIMO_KM:
        raise RuntimeError(
            f'radio_km={radio_km} supera el máximo soportado por la simulación dinámica '
            f'(LISFLOOD-FP): {RADIO_MAXIMO_KM} km. Con radios más grandes el modelo no '
            f'llega a terminar dentro de {TIMEOUT_SUBPROCESO_SEG}s. Para áreas más '
            f'extensas usá el Simulador de Inundación estático (HAND, /inundacion_local).'
        )
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                f'La simulación no terminó en {TIMEOUT_SUBPROCESO_SEG}s. Esto suele pasar en '
                f'terreno muy plano (esteros, bañados, zonas de humedal) — con pendiente casi '
                f'nula, el modelo necesita pasos de tiempo carísimamente chicos. Probá con un '
                f'punto de mayor pendiente, o usá el Simulador de Inundación estático (HAND) '
                f'para esta zona en particular.'
            )
