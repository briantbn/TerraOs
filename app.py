...
def inundacion_tiles():
    ...
    try:
        ...
        if tipo == 'riesgo':
            ...
            # Candidatas por elevación: todo lo que está por debajo del umbral,
            # conectado o no. Esto es lo único que había antes (bathtub-fill puro).
            candidatas = elevacion.lte(umbral)

            # ── Corrección de conectividad hidráulica ──────────────────
            # Semilla = agua permanente conocida (JRC Global Surface Water):
            # píxeles con >50% de ocurrencia histórica de agua en el período
            # 1984-2021. Es nuestro proxy de "acá hay río/laguna real".
            agua_fuente = (ee.Image('JRC/GSW1_4/GlobalSurfaceWater')
                             .select('occurrence')
                             .gt(50)
                             .clip(region))

            # "Costo" de avanzar = 1 únicamente sobre celdas candidatas (bajas);
            # el resto queda enmascarado = infranqueable para el flood-fill.
            costo = candidatas.selfMask()

            # RENDIMIENTO: cumulativeCost es caro — su costo crece con
            # (distancia máxima / escala del píxel). A 30 m nativos de SRTM
            # y hasta 200 km de radio, la relación se dispara (~6700 pasos)
            # y puede superar los límites de cómputo de Earth Engine o
            # tardar demasiado en un server gratuito como Render.
            # Mitigamos con dos ajustes:
            #   1) Reproyectar a 90 m para el cálculo de conectividad
            #      (alcanza para distinguir zonas conectadas de aisladas,
            #      no necesitamos precisión de 30 m acá).
            #   2) Topear la propagación a 50 km: más allá de esa distancia,
            #      si sigue habiendo terreno bajo continuo, es un caso muy
            #      atípico y no vale el costo de cómputo extra.
            ...
            # cumulativeCost propaga desde la fuente de agua SOLO a través de
            # celdas transitables (candidatas), acumulando distancia real en
            # metros. Los píxeles inalcanzables quedan sin valor (masked).
            costo_acumulado = costo_90.cumulativeCost(
                source=fuente_90,
                maxDistance=max_dist_conectividad,
            )

            # Zona final = candidata por elevación Y alcanzable desde agua real
            # sin cruzar terreno alto. Esto descarta depresiones aisladas.
            zona_conectada = candidatas.updateMask(costo_acumulado.gte(0)).selfMask()

            # Fallback: si no hay ningún píxel de agua conocida en el radio
            # consultado (zona sin río/laguna mapeada por JRC), cumulativeCost
            # no tiene desde dónde propagar y todo quedaría vacío. En ese caso
            # devolvemos las candidatas sin filtrar, mejor mostrar de más que
            # no mostrar nada, y lo marcamos en la respuesta.
            ...
            conectividad_aplicada = ee.Algorithms.If(hay_agua_fuente, True, False).getInfo()
