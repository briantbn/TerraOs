...
overland_flow_engine = _LazyModule('overland_flow_engine')
...
# ──────────────────────────────────────────────────────────────────────────
# MÓDULO OPCIONAL: watershed_delineation
# ──────────────────────────────────────────────────────────────────────────
# Delimitación de la cuenca hidrológica aportante a un punto (pensado como
# el punto más bajo del lote del productor). Usa MERIT Hydro (Earth Engine,
# recorte local descargado on-the-fly, sin datasets globales autoalojados
# como MERIT-Basins) + WhiteboxTools para el pipeline D8 estándar. Necesita
# 'whitebox' y 'pyshp' en requirements.txt, además de rasterio/shapely ya
# usados por otros módulos.
watershed_delineation = _LazyModule('watershed_delineation')
_watershed_disponible = watershed_delineation.disponible
...
# ------------------------------------------------------------------------
# Simulación DINÁMICA (landlab OverlandFlow) — el agua se propaga en el
# tiempo según una tormenta de diseño, en vez del umbral HAND estático de
# /inundacion_local. Radio chico a propósito (ver RADIO_ANALISIS_KM en
# overland_flow_engine.py) para que corra en segundos. Sin Earth Engine.
# ──────────────────────────────────────────────────────────────────────────
def _overland_flow_disponible_check():
    """Igual razón que _flood_local_disponible_check(): 'disponible' acá
    depende de si landlab se pudo importar DENTRO del módulo, no solo de
    que el archivo overland_flow_engine.py exista."""
    try:
        mod = overland_flow_engine._cargar()
...
def inundacion_dinamica():
    ...
    Parámetros:
        ...
        radio_km         opcional, default 3 km (ver overland_flow_engine.py
                         para por qué no conviene agrandarlo mucho).
    """
    if not _overland_flow_disponible_check():
        return jsonify({
            'error': ('Módulo overland_flow_engine no disponible en el servidor '
                      '(falta "landlab" en requirements.txt, o el archivo no está '
                      'desplegado junto a app.py).'),
        }), 503
    ...
    # Límite duro de radio: este motor resuelve las ecuaciones de aguas
    # someras celda a celda (no un umbral estático como local_flood_engine),
    # por eso solo es viable a radios chicos -- ver overland_flow_engine.py.
    # A 50 km, por ejemplo, la grilla saltaría a millones de nodos y el
    # request se colgaría en vez de tardar más; mejor devolver un error
    # claro ahora que un timeout silencioso después.
    RADIO_MAXIMO_DINAMICO_KM = 8.0
    if radio_km is not None and radio_km > RADIO_MAXIMO_DINAMICO_KM:
        return jsonify({
            'error': (f'radio_km={radio_km} es demasiado grande para la simulación dinámica '
                      f'(máximo {RADIO_MAXIMO_DINAMICO_KM} km). Este motor resuelve el flujo '
                      f'celda a celda, no un umbral estático -- a radios grandes la grilla '
                      f'crece a millones de nodos y el cálculo no es viable en un request '
                      f'HTTP. Para áreas más grandes usá el Simulador de inundación estático '
                      f'(HAND + umbral, local_flood_engine).'),
        }), 400
    ...
    try:
        resultado = overland_flow_engine.simular_crecida_dinamica(
    ...
    imagen_b64 = overland_flow_engine.generar_imagen_profundidad_base64(
    ...
    respuesta = {
        ...
        'vis': {'min': 0.0, 'max': 1.5, 'palette': overland_flow_engine.PALETA_PROFUNDIDAD},
        ...
        'escenario': escenario,
        'motor': 'local_overland_flow_landlab_copernicus30',
    }
...
# ──────────────────────────────────────────────────────────────────────────
# Endpoint: /cuenca_aportante
# ------------------------------------------------------------------------
# Delimita la cuenca hidrológica aportante a un punto (pensado como el
# punto más bajo del lote del productor, marcado a mano o derivado de un
# KML/GeoJSON en el frontend). Descarga un recorte local de MERIT Hydro
# (Earth Engine) y corre el pipeline D8 con WhiteboxTools -- no usa
# datasets globales de MERIT-Basins ni depende de un entorno Python aparte.
# ──────────────────────────────────────────────────────────────────────────
@app.route('/cuenca_aportante')
def cuenca_aportante():
    if not _ee_ready:
        return jsonify({'error': f'Earth Engine no inicializado: {_ee_error}'}), 503
    if not _watershed_disponible:
        return jsonify({
            'error': ('Módulo watershed_delineation no disponible en el servidor. '
                      'Subí watershed_delineation.py junto a app.py, agregá "whitebox" '
                      'y "pyshp" a requirements.txt, y volvé a desplegar.'),
        }), 503
    try:
        lat = float(request.args.get('lat'))
        lon = float(request.args.get('lon'))
        buffer_km = float(request.args.get('buffer_km', 15))
    except (TypeError, ValueError):
        return jsonify({'error': 'Parámetros lat/lon requeridos y deben ser numéricos.'}), 400

    try:
        resultado = watershed_delineation.delimitar_cuenca_aportante(lat, lon, buffer_km)
    except Exception as e:
        import traceback
        return jsonify({
            'ok': False,
