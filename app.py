...
# Estos dos endpoints son los únicos que corren _conectividad_hidraulica(), la
# cual hace .reproject(scale=90m) + cumulativeCost() sobre TODA la región
# recortada. La propagación de agua ya está limitada internamente a 50 km
# (ver max_dist_conectividad en _conectividad_hidraulica), así que un radio
# de AOI mucho mayor a eso no aporta nada útil — solo hace que Earth Engine
# tenga que reproyectar y procesar cientos de millones de píxeles de más
# antes de poder renderizar cada tile. Eso NO falla en el request inicial
# (getMapId() no ejecuta nada todavía), sino después, cuando el navegador
# pide cada tile individual: ahí Earth Engine devuelve 400, y la capa queda
# invisible aunque el backend haya respondido "ok" con una tile_url válida.
RADIUS_KM_MAX_INUNDACION = 100
...
def _dem_corregido_por_dosel(elevacion, region):
    """
    Corrige el sesgo de altura de copa de árboles en el DEM SRTM.

    SRTM es un DSM (Digital Surface Model): el radar mide la altura de lo
    primero que encuentra, que en zonas de bosque cerrado es la copa de los
    árboles, no el suelo real. En parques con monte denso (ej. Mburucuyá)
    esto genera "lomas" falsas que no existen en el terreno, distorsionando
    la simulación de inundación (el agua "rebota" contra un bosque que en
    el DEM parece una loma, cuando en realidad puede ser una zona baja).

    VERSIÓN INTERINA (solo NDVI): hasta integrar MapBiomas Argentina como
    máscara real de "esto es bosque", se usa una heurística de DOS
    condiciones para decidir dónde corregir, pensada para NO tocar nunca
    cultivos/pasturas (que también pueden tener NDVI alto en pico de
    crecimiento, pero no tienen relieve local marcado):

      1. NDVI > 0.75                    -> vegetación densa
      2. elevación sobresale > 5 m del
         promedio de su entorno (300 m) -> "pico" típico de dosel/árbol
                                             aislado, no de un cultivo

    Solo cuando SE CUMPLEN AMBAS se resta altura, con una función CONTINUA
    del NDVI (no escalones fijos) para no crear acantilados artificiales
    en el DEM que después distorsionen el ruteo de agua (cálculo de
    Manning). El borde de la corrección también se suaviza para lo mismo.

    LIMITACIÓN CONOCIDA (por diseño, a resolver con MapBiomas más
    adelante): dentro de un bosque grande y continuo, todo el vecindario
    está igual de "inflado", así que la condición 2 no dispara y el
    interior del bosque queda sin corregir del todo. Es decir, esta
    versión corrige bien bordes de bosque y árboles/grupos aislados, pero
    subestima la corrección en el interior de manchas boscosas grandes.
    Cuando se sume MapBiomas, la condición 2 se reemplaza por "el píxel
    está catalogado como Bosque", resolviendo esa limitación.
    """
    hoy = ee.Date(datetime.date.today().isoformat())

    col = (ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED')
             .filterBounds(region)
             .filterDate(hoy.advance(-90, 'day'), hoy))
    col_masked = col.map(_mask_s2_clouds)
    img = col_masked.sort('CLOUDY_PIXEL_PERCENTAGE').mosaic()
    # unmask(-1): donde no hubo NINGUNA imagen sin nubes en los 90 días, en
    # vez de dejar el píxel "sin dato" (lo que contagiaría la máscara al
    # resultado final y generaría huecos falsos en el DEM/pendiente), se
    # asume NDVI inválido/bajo -> ese píxel simplemente no se corrige.
    ndvi = img.normalizedDifference(['B8', 'B4']).rename('ndvi').unmask(-1)

    # "Pico" respecto al entorno: elevación vs. promedio en un radio de 300 m.
    kernel = ee.Kernel.circle(radius=300, units='meters')
    elev_vecinos = elevacion.reduceNeighborhood(
        reducer=ee.Reducer.mean(), kernel=kernel,
    )
    pico = elevacion.subtract(elev_vecinos).gt(5)
    ndvi_alto = ndvi.gt(0.75)
    mascara = pico.And(ndvi_alto).unmask(0)

    # Suavizar el borde de la máscara (0/1 duro -> gradiente 0..1) para
    # evitar "acantilados" en el DEM corregido.
    mascara_suave = mascara.reduceNeighborhood(
        reducer=ee.Reducer.mean(), kernel=ee.Kernel.circle(radius=90, units='meters'),
    ).unmask(0)

    # Cuánto restar: función continua de 0 a 15 m entre NDVI 0.75 y 1.0.
    # Exponente 1.5 -> la resta crece más rápido cerca de NDVI=1 (dosel
    # realmente cerrado) que cerca de 0.75 (límite difuso bosque/no-bosque).
    intensidad = ndvi.subtract(0.75).divide(0.25).clamp(0, 1)
    resta_m = intensidad.pow(1.5).multiply(15).unmask(0)

    return elevacion.subtract(resta_m.multiply(mascara_suave)).rename('elevation')
...
def inundacion_tiles():
    ...
    Parámetros:
        ...
        radius_km  radio del área (default: 200 km, TOPE: 100 km — ver
                   RADIUS_KM_MAX_INUNDACION; valores mayores se recortan
                   automáticamente porque la conectividad hidráulica ya
                   está limitada a 50 km de propagación)
...
def inundacion_punto():
    ...
    try:
        ...
        elevacion    = _dem_corregido_por_dosel(elevacion_cr, region)
        pendiente    = ee.Terrain.slope(elevacion)
...
# ──────────────────────────────────────────────────────────────────────────
# MAPBIOMAS ARGENTINA — cobertura y uso del suelo (hectáreas por clase)
# ──────────────────────────────────────────────────────────────────────────
# Asset público de Google Earth Engine, mismo patrón que ya usamos para
# SRTM/JRC: no requiere token ni registro adicional, ya está disponible con
# las credenciales de GEE que este backend ya tiene.
MAPBIOMAS_ARGENTINA_ASSET = (
    'projects/mapbiomas-public/assets/argentina/lulc/collection2/'
    'mapbiomas_argentina_collection2_integration_v3'
)

# ⚠️ PENDIENTE: tabla real de códigos de píxel -> nombre de clase.
# MapBiomas Argentina usa códigos numéricos por píxel para cada clase de
# cobertura (Bosque, Pastizal, Agricultura, Pasturas, etc.), pero a
# diferencia de la colección de Brasil, no encontré publicada la tabla CSV
# de códigos específica de Argentina para poder completarla acá con
# confianza. Completar esto ADIVINANDO los números sería peor que no
# tenerlo: generaría hectáreas "de Agricultura" que en realidad son de
# otra clase, sin ninguna señal de que está mal.
#
# Cómo conseguir la tabla real (Briant, esto lo tenés que hacer vos porque
# yo no tengo acceso a un editor de código de GEE en este entorno):
#   1) Abrí https://code.earthengine.google.com/
#   2) Pegá:
#        var img = ee.Image('projects/mapbiomas-public/assets/argentina/lulc/collection2/mapbiomas_argentina_collection2_integration_v3');
#        print(img.get('classes_names'));   // o 'class_names', revisar propiedades
#        print(img.bandNames());
#   3) Con el botón derecho sobre la imagen en el Inspector, click en un
#      píxel de un cultivo conocido y anotá qué número te devuelve.
#   4) Pegá esa tabla acá abajo y quitamos este comentario.
MAPBIOMAS_CLASES = {
    # 15: 'Pasturas',
    # 18: 'Agricultura',
    # 9:  'Forestación / Silvicultura',
    # 3:  'Bosque',
    # ... completar con la tabla real
}


def _mapbiomas_area_por_clase(region, scale_m=30):
    """
    Devuelve un dict {codigo_pixel: hectareas} con el área de cada clase de
    cobertura de MapBiomas dentro de `region`. Usa la banda más reciente
    disponible (la última del asset, no un año fijo hardcodeado, para que
    siga funcionando cuando MapBiomas publique años nuevos).
    """
    img = ee.Image(MAPBIOMAS_ARGENTINA_ASSET)
    bandas = img.bandNames()
    ultima_banda = bandas.get(bandas.size().subtract(1))
    clasif = img.select([ultima_banda]).clip(region)

    # Histograma de píxeles por clase dentro de la región.
    hist = clasif.reduceRegion(
        reducer=ee.Reducer.frequencyHistogram(),
        geometry=region,
        scale=scale_m,
        maxPixels=1e10,
        bestEffort=True,
    ).get(ultima_banda).getInfo()

    if not hist:
        return {}

    area_pixel_ha = (scale_m * scale_m) / 10000.0  # m² -> ha
    return {int(float(codigo)): round(cantidad * area_pixel_ha, 2)
            for codigo, cantidad in hist.items()}


@app.route('/uso_suelo_mapbiomas')
def uso_suelo_mapbiomas():
    """
    Cobertura y uso del suelo (MapBiomas Argentina) dentro de un radio.
    Parámetros: lat, lon, radius_km (default 5)

    NOTA IMPORTANTE: hasta completar MAPBIOMAS_CLASES con la tabla real de
    códigos (ver comentario arriba), esta respuesta trae los códigos
    numéricos crudos en 'clases_sin_nombre' además de, si ya están
    mapeados, las clases con nombre en 'clases'. Por ahora 'clases' puede
    venir vacío — es esperado, no es un bug.
    """
    if not _ee_ready:
        return jsonify({'error': f'Earth Engine no inicializado: {_ee_error}'}), 503
    try:
        ...
        region = ee.Geometry.Point([lon, lat]).buffer(radius_km * 1000)
        area_por_codigo = _mapbiomas_area_por_clase(region)

        clases_con_nombre = {}
        clases_sin_nombre = {}
        for codigo, hectareas in area_por_codigo.items():
            nombre = MAPBIOMAS_CLASES.get(codigo)
            if nombre:
                clases_con_nombre[nombre] = clases_con_nombre.get(nombre, 0) + hectareas
            else:
                clases_sin_nombre[codigo] = hectareas

        return jsonify({
            'fuente': 'MapBiomas Argentina (Google Earth Engine)',
            'radius_km': radius_km,
            'clases': clases_con_nombre,
            'clases_sin_nombre': clases_sin_nombre,
            'nota': (None if MAPBIOMAS_CLASES else
                     'MAPBIOMAS_CLASES todavía no tiene la tabla real de códigos '
                     'de Argentina; todo viene en clases_sin_nombre por ahora. '
                     'Ver instrucciones en el código fuente (MAPBIOMAS_CLASES).'),
        })
    except ee.EEException as exc:
        return jsonify({'error': f'Error de Earth Engine: {exc}'}), 502
    except Exception as exc:  # noqa: BLE001
...
# ──────────────────────────────────────────────────────────────────────────
# OPENSTREETMAP / OVERPASS — infraestructura y elementos expuestos
# ──────────────────────────────────────────────────────────────────────────
# Overpass API: gratuita, sin token/registro. Usamos varios espejos (mirrors)
# con reintento, porque el servidor público oficial rate-limitea agresivo.
OVERPASS_ENDPOINTS = [
...
# Qué buscamos y cómo lo etiquetamos en la respuesta. Cada entrada es una
# tupla (etiqueta_amigable, filtro_overpass_ql). El filtro se aplica tanto a
# node como a way (algunas cosas, como escuelas, pueden estar mapeadas como
# nodo puntual o como polígono del edificio).
OSM_CONSULTAS = [
    ('viviendas',            '["building"~"^(house|residential|detached|farm)$"]'),
    ('establecimientos_rurales', '["building"="farm_auxiliary"]'),
    ('escuelas',              '["amenity"="school"]'),
    ('hospitales',            '["amenity"~"^(hospital|clinic)$"]'),
    ('lineas_electricas',     '["power"="line"]'),
    ('rutas',                 '["highway"~"^(motorway|trunk|primary|secondary|tertiary)$"]'),
    ('puentes',               '["bridge"="yes"]'),
    ('areas_protegidas',      '["boundary"="protected_area"]'),
]


def _overpass_query(region_geojson_coords, consultas=OSM_CONSULTAS, timeout_s=25):
    """
    Consulta Overpass API por los elementos de OSM_CONSULTAS dentro del
    polígono dado (lista de [lon, lat] del anillo exterior, como viene de
    geometry.getInfo()['coordinates'][0] de un ee.Geometry, o de cualquier
    polígono GeoJSON). Devuelve {etiqueta: [ {lat, lon, tags}, ... ]}.

    Prueba varios espejos de Overpass en orden hasta que uno responda.
    """
    # Overpass QL usa "lat lon lat lon ..." (orden invertido respecto a GeoJSON)
    poly_str = ' '.join(f'{lat} {lon}' for lon, lat in region_geojson_coords)

    bloques = []
    for _, filtro in consultas:
        bloques.append(f'node{filtro}(poly:"{poly_str}");')
        bloques.append(f'way{filtro}(poly:"{poly_str}");')

    query = f'[out:json][timeout:{timeout_s}];({"".join(bloques)});out center tags;'

    ultimo_error = None
    for endpoint in OVERPASS_ENDPOINTS:
        try:
            r = requests.post(endpoint, data={'data': query}, timeout=timeout_s + 5)
            r.raise_for_status()
            data = r.json()
            break
        except Exception as exc:  # noqa: BLE001
        ...
        raise RuntimeError(f'Ningún espejo de Overpass respondió: {ultimo_error}')

    # Reclasificar cada elemento devuelto según a cuál de las OSM_CONSULTAS
    # pertenece, mirando sus tags (Overpass no nos dice directamente "esto
    # vino del filtro N", así que lo inferimos de nuevo con las mismas claves).
    resultado = {etiqueta: [] for etiqueta, _ in consultas}
    for el in data.get('elements', []):
        ...
        if tags.get('building') in ('house', 'residential', 'detached', 'farm'):
            resultado['viviendas'].append({'lat': lat, 'lon': lon, 'tags': tags})
        elif tags.get('building') == 'farm_auxiliary':
            resultado['establecimientos_rurales'].append({'lat': lat, 'lon': lon, 'tags': tags})
        elif tags.get('amenity') == 'school':
            resultado['escuelas'].append({'lat': lat, 'lon': lon, 'tags': tags})
        elif tags.get('amenity') in ('hospital', 'clinic'):
            resultado['hospitales'].append({'lat': lat, 'lon': lon, 'tags': tags})
        elif tags.get('power') == 'line':
            resultado['lineas_electricas'].append({'lat': lat, 'lon': lon, 'tags': tags})
        elif tags.get('highway') in ('motorway', 'trunk', 'primary', 'secondary', 'tertiary'):
            resultado['rutas'].append({'lat': lat, 'lon': lon, 'tags': tags})
        elif tags.get('bridge') == 'yes':
            resultado['puentes'].append({'lat': lat, 'lon': lon, 'tags': tags})
        elif tags.get('boundary') == 'protected_area':
            resultado['areas_protegidas'].append({'lat': lat, 'lon': lon, 'tags': tags})

    return resultado


@app.route('/infraestructura_osm')
def infraestructura_osm():
    """
    Elementos de OpenStreetMap (viviendas, escuelas, rutas, líneas
    eléctricas, etc.) dentro de un radio de un punto.
    Parámetros: lat, lon, radius_km (default 5)

    ⚠️ LIMITACIÓN CONOCIDA (no es un bug, es cobertura real de OSM): en
    zonas rurales de Argentina el mapeo comunitario de OpenStreetMap es
    desparejo. Un resultado de "0 viviendas" puede significar que
    realmente no hay viviendas, O que hay viviendas reales que nadie
    cargó todavía en OSM. Esto se lo debe comunicar el frontend al
    usuario, no asumir que la ausencia de datos es ausencia de riesgo.
    """
    try:
        lat = float(request.args.get('lat'))
        lon = float(request.args.get('lon'))
        ...
        # Círculo aproximado como polígono (36 vértices) para pasarle a Overpass.
        n_vertices = 36
        radio_grados_lat = radius_km / 111.0  # aproximación esférica simple
        coords = []
        for i in range(n_vertices + 1):
            ang = 2 * math.pi * i / n_vertices
            dlat = radio_grados_lat * math.cos(ang)
            dlon = (radius_km / (111.0 * max(math.cos(math.radians(lat)), 0.1))) * math.sin(ang)
            coords.append([lon + dlon, lat + dlat])

        elementos = _overpass_query(coords)
        conteos = {etiqueta: len(lista) for etiqueta, lista in elementos.items()}
        ...
        return jsonify({
            ...
            'conteos': conteos,
            'elementos': elementos,
            'advertencia': ('La cobertura de OSM en zonas rurales de Argentina es '
                             'desigual. Un conteo en 0 no garantiza ausencia real del '
                             'elemento, solo que no está cargado en OSM.'),
        })
    except Exception as exc:  # noqa: BLE001
        return jsonify({'error': str(exc)}), 500
