...
def inundacion_dinamica():
    ...
    # Rechazo temprano: si el radio pedido (ej. un slider de la UI pensado
    # para el HAND estático, que sí banca 50-250km) supera lo que la
    # simulación dinámica puede correr dentro de su timeout, devolver un
    # 400 claro ahora es mucho más barato que descargar el DEM, armar la
    # grilla y recién enterarse 60s después (ver RADIO_MAXIMO_KM en
    # lisflood_engine.py).
    if radio_km is not None and radio_km > lisflood_engine.RADIO_MAXIMO_KM:
        return jsonify({
            'error': (f'radio_km={radio_km} es demasiado grande para la simulación dinámica '
                      f'(LISFLOOD-FP). Máximo soportado: {lisflood_engine.RADIO_MAXIMO_KM} km. '
                      f'Para áreas más grandes usá /inundacion_local (HAND estático).'),
        }), 400
