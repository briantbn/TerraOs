...
# criterio de "candidata a inundarse" en zona_baja/anegamiento/velocidad_flujo.
#
...
def _preparar_hand_y_agua(lat, lon, radius_km, region=None):
    """HAND (MERIT Hydro) + agua permanente (JRC), recortados a la región."""
    if region is None:
        region = ee.Geometry.Point([lon, lat]).buffer(radius_km * 1000)
    hand = ee.Image('MERIT/Hydro/v1_0_1').select('hnd').clip(region)
    agua_fuente = (ee.Image('JRC/GSW1_4/GlobalSurfaceWater')
                     .select('occurrence')
                     .gt(50)
                     .clip(region))
...
def inundacion_tiles():
    ...
    Parámetros:
        ...
        radius_km  radio del área (default: 200 km, TOPE: 800 km — ver
                   ...
                   constante)
        umbral_m   SOLO para tipo=zona_baja/anegamiento/velocidad_flujo.
    ...
    Nota sobre 'zona_baja'/'anegamiento'/'velocidad_flujo' (usados por el
    ...
    try:
        ...
        if tipo == 'riesgo':
            ...
            candidatas, zona_conectada, _costo, conectividad_aplicada = (
                _conectividad_hidraulica(hand, agua_fuente, region, radius_km, umbral)
            )
            ...
            # celda inundada, en el mismo marco de referencia que HAND
            # (metros sobre el drenaje más cercano). Fuera de la zona
            # conectada, se enmascara (sin agua, sin velocidad).
