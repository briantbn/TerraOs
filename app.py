...
def inundacion_tiles():
    ...
    try:
        ...
        # ── Agua permanente conocida (semilla para conectividad hidráulica) ──
        # JRC Global Surface Water: píxeles con >50% de ocurrencia histórica
        # de agua (1984-2021). Se usa tanto para 'zona_baja' como para
        # 'anegamiento' (ver _conectividad_hidraulica más abajo).
        agua_fuente = (ee.Image('JRC/GSW1_4/GlobalSurfaceWater')
                         .select('occurrence')
                         .gt(50)
                         .clip(region))

        def _conectividad_hidraulica(umbral_elev):
            """
            Dado un umbral de elevación, devuelve:
              candidatas         -> todas las celdas <= umbral (bathtub-fill puro)
              zona_conectada     -> subconjunto alcanzable desde agua real sin
                                     cruzar terreno por encima del umbral
              conectividad_ok    -> True si se pudo propagar (había agua_fuente
                                     visible en la región consultada)
            Ver notas de rendimiento en el bloque original de 'zona_baja':
            se reproyecta a 90 m y se topea la propagación a 50 km.
            """
        ...
        if tipo == 'riesgo':
            ...
            candidatas, zona_conectada, conectividad_aplicada = _conectividad_hidraulica(umbral)
