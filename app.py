...
def _dem_composite_correccion_dosel(elevacion, region):
    ...
    es_arboles = lc.eq(10)                        # WorldCover: 10 = cobertura arbórea

    # 4. Factor dinámico de corrección
    # Limitar CHM si supera 1.5x el desnivel local, PERO solo fuera de zonas
    # de cobertura arbórea confirmada (WorldCover clase 10). Este límite
    # busca descartar lecturas de CHM que sean ruido del dataset (anomalías
    # aisladas), pero en un monte cerrado y uniforme (ej. Parque Nacional
    # Mburucuyá) el relieve local del DSM crudo sale bajo justamente porque
    # el dosel es parejo -- no porque no haya árboles. Aplicar el tope ahí
    # capaba el CHM real (15-25 m) a ~2 m y dejaba la corrección casi nula,
    # justo donde más hace falta. Donde WorldCover confirma árboles, se
    # confía en el CHM real sin recortarlo por relieve local.
    cap_anomalia_chm = local_relief.multiply(1.5).max(2)
    chm_limitado = chm.where(es_arboles.Not(), chm.min(cap_anomalia_chm))
