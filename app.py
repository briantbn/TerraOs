...
# ── Calibración de escenarios históricos del Simulador de Inundación ──────
# Adaptado del script GEE del usuario ("Simulación GeoSentinel"): en vez de
# mandar la lectura cruda de la escala hidrométrica de una estación (que es
# una cota absoluta puntual) como si fuera "metros de subida HAND" en
# cualquier parte del mapa, se mira qué HAND tenían realmente los píxeles
# que el satélite observó bajo agua durante ese evento histórico — así el
# umbral que se aplica es específico de la zona, no de la estación.
#
# JRC/GSW1_4/MonthlyHistory solo cubre marzo-1984 a dic-2021: la crecida de
# 1983 queda fuera de rango y no hay SAR de esa fecha tampoco, así que NO se
# puede calibrar contra observación satelital real. Para 1983 no hay entrada
# 'jrc' — el llamador debe usar la lectura de estación tal cual y avisarlo.
_gsw_monthly_hist = ee.ImageCollection('JRC/GSW1_4/MonthlyHistory')

EVENTOS_HISTORICOS_INUNDACION = {
    # Ventana de año completo (no solo el mes del pico real): JRC es óptico
    # y una ventana angosta puede perderse el evento por nubes — mismo
    # criterio que usa el script original del usuario.
    '1998': {'inicio': '1998-01-01', 'fin': '1998-12-31'},
    '2013': {'inicio': '2013-01-01', 'fin': '2013-12-31'},
}

# JRC/GSW1_4/MonthlyHistory no cubre 2026: para el evento "reciente" el
# script del usuario usa Sentinel-1 SAR (caída de backscatter = agua),
# comparando una ventana "antes" contra la ventana "durante" el evento.
SAR_ANTES_INICIO   = '2025-08-01'
SAR_ANTES_FIN      = '2025-09-30'
SAR_DURANTE_INICIO = '2026-04-01'
SAR_DURANTE_FIN    = '2026-04-30'
UMBRAL_SAR_DB      = -3  # caída típica de backscatter (dB) cuando aparece agua

UMBRAL_FRECUENCIA_JRC_HIST = 30  # % mínimo de observaciones despejadas como agua
PERCENTIL_CALIBRACION_HIST = 90  # percentil alto: ignora ruido de borde

_cache_calibracion_escenarios = {}  # in-memory: se recalcula solo si se pierde (redeploy/restart)


def _agua_evento_jrc(inicio, fin, umbral_frecuencia, region):
    """Máscara de 'estuvo bajo agua durante este evento' (misma lógica que
    aguaEventoJRC() del script GEE del usuario)."""
    coleccion  = _gsw_monthly_hist.filterDate(inicio, fin).filterBounds(region)
    es_valido  = coleccion.map(lambda img: img.select('water').gt(0))
    es_agua    = coleccion.map(lambda img: img.select('water').eq(2))
    frecuencia = es_agua.sum().divide(es_valido.sum()).multiply(100)
    return frecuencia.gte(umbral_frecuencia).clip(region).selfMask().rename('agua')


def _agua_evento_sar_reciente(region, agua_permanente_region):
    """Máscara de 'estuvo bajo agua durante el evento reciente' vía Sentinel-1
    SAR (misma lógica que construirEventoSAR() del script GEE del usuario):
    compara backscatter VV antes/durante, y suma el agua permanente conocida
    (JRC) para no perder cobertura donde el SAR sea ruidoso. Devuelve None si
    no hay escenas suficientes en la zona/ventanas (no rompe el resto)."""
    def coleccion_s1(pasada):
        return (ee.ImageCollection('COPERNICUS/S1_GRD')
                  .filterBounds(region)
                  .filter(ee.Filter.eq('instrumentMode', 'IW'))
                  .filter(ee.Filter.listContains('transmitterReceiverPolarisation', 'VV'))
                  .filter(ee.Filter.eq('orbitProperties_pass', pasada))
                  .select('VV'))

    s1_desc = coleccion_s1('DESCENDING')
    s1_asc  = coleccion_s1('ASCENDING')

    n_antes_desc   = s1_desc.filterDate(SAR_ANTES_INICIO, SAR_ANTES_FIN).size().getInfo()
    n_durante_desc = s1_desc.filterDate(SAR_DURANTE_INICIO, SAR_DURANTE_FIN).size().getInfo()
    n_antes_asc    = s1_asc.filterDate(SAR_ANTES_INICIO, SAR_ANTES_FIN).size().getInfo()
    n_durante_asc  = s1_asc.filterDate(SAR_DURANTE_INICIO, SAR_DURANTE_FIN).size().getInfo()

    usar_desc = n_antes_desc > 0 and n_durante_desc > 0
    usar_asc  = n_antes_asc > 0 and n_durante_asc > 0
    if not usar_desc and not usar_asc:
        return None

    s1_elegida = s1_desc if usar_desc else s1_asc
    suavizar = lambda img: img.focal_median(50, 'circle', 'meters')

    antes_suave   = suavizar(s1_elegida.filterDate(SAR_ANTES_INICIO, SAR_ANTES_FIN).median()).clip(region)
    durante_suave = suavizar(s1_elegida.filterDate(SAR_DURANTE_INICIO, SAR_DURANTE_FIN).median()).clip(region)
    diferencia_db = durante_suave.subtract(antes_suave)

    return (diferencia_db.lt(UMBRAL_SAR_DB)
            .Or(agua_permanente_region)
            .clip(region)
            .selfMask()
            .rename('agua'))


def _calibrar_umbral_hand(hand, mascara_agua, region, escala=90, tile_scale=4):
    """Percentil 90 del HAND bajo los píxeles que el satélite observó
    inundados en ese evento (equivalente a calibrarUmbral() del script,
    pero sobre HAND en vez de elevación absoluta: el resultado es
    directamente el umbral_m que ya consume el simulador)."""
    hand_bajo_agua = hand.updateMask(mascara_agua)
    stats = hand_bajo_agua.reduceRegion(
        reducer=ee.Reducer.percentile([PERCENTIL_CALIBRACION_HIST]),
        geometry=region, scale=escala, maxPixels=1e13, bestEffort=True, tileScale=tile_scale
    )
    return stats.get('hand')


def obtener_umbral_calibrado_evento(nombre_evento, lat, lon, radio_km=150):
    """Umbral_m calibrado (subida HAND real observada por satélite) para un
    escenario histórico. Cachea en memoria por evento + zona aproximada
    (redondeo a ~0.1° para no recalibrar puntos casi idénticos). Devuelve
    None si el evento no tiene forma de calibrarse en esa zona (sin escenas
    SAR suficientes, sin píxeles de agua JRC, etc.) — el llamador debe tener
    un fallback."""
    if nombre_evento not in EVENTOS_HISTORICOS_INUNDACION and nombre_evento != 'abril_2026':
        return None

    clave = (nombre_evento, round(lat, 1), round(lon, 1), radio_km)
    if clave in _cache_calibracion_escenarios:
        return _cache_calibracion_escenarios[clave]

    try:
        region = ee.Geometry.Point([lon, lat]).buffer(radio_km * 1000)
        _, hand, agua_permanente = _preparar_hand_y_agua(lat, lon, radio_km, region=region)

        if nombre_evento == 'abril_2026':
            mascara = _agua_evento_sar_reciente(region, agua_permanente)
            if mascara is None:
                _cache_calibracion_escenarios[clave] = None
                return None
        else:
            evento = EVENTOS_HISTORICOS_INUNDACION[nombre_evento]
            mascara = _agua_evento_jrc(evento['inicio'], evento['fin'], UMBRAL_FRECUENCIA_JRC_HIST, region)

        valor = _calibrar_umbral_hand(hand, mascara, region).getInfo()
        umbral = round(valor, 2) if valor is not None else None
    except ee.EEException:
        umbral = None

    _cache_calibracion_escenarios[clave] = umbral
    return umbral


@app.route('/calibrar_escenario_inundacion')
def calibrar_escenario_inundacion():
    """
    Devuelve el umbral_m (subida HAND) calibrado contra el agua real
    observada por satélite durante un evento histórico, para el Simulador
    de Inundación. Uso exclusivo de ese simulador — no reemplaza umbral_m
    manual en /inundacion_animacion ni en /zona_baja.

    Parámetros: evento ('1998' | '2013' | 'abril_2026'), lat, lon
    Devuelve: { ok, umbral_m, calibrado: bool, motivo? }
    """
    if not _ee_ready:
    ...
    try:
        ...
        return jsonify({'ok': False, 'error': 'Parámetros evento, lat y lon requeridos'}), 400

    if evento not in EVENTOS_HISTORICOS_INUNDACION and evento != 'abril_2026':
        return jsonify({
            'ok': True, 'umbral_m': None, 'calibrado': False,
            'motivo': f'El evento {evento} no está entre los calibrables (1998, 2013, abril_2026).',
        })

    umbral = obtener_umbral_calibrado_evento(evento, lat, lon)
    if umbral is None:
        motivo = ('No se encontraron escenas Sentinel-1 suficientes en esta zona para el evento reciente.'
                   if evento == 'abril_2026' else
                   'No se pudo calibrar (sin píxeles de agua detectados en la zona/evento consultado).')
        return jsonify({'ok': True, 'umbral_m': None, 'calibrado': False, 'motivo': motivo})
    return jsonify({'ok': True, 'umbral_m': umbral, 'calibrado': True})
...
def interpretar_riesgo():
    ...
    usuario. Recibe un único JSON con los datos del lote/análisis (campo,
    clima, incendio, inundación, sequía, índice general) y devuelve un
    único párrafo técnico dirigido a quien va a actuar en el campo
    (ingeniero agrónomo, ingeniero forestal, bombero).
    ...
    prompt_sistema = (
        ...
        "sin viñetas), los datos de riesgo de incendio, inundación y sequía que "
        "recibís en JSON. El texto lo va a leer un ingeniero agrónomo, un "
        "ingeniero forestal o un bombero que puede necesitar actuar en el campo "
        "a partir de esto. Priorizá, en este orden: qué está pasando, qué tan "
        "urgente es, y en qué conviene fijarse o actuar primero. Las "
        "recomendaciones que hagas tienen que estar ancladas a los datos "
        "recibidos (ej. distancia y ubicación de un foco, dirección del "
        "viento, nivel de riesgo hídrico), nunca genéricas.\n\n"
        "Reglas estrictas:\n"
...
def inundacion_punto():
    ...
    try:
        ...
        # HAND (altura sobre el drenaje más cercano) — mismo cálculo que ya usan
        # zona_baja/anegamiento (MERIT Hydro, corregido por dosel/edificios).
        # Se agrega acá como dato adicional (no reemplaza elevación/pendiente,
        # que siguen siendo informativas) para que el panel de Riesgo
        # Hidrológico del Suelo pueda usarla como señal de "qué tan cerca está
        # este punto de un cauce real", algo que la posición relativa dentro
        # del campo por sí sola no puede saber.
        _, hand, _ = _preparar_hand_y_agua(lat, lon, 200, region=region)
