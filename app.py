...
def _dem_composite_correccion_dosel(elevacion, region):
    ...
    proxy de "esto es bosque"), esta versión usa:
      ...
      1. CHM real (NASA JPL Global Forest Canopy Height) en vez de un proxy.
      ...
      4. Corrección específica para EDIFICIOS (ESA WorldCover, clase
         urbana): ahí el "CHM" que aparece no es dosel, es techo, y se le
         aplica un factor distinto (0.80) en vez del factor de vegetación.
         (Nota: esto solo cubre el caso donde el CHM ya capta la altura del
         edificio; para tu script aparte de edificios, avisame cuando lo
         quieras sumar.)
    ...
    Devuelve un ee.Image con las bandas:
      ...
      correctionFactor, correctionApplied, landCover
    """
    ...
    chm = (ee.Image('NASA/JPL/global_forest_canopy_height_2005')
             .select('1').clip(region))
    ...
    # Limitar CHM si supera 1.5x el desnivel local (evita anomalías del CHM)
    chm_limitado = chm.min(local_relief.multiply(1.5).max(2))
    ...
    # En zonas urbanas restamos ~80% (edificio, no dosel); en bosque
    # recién perdido no restamos nada (el árbol ya no está).
    factor = (factor_dinamico
              .where(es_urbano, 0.80)
              .where(bosque_perdido_reciente, 0)
              .clamp(0, 0.85)
              .rename('correctionFactor'))

    # 5. Corrección parcial del DEM
    correccion_aplicada = chm_limitado.multiply(factor).rename('correctionApplied')
...
def inundacion_punto():
    ...
    try:
        ...
        vals  = (riesgo_compuesto
                   .addBands(elevacion)
                   .addBands(pendiente)
                   .addBands(dosel.select(['confidence', 'canopyHeight', 'correctionApplied']))
                   .reduceRegion(
