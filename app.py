...
class _LazyModule:
    """Envoltorio que difiere el import real de un módulo pesado (rasterio,
    pysheds, numpy, scikit-image, etc. como dependencias transitivas) hasta
    el primer uso efectivo del endpoint que lo necesita.

    - `.disponible` hace un chequeo BARATO: solo busca si el archivo/paquete
      existe (importlib.util.find_spec), sin ejecutar su código ni reservar
      memoria. Sirve para decidir si un endpoint debe devolver 503 al toque.
    - El import real y costoso recién ocurre la primera vez que se accede a
      un atributo del módulo (ej. `hidro_vectorial.foo(...)`), gracias a
      __getattr__. Después queda cacheado para las siguientes llamadas.

    Con esto, arrancar el server (gunicorn) ya no carga rasterio/pysheds/etc.
    de entrada — cada módulo pesado solo entra a memoria cuando alguien
    efectivamente pega al endpoint que lo usa.
    """
    ...
    def _cargar(self):
        if self._mod is None and self._error is None:
            try:
                self._mod = importlib.import_module(self._name)
                print(f'✅ Import diferido de "{self._name}" cargado (primer uso).')
            except Exception as exc:  # noqa: BLE001
                self._error = exc
                print(f'⚠️ Falló el import diferido de "{self._name}": {exc}')
...
_motor_inundacion = _LazyModule('flood_simulation_engine')
_MOTOR_DISPONIBLE = _motor_inundacion.disponible
...
hidro_vectorial = _LazyModule('hidrografia_vectorial')
_hidro_vectorial_disponible = hidro_vectorial.disponible

# ──────────────────────────────────────────────────────────────────────────
# MÓDULO OPCIONAL: suelos_inta_regional
# ──────────────────────────────────────────────────────────────────────────
# Mismo patrón defensivo que hidrografia_vectorial: si el archivo no está
# desplegado junto a este app.py, solo se desactiva /suelo_mejor_resolucion
# (con un mensaje claro) — el resto de la app sigue funcionando.
suelos_inta_regional = _LazyModule('suelos_inta_regional')
_suelos_regional_disponible = suelos_inta_regional.disponible

# ──────────────────────────────────────────────────────────────────────────
# MÓDULO OPCIONAL: humedales_inta
# ──────────────────────────────────────────────────────────────────────────
# Mismo patrón defensivo. Además de faltar el archivo, acá puede fallar por
# no tener 'rasterio' instalado (agregar 'rasterio' a requirements.txt) —
# ambos casos se manejan igual: se desactiva /humedal_punto, no toda la app.
humedales_inta = _LazyModule('humedales_inta')
_humedales_disponible = humedales_inta.disponible

# ──────────────────────────────────────────────────────────────────────────
# MÓDULO OPCIONAL: terreno_merit_hydro
# ──────────────────────────────────────────────────────────────────────────
# Mismo patrón defensivo. Además de rasterio, usa numpy (diferencias finitas
# para la pendiente) — agregar 'numpy' a requirements.txt si no viene ya
# como dependencia transitiva de rasterio.
terreno_merit_hydro = _LazyModule('terreno_merit_hydro')
_terreno_disponible = terreno_merit_hydro.disponible

# ──────────────────────────────────────────────────────────────────────────
# MÓDULO OPCIONAL: local_flood_engine
# ──────────────────────────────────────────────────────────────────────────
# HAND calculado localmente (Copernicus GLO-30 + pysheds), SIN Earth Engine,
# para /inundacion_local — reemplaza a /inundacion_tiles (tipo=zona_baja/
# anegamiento) en el Simulador de Inundación del frontend. A diferencia de
# terreno_merit_hydro/humedales_inta, este módulo YA envuelve su propio
# import de rasterio/pysheds en un try/except interno (ver
# _MOTOR_LOCAL_DISPONIBLE en local_flood_engine.py) y no levanta ImportError
# por eso — pero igual se envuelve acá por si falta 'numpy' (import sin
# guardar en el módulo) o el archivo no está desplegado.
local_flood_engine = _LazyModule('local_flood_engine')
lisflood_engine = _LazyModule('lisflood_engine')
cuenca_engine = _LazyModule('cuenca_engine')


def _flood_local_disponible_check():
    """A diferencia de los demás módulos opcionales, local_flood_engine
    expone un flag interno (_MOTOR_LOCAL_DISPONIBLE) que solo se puede leer
    DESPUÉS de importarlo de verdad (no alcanza con find_spec). Por eso esta
    función fuerza el import diferido recién cuando /inundacion_local la
    llama — no al arrancar el server — y de ahí en más queda cacheado.

    Nota: el módulo se importa bien como archivo Python SIEMPRE (envuelve su
    propio import de pysheds/rasterio en un try/except interno), así que
    _LazyModule._cargar() nunca ve una excepción acá — el print de "Falló
    el import diferido" de esa clase NO se dispara para este caso. La razón
    real de por qué pysheds/rasterio no cargaron queda en
    local_flood_engine._error_import, y hasta este cambio no se mostraba en
    ningún lado — de ahí que el 503 fuera un misterio."""
    try:
        mod = local_flood_engine._cargar()
        disponible = bool(mod._MOTOR_LOCAL_DISPONIBLE)
        if not disponible:
            print(f'⚠️ [local_flood_engine] Motor local no disponible: {mod._error_import}')
        return disponible
    except Exception as exc:
        print(f'⚠️ [local_flood_engine] Excepción al chequear disponibilidad: {exc}')
        return False


def _flood_local_error_detalle():
    """Texto del error real (pysheds/rasterio faltante, versión incompatible,
    etc.) para incluir en la respuesta 503 — antes quedaba oculto."""
    try:
        mod = local_flood_engine._cargar()
        return getattr(mod, '_error_import', None)
    except Exception as exc:
        return str(exc)

# ──────────────────────────────────────────────────────────────────────────
# MÓDULO OPCIONAL: jrc_raster_regional
# ──────────────────────────────────────────────────────────────────────────
# Mismo patrón defensivo. Envoltura histórica del agua (occurrence +
# recurrence + seasonality + transitions, JRC GSW v1.5 2024) leída por
# ventana desde COGs propios alojados en Hugging Face — no depende de GEE.
# Además de rasterio (ya usado por humedales_inta y terreno_merit_hydro),
# necesita 'shapely' en requirements.txt.
jrc_raster_regional = _LazyModule('jrc_raster_regional')
_jrc_raster_disponible = jrc_raster_regional.disponible

# ──────────────────────────────────────────────────────────────────────────
# MÓDULO OPCIONAL: jrc_memoria_hidrica
# ──────────────────────────────────────────────────────────────────────────
jrc_memoria_hidrica = _LazyModule('jrc_memoria_hidrica')
_jrc_disponible = jrc_memoria_hidrica.disponible

# ──────────────────────────────────────────────────────────────────────────
# MÓDULO OPCIONAL: enso_oni
# ──────────────────────────────────────────────────────────────────────────
enso_oni = _LazyModule('enso_oni')
...
def _conectividad_hidraulica(elevacion, agua_fuente, region, radius_km, umbral_elev):
    ...
    RENDIMIENTO: se reproyecta a 90 m y se topea la propagación a 25 km
    (cumulativeCost es caro — ver nota extendida en el commit del punto 1).
    Antes el tope era 50 km; se bajó porque en zonas con mucho terreno
    bajo/conectado (esteros, humedales) la propagación completa se volvía
    tan pesada que Earth Engine rechazaba calcular algunos tiles (HTTP 400)
    en vez de devolverlos. 25 km sigue siendo un radio realista para "qué
    tan lejos del río puede llegar el agua" en la inmensa mayoría de los
    casos de uso de este simulador.
    """
...
def _simular_inundacion(hand, agua_fuente, region, radius_km, umbral_m, perfil=None,
    ...
    `lat`/`lon` son opcionales — solo se usan para que FloodSimulationEngine
    pueda cachear el perfil regional entre pedidos sobre la misma zona (ver
    simulate_flood en flood_simulation_engine.py). No afectan el fallback.
...
#
# Estrategia: identificar automáticamente el PAÍS (nivel 0) que contiene el
# punto analizado, usando el dataset global FAO GAUL 2015 (nivel 0 para el
# polígono de filtrado, nivel 1 solo para mostrar el nombre de provincia/
# estado en pantalla) y devolver la geometría del país al frontend, que la usa
# para filtrar los focos por intersección espacial real (punto-en-polígono),
# no por distancia ni por bounding box. Se filtra por país entero (no por
# provincia/estado) para no descartar focos reales cercanos solo por caer del
# otro lado del límite provincial/estatal; el límite de país sigue evitando
# que aparezcan focos de un país vecino.
...
_gaul_level0_fc = None       # cache del FeatureCollection de PAÍSES (GAUL nivel 0)
...
def _gaul_level0():
    """FeatureCollection global de límites de PAÍS (FAO GAUL 2015 nivel 0).
    Se usa para el polígono de filtrado de focos (más amplio que la provincia/
    estado), mientras que el nombre de provincia/estado que se muestra en
    pantalla se sigue sacando de nivel 1. Se pide una sola vez y se reutiliza."""
    global _gaul_level0_fc
    if _gaul_level0_fc is None:
        _gaul_level0_fc = ee.FeatureCollection('FAO/GAUL/2015/level0')
    return _gaul_level0_fc
...
def getAdministrativeBoundary(lat, lon):
    ...
    que lo contiene, usando el dataset mundial FAO GAUL 2015 (nivel 1 para el
    nombre de provincia/estado, nivel 0 —el país completo— para el polígono
    que el frontend usa para filtrar los focos). Se filtra por país y no por
    provincia/estado para no cortar de golpe focos que están cerca del punto
    analizado pero del otro lado del límite provincial/estatal; el límite de
    país sigue evitando que aparezcan focos de un país vecino.

    Devuelve un dict:
      - ok=True  -> {ok, pais, jurisdiccion, nivel, geometry}  (geometry = GeoJSON
        simplificado del polígono DEL PAÍS, para que el frontend haga el
        filtrado punto-en-polígono de los focos)
    ...
    # Nivel 1 (provincia/estado): solo para el nombre informativo en pantalla.
    feature1 = _gaul_level1().filterBounds(punto).first()
    info1 = feature1.getInfo()

    # Nivel 0 (país): el que realmente se usa para el polígono de filtrado,
    # bastante más amplio que la provincia/estado. Se consulta aparte porque
    # algunos países chicos no tienen cobertura en el dataset de nivel 1.
    feature0 = _gaul_level0().filterBounds(punto).first()
    info0 = feature0.getInfo()
    ...
    if info0 is None:
        ...
        pais = getCountry(info0)
        jurisdiccion = getJurisdiction(info1) if info1 is not None else None
        # Simplificar la geometría antes de traerla al cliente. El polígono de
        # un país entero (EE.UU., Canadá, Brasil, etc.) puede tener muchísimos
        # vértices, más que una sola provincia/estado, así que acá usamos más
        # tolerancia que para nivel 1 — para un filtro punto-en-polígono de
        # focos de incendio no hace falta precisión de metros en toda la
        # costa/frontera del país, y así evitamos inflar la respuesta y la
        # latencia. Sigue siendo lo bastante fiel como para no descartar por
        # error focos que están cerca del límite real del país.
        geometria = feature0.geometry().simplify(500).getInfo()  # tolerancia ~500 m
        resultado = {
            'ok': True,
            'pais': pais,
            'jurisdiccion': jurisdiccion if jurisdiccion is not None else pais,
            'nivel': _NIVEL1_LABELS_POR_PAIS.get(pais, 'División administrativa') if jurisdiccion is not None else 'País',
            'geometry': geometria,
        }
...
# ──────────────────────────────────────────────────────────────────────────
# MODELO DE PROPAGACIÓN DE INCENDIOS — McArthur Mk5 (bosque y pastizal)
# ──────────────────────────────────────────────────────────────────────────
# Reemplaza la fórmula lineal inventada que usaba el cono de propagación
# del frontend (velocidad = viento*0.35, sin sustento científico).
#
# Fuente primaria de TODAS las ecuaciones de esta sección (verificado
# contra el paper original, no una fuente secundaria):
...
#
# El paper da fórmulas separadas para bosque (Forest Fire Danger Meter Mk5)
# y pastizal (Grassland Fire Danger Meter Mk5), y una fórmula común para el
# "drought factor" D (0-10) usado en la de bosque. D reutiliza el KBDI que
# ya calculábamos para el índice Ångström/FWI (mismo valor en mm, sin
# duplicar trabajo).
#
# Lo que ESTE módulo NO es (para ser honestos si esto se muestra a
# profesionales): McArthur Mk5 es de los años 60-70, sabidamente subestima
# la velocidad real en incendios rápidos por un factor de 2-3x (Burrows
# 1999; McCaw et al. 2008) y fue reemplazado operacionalmente en Australia
# por el AFDRS en 2022. Se eligió acá por ser gratuito, liviano (sin tablas
# de 17-40 modelos de combustible) y fácil de instalar — no por ser el
# estado del arte. Es un piso científico razonable y citable, no el techo.
# ──────────────────────────────────────────────────────────────────────────

# Carga de combustible fino (W, toneladas/hectárea) por clase de
# ESA WorldCover v200. El valor de bosque (12.5 t/ha) es el que usa el
# propio Mk5 como referencia estándar (CSIRO, "high eucalypt forest").
# Pastizal (4.5 t/ha) y el resto son valores genéricos de referencia
# (no medidos localmente) — si en algún momento hay un inventario de
# combustible real de la zona, reemplazar esto por ese dato.
_FUEL_LOAD_T_HA_POR_WORLDCOVER = {
    10: 12.5,  # Árboles/bosque — carga de referencia del propio Mk5
    20: 8.0,   # Arbustal
    30: 4.5,   # Pastizal
    40: 3.0,   # Cultivo (rastrojo)
    90: 4.5,   # Humedal herbáceo — se trata como pastizal
    95: 8.0,   # Manglar — se trata como arbustal
}
# Clases WorldCover que se modelan con la fórmula de PASTIZAL (Grassland
# Mk5) en vez de la de BOSQUE (Forest Mk5) — el resto de las que están en
# el diccionario de arriba usa bosque.
_CLASES_WORLDCOVER_PASTIZAL = {30, 40, 90}
# Clases sin combustible fino relevante (urbano, agua, nieve, desnudo):
# no tiene sentido calcular un frente de fuego ahí.
_CLASES_WORLDCOVER_SIN_COMBUSTIBLE = {50, 60, 70, 80, 100}


def _dias_desde_lluvia_y_monto(dias_clima):
    """
    A partir de la serie diaria de precipitación (más antigua -> más
    reciente, mismo formato que _obtener_clima_diario_openmeteo), calcula
    N (días desde el último evento de lluvia significativa) y P (mm
    totales de ese evento) — los dos insumos que pide la fórmula de
    drought factor de Noble et al. (1980), Tabla 1.
    Umbral de "lluvia significativa": 2mm/día (uso estándar en la
    literatura del FFDI, ej. Finkele et al. 2006).
    Si no hay ningún día de lluvia >=2mm en toda la ventana, devuelve
    N = tamaño de la ventana (censurado: no sabemos hace cuánto llovió
    de verdad más allá de lo que pedimos a Open-Meteo) y P = 0.
    """
    n = 0
    idx = len(dias_clima) - 1
    while idx >= 0 and (dias_clima[idx]['lluvia_mm'] or 0.0) < 2.0:
        n += 1
        idx -= 1
    if idx < 0:
        return n, 0.0
    p = 0.0
    while idx >= 0 and (dias_clima[idx]['lluvia_mm'] or 0.0) >= 2.0:
        p += dias_clima[idx]['lluvia_mm'] or 0.0
        idx -= 1
    return n, p


def _drought_factor_mcarthur(dias_clima, kbdi_mm):
    """
    Drought factor D (0-10), Noble, Bary & Gill (1980):
        D = 0.191*(I+104)*(N+1)^1.5 / (3.52*(N+1)^1.5 + P - 1)
    I = KBDI en mm equivalentes (0-203.2) -- el mismo kbdi_mm que ya
    calcula _calcular_indices_incendio_meteorologicos, reutilizado tal cual.
    """
    n, p = _dias_desde_lluvia_y_monto(dias_clima)
    denominador = 3.52 * (n + 1) ** 1.5 + p - 1.0
    if denominador <= 0:
        return 10.0
    d = (0.191 * (kbdi_mm + 104.0) * (n + 1) ** 1.5) / denominador
    return min(max(d, 0.0), 10.0)


def _ffdi_mcarthur(temp_c, hr_pct, viento_kmh, drought_factor):
    """FFDI — Forest Fire Danger Index, Mk5 (Noble et al. 1980, ec. exacta)."""
    d = max(drought_factor, 0.5)  # evita ln(0); D<0.5 ya da FFDI ~0 de por sí
    hr = min(max(hr_pct, 0.0), 100.0)
    return 2.0 * math.exp(-0.450 + 0.987 * math.log(d) - 0.0345 * hr + 0.0338 * temp_c + 0.0234 * viento_kmh)


def _gfdi_mcarthur(temp_c, hr_pct, viento_kmh, curado_pct, fuel_load_t_ha):
    """GFDI — Grassland Fire Danger Index, Mk5 (Noble et al. 1980, ec. exacta)."""
    hr = min(max(hr_pct, 0.0), 100.0)
    c = min(max(curado_pct, 20.0), 100.0)  # <20% de curado McArthur no lo cubre (pasto demasiado verde)
    m = (97.7 + 4.06 * hr) / (temp_c + 6.0) - 0.00854 * hr + 3000.0 / c - 30.0
    if m < 18.8:
        return 3.35 * fuel_load_t_ha * math.exp(-0.0897 * m + 0.0403 * viento_kmh)
    if m < 30.0:
        return 0.299 * fuel_load_t_ha * math.exp(-1.686 + 0.0403 * viento_kmh) * (30.0 - m)
    return 0.0  # >=30% de humedad de combustible: el pasto no sostiene fuego (fuera del rango del meter)


def _clasificar_indice_mcarthur(f):
    """
    Clases oficiales del FFDI (Bureau of Meteorology). Se aplican también
    al GFDI por analogía -- ambos índices de McArthur están construidos en
    una escala comparable, pero el GFDI no tiene bandas oficiales propias
    tan citadas como las del FFDI; si esto se presenta formalmente y hace
    falta precisión en pastizal, conviene validarlo con un profesional.
    """
    if f < 12: return 'Baja-Moderada'
    if f < 32: return 'Alta'
    if f < 50: return 'Muy alta'
    if f < 75: return 'Severa'
    if f < 100: return 'Extrema'
    return 'Catastrófica'


def _curado_estimado_desde_ndvi(ndvi):
    """
    Grado de curado del pastizal (C, %) estimado a partir del NDVI.
    NO es el algoritmo oficial de curado de pastizales (BoM/CSIRO usan
    curado satelital validado con calibración regional, ver Newnham et
    al. 2011) -- es un proxy simple y transparente: pasto muy verde
    (NDVI alto) = poco curado; pasto seco/amarillo (NDVI bajo) = muy
    curado. Documentado así para que quede claro qué parte del cálculo
    es la ecuación publicada exacta (McArthur/Noble) y qué parte es una
    aproximación nuestra.
    """
    if ndvi is None:
        return 60.0  # sin dato satelital: valor medio conservador
    ndvi = min(max(ndvi, 0.0), 0.85)
    return min(max(100.0 - ndvi * 120.0, 20.0), 100.0)


def _combustible_worldcover_ndvi(lat, lon, radius_m=300):
    """
    Consulta puntual (ESA WorldCover v200 + NDVI Sentinel-2 reciente) para
    saber qué modelo de combustible aplica en el punto del foco de
    incendio, y el NDVI para estimar curado si es pastizal.
    """
    if not _ee_ready:
        raise RuntimeError(f'Earth Engine no inicializado: {_ee_error}')

    punto = ee.Geometry.Point([lon, lat]).buffer(radius_m)
    lc = ee.ImageCollection('ESA/WorldCover/v200').mosaic().select('Map')
    clase_lc = lc.reduceRegion(
        reducer=ee.Reducer.mode(), geometry=punto, scale=10, bestEffort=True,
    ).get('Map').getInfo()
    ...
    if col.size().getInfo() > 0:
        ...
        stats = ndvi_img.reduceRegion(
            reducer=ee.Reducer.mean(), geometry=punto, scale=20, bestEffort=True,
        ).getInfo()
        ndvi_val = stats.get('B8')

    return clase_lc, ndvi_val


def calcular_propagacion_incendio(lat, lon, viento_kmh_actual=None, viento_dir_actual=None):
    """
    Punto de entrada del modelo completo: clima real + KBDI/drought factor
    + combustible (WorldCover) -> FFDI o GFDI según corresponda -> R
    (rate of spread, km/h, McArthur Mk5, Noble et al. 1980).
    """
    dias_kbdi = _obtener_clima_diario_openmeteo(lat, lon, KBDI_DIAS_ARRANQUE, forecast_days=1)
    if not dias_kbdi:
        raise ValueError('Open-Meteo no devolvió clima diario para este punto.')
    lluvia_anual_mm = _obtener_lluvia_anual_normal_openmeteo(lat, lon)
    kbdi_mm = 0.0
    ...
    ultimo = dias_kbdi[-1]
    temp_c = ultimo['temp_max']
    hr_pct = ultimo['hr_min']
    viento_kmh = viento_kmh_actual if viento_kmh_actual is not None else ultimo['viento_max_kmh']

    drought_factor = _drought_factor_mcarthur(dias_kbdi, kbdi_mm)

    clase_lc, ndvi_val = _combustible_worldcover_ndvi(lat, lon)

    if clase_lc in _CLASES_WORLDCOVER_SIN_COMBUSTIBLE or clase_lc is None:
        return {
            'ok': True,
            'modelo_aplicable': False,
            'motivo': 'Sin combustible fino relevante en el punto (urbano/agua/desnudo/nieve según WorldCover).',
            'worldcover_clase': clase_lc,
        }

    fuel_load = _FUEL_LOAD_T_HA_POR_WORLDCOVER.get(clase_lc, 6.0)
    es_pastizal = clase_lc in _CLASES_WORLDCOVER_PASTIZAL

    if es_pastizal:
        curado_pct = _curado_estimado_desde_ndvi(ndvi_val)
        indice = _gfdi_mcarthur(temp_c, hr_pct, viento_kmh, curado_pct, fuel_load)
        ros_kmh = 0.13 * indice  # Noble et al. 1980: R = 0.13*F para pastizal
        modelo = 'Grassland Fire Danger Meter Mk5'
    else:
        indice = _ffdi_mcarthur(temp_c, hr_pct, viento_kmh, drought_factor)
        ros_kmh = 0.0012 * indice * fuel_load  # Noble et al. 1980: R = 0.0012*F*W
        modelo = 'Forest Fire Danger Meter Mk5'

    ros_kmh = round(min(ros_kmh, 20.0), 2)  # tope de sensatez (McArthur ya subestima; nada por encima le da sustento)

    return {
        'ok': True,
        'modelo_aplicable': True,
        'modelo': modelo,
        'combustible': 'Pastizal/cultivo' if es_pastizal else 'Bosque/arbustal',
        'worldcover_clase': clase_lc,
        'fuel_load_t_ha': fuel_load,
        'drought_factor': round(drought_factor, 2) if not es_pastizal else None,
        'curado_pastizal_pct': round(curado_pct, 1) if es_pastizal else None,
        'ndvi': round(ndvi_val, 3) if ndvi_val is not None else None,
        'indice': round(indice, 1),
        'indice_clase': _clasificar_indice_mcarthur(indice),
        'ros_kmh': ros_kmh,
        'distancia_15min_km': round(ros_kmh * 0.25, 2),
        'distancia_30min_km': round(ros_kmh * 0.5, 2),
        'distancia_60min_km': round(ros_kmh * 1.0, 2),
        'temp_c': temp_c, 'hr_pct': hr_pct, 'viento_kmh': viento_kmh,
        'fuente': ('Noble, Bary & Gill (1980) "McArthur\'s fire-danger meters expressed as '
                   'equations", Australian Journal of Ecology 5, 201-203. Combustible: ESA '
                   'WorldCover v200. Limitación conocida del modelo Mk5: subestima la velocidad '
                   'real en incendios rápidos por ~2-3x (Burrows 1999; McCaw et al. 2008).'),
    }
...
def health():
    return jsonify({
        ...
        'lisflood_disponible': _lisflood_disponible_check(),
...
_PROMPT_BASE_INTERPRETACION = (
    ...
    "- No uses emojis, íconos, ni símbolos especiales en ningún lado del "
    "texto, ni siquiera entre paréntesis (ej. NUNCA escribas algo como "
    "'riesgo bajo ( )' con un símbolo adentro de los paréntesis, o vacío): "
    "el texto se inserta en un PDF que solo soporta texto plano "
    "(ASCII/Latin-1) y esos caracteres se rompen.\n"
    "- Si mencionás la regla 30-30-30 (temperatura, humedad, viento), "
    "escribila siempre así, con guiones: '30-30-30'. Nunca la escribas "
    "pegada como '303030' o sin separadores.\n"
    "- PRIORIDAD MÁXIMA: el párrafo debe terminar en una oración completa, "
    "con punto final. Es preferible un párrafo más corto y completo que uno "
    "más largo cortado a mitad de frase — si ves que te estás quedando sin "
    "espacio, cerrá la idea antes en vez de seguir agregando información.\n"
    "- Máximo 100 palabras (dejate margen: es un techo, no un objetivo a "
    "alcanzar siempre)."
)
...
def _interpretar_con_groq(prompt_sistema, datos):
    ...
    # el último punto/cierre de frase completo, para no mostrar una oración
    # a medias en el informe.
    finish_reason = respuesta.choices[0].finish_reason
    if finish_reason == 'length':
        ultimo_punto = max(texto.rfind('.'), texto.rfind('!'), texto.rfind('?'))
        if ultimo_punto > 20:  # evita dejar un párrafo demasiado corto/vacío
...
def jurisdiccion_area():
    """
    Identifica automáticamente el país que contiene el punto (lat, lon), usando
    el dataset mundial FAO GAUL 2015. La geometría devuelta es la del PAÍS
    completo (nivel 0); el nombre de provincia/estado (nivel 1) se incluye
    solo como dato informativo para mostrar en pantalla.

    Usado por la herramienta "Direcciones de Foco" del frontend para filtrar
    los focos NASA FIRMS y quedarse solo con los que pertenecen al mismo país
    que el área analizada, descartando focos de un país vecino aunque estén
    geográficamente cerca (ya no se corta en el límite de provincia/estado).
...
@app.route('/propagacion_incendio')
def propagacion_incendio():
    """
    Velocidad de propagación (ROS, km/h) y distancias a 15/30/60 min para
    el cono de un incendio activo, según el modelo McArthur Mk5 (Noble,
    Bary & Gill 1980) — bosque o pastizal, elegido automáticamente según
    ESA WorldCover en el punto. Reemplaza el cálculo lineal inventado que
    hacía el frontend (viento*0.35). Ver calcular_propagacion_incendio
    para las fuentes y las limitaciones conocidas del modelo.

    Parámetros: lat, lon, viento_kmh (opcional, viento actual del foco si
    el frontend ya lo tiene; si no se pasa, se usa el de Open-Meteo del día).
    """
    try:
        lat = float(request.args.get('lat'))
        lon = float(request.args.get('lon'))
        viento_kmh = request.args.get('viento_kmh')
        viento_kmh = float(viento_kmh) if viento_kmh not in (None, '') else None
        resultado = calcular_propagacion_incendio(lat, lon, viento_kmh_actual=viento_kmh)
        return jsonify(resultado)
    except requests.RequestException as exc:
        return jsonify({'error': f'Error consultando Open-Meteo: {exc}'}), 502
    except ee.EEException as exc:
...
@app.route('/incendios_goes')
def incendios_goes():
    """
    Focos de calor del satélite geoestacionario GOES-19 (GOES-East, cubre
    todo el continente americano) — producto oficial NOAA/NASA "ABI L2+
    Fire/Hot Spot Characterization" (FDCF, disco completo), disponible
    como ImageCollection de Earth Engine: NOAA/GOES/19/FDCF.

    Pensado como COMPLEMENTO de FIRMS (VIIRS/MODIS) en el frontend, no
    como reemplazo:
      - FIRMS: ~375 m de resolución, pero solo ~4 pasadas/día (~3h de
        latencia global).
      - GOES-19: ~2 km de resolución (más grueso), pero se actualiza
        cada ~10 minutos — la misma combinación GEO+LEO que usan
        sistemas comerciales como Satellites On Fire para detectar más
        rápido que FIRMS solo.

    Solo se devuelven píxeles de las categorías "processed"/"saturated"
    de mayor confianza (códigos de máscara 10, 11, 30, 31 — ver
    GOES-19_ABI_L2_FireHotSpot_Beta_ReadMe.pdf, NOAA/NESDIS), que son
    las de menor tasa de falsas alarmas.

    Parámetros: lat, lon, radius_km (default 150, tope 300 — el disco
    completo hace que un radio muy grande sea lento e innecesario para
    esta app, pensada para consultas puntuales sobre un campo).
    """
    if not _ee_ready:
        return jsonify({'error': f'Earth Engine no inicializado: {_ee_error}'}), 503
    try:
        lat = float(request.args.get('lat'))
        lon = float(request.args.get('lon'))
        radius_km = min(float(request.args.get('radius_km', 150)), 300)

        region = ee.Geometry.Point([lon, lat]).buffer(radius_km * 1000)

        ahora = ee.Date(datetime.datetime.utcnow().isoformat())
        hace_60min = ahora.advance(-60, 'minute')

        coleccion = (ee.ImageCollection('NOAA/GOES/19/FDCF')
                     .filterDate(hace_60min, ahora)
                     .filterBounds(region)
        ...
        if coleccion.size().getInfo() == 0:
            return jsonify({
                ...
                'focos': [],
                'cantidad': 0,
                'fuente': 'GOES-19 (ABI FDCF)',
                'nota': 'Sin imágenes GOES en la última hora para esta zona '
                        '(demora de procesamiento de NOAA o cobertura parcial).',
            })

        imagen = coleccion.first()
        fecha_imagen = ee.Date(imagen.get('system:time_start')) \
            .format("yyyy-MM-dd'T'HH:mm:ss'Z'").getInfo()

        mascara = imagen.select('Mask')
        # Categorías de menor falsa-alarma: 10/30 = "processed"/"saturated"
        # fire; 11/31 = sus equivalentes de alta confianza.
        es_fuego = (mascara.eq(10).Or(mascara.eq(11))
                    .Or(mascara.eq(30)).Or(mascara.eq(31)))

        datos = imagen.select(['Power', 'Temp', 'Area']).updateMask(es_fuego)

        puntos = datos.sample(region=region, scale=2000, geometries=True).getInfo()

        focos = []
        for f in puntos.get('features', []):
            ...
            focos.append({
                'lat': coords[1],
                'lon': coords[0],
                'frp_mw': props.get('Power'),
                'temp_k': props.get('Temp'),
        ...
        return jsonify({
            ...
            'focos': focos,
            'cantidad': len(focos),
            'fuente': 'GOES-19 (ABI FDCF, disco completo)',
            'fecha_imagen': fecha_imagen,
            'resolucion_aprox_km': 2,
        })
    except ee.EEException as exc:
        return jsonify({'error': f'Error de Earth Engine: {exc}'}), 502
    except Exception as exc:  # noqa: BLE001
    ...
    try:
        ...
        if tipo == 'riesgo':
            ...
            candidatas, zona_conectada, _costo, conectividad_aplicada, info_motor = (
                _simular_inundacion(hand, agua_fuente, region, radius_km, umbral, lat=lat, lon=lon)
            )
...
# ════════════════════════════════════════════════
#  INUNDACIÓN HISTÓRICA REAL (no simulada) — JRC GSW / Sentinel-1
#  Endpoint: /inundacion_historica_real
#
#  Los 3 botones de "escenario histórico" del Simulador (1998, 2013,
#  Abril 2026) hasta ahora mandaban un valor fijo de "subida" (metros)
#  al mismo motor HAND genérico que el resto del simulador — que en
#  terreno muy plano (Iberá, esteros) no distingue bien entre subidas
#  de altura distintas: el DEM (30m) tiene un error vertical del mismo
#  orden que la diferencia entre escenarios, así que terminan pintando
#  prácticamente la misma mancha para 2.83m que para 8.39m.
#
#  Este endpoint NO simula: muestra la extensión de agua REALMENTE
#  observada por satélite para ese evento puntual —
#    - 1998 / 2013 -> JRC Global Surface Water, YearlyHistory (30m,
#      por año calendario). waterClass: 2=estacional, 3=permanente
#      (se incluyen ambas — el agua estacional es la que mejor capta
#      la extensión de una creciente en un año que normalmente no
#      está inundado).
#    - abril_2026 (o cualquier evento reciente puntual) -> Sentinel-1
#      SAR (banda VH). El radar atraviesa nubes, por eso sirve para
#      un evento acotado en el tiempo (a diferencia de JRC, que es
#      agregado por año calendario completo). Agua abierta da
#      backscatter característicamente bajo (superficie lisa =
#      reflexión especular) -> se umbraliza.
#
#  Devuelve tile_url igual que /inundacion_tiles, así el frontend no
#  necesita ningún cambio en cómo dibuja la capa — solo qué endpoint
#  llama para estos 3 botones puntuales en vez del genérico.
# ════════════════════════════════════════════════

# año calendario que mejor representa cada escenario JRC. Botones reales
# del Simulador de Inundación con mancha satelital real disponible: 1998
# (8.39m), 1992 (8.63m), 2016 (7.15m, pico de la crecida El Niño 2015-16
# en Corrientes — fuente: El Litoral, ene-2016). 1983 (9.03m) queda
# deliberadamente afuera de este diccionario: JRC GSW YearlyHistory
# arranca en 1984, así que ese año no se puede mostrar con este método
# (el frontend lo sabe — ESCENARIOS_SIMULADOR_INUNDACION no le pone
# `anioJRC` a ese botón — y sigue usando el simulador HAND genérico para
# él en vez de llamar a este endpoint).
ESCENARIOS_JRC_ANIO = {
    '1998': 1998,
    '1992': 1992,
    '2016': 2016,
    '2013': 2013,  # sin botón dedicado en el frontend hoy, pero disponible si se agrega
}

# Rango de fechas real del evento — cuanto más angosto, más fiel al
# momento exacto del pico. Ajustar si se conoce la fecha exacta del
# pico de cada evento reciente que se vaya agregando acá. Sin botón
# dedicado en el frontend hoy (se probó a mano vía el campo de valor
# exacto) — queda disponible si se agrega uno.
ESCENARIOS_SENTINEL1 = {
    'abril_2026': ('2026-04-01', '2026-04-30'),
}

# ── Caché en memoria de proceso ──
# A diferencia de una consulta con lat/lon dinámicos, acá el resultado
# de Earth Engine NUNCA cambia para una misma (escenario, zona): son
# años calendario y rangos de fecha fijos del pasado. Antes de este
# cambio, estos 2 botones (1998/1992) pasaban por el motor local
# cacheado en disco (local_flood_engine.py) — con este caché en
# memoria se recupera esa misma sensación de "instantáneo" al volver a
# una zona/escenario ya consultado, sin tener que golpear Earth Engine
# de nuevo en cada clic.
# Se redondea lat/lon (grados) y radius_km para que paneos/zooms chicos
# sigan pegándole al mismo hit de caché en vez de fallar por un
# centro/radio levemente distinto en cada request.
_CACHE_HISTORICA_REAL_LOCK = threading.Lock()
_CACHE_HISTORICA_REAL = OrderedDict()  # (escenario, lat_r, lon_r, radius_r) -> dict de respuesta ya armado
_CACHE_HISTORICA_REAL_MAX = 300


def _clave_cache_historica_real(escenario, lat, lon, radius_km):
    return (escenario, round(lat, 1), round(lon, 1), round(radius_km / 10) * 10)
...
def inundacion_historica_real():
    """
    Parámetros:
        escenario   '1998' | '1992' | '2013' | 'abril_2026' (requerido)
        lat, lon    centro del área (default: Corrientes, igual que
                    /inundacion_tiles)
        radius_km   radio del área (default 200, mismo tope que
                    /inundacion_tiles: RADIUS_KM_MAX_INUNDACION)
    """
    if not _ee_ready:
        return jsonify({'error': f'Earth Engine no inicializado: {_ee_error}'}), 503
    try:
        escenario = request.args.get('escenario', '')
        if escenario not in ESCENARIOS_JRC_ANIO and escenario not in ESCENARIOS_SENTINEL1:
            return jsonify({
                'error': (
                    f"escenario '{escenario}' no reconocido. Válidos: "
                    f"{', '.join(list(ESCENARIOS_JRC_ANIO) + list(ESCENARIOS_SENTINEL1))}"
                ),
            }), 400
        ...
        clave = _clave_cache_historica_real(escenario, lat, lon, radius_km)
        with _CACHE_HISTORICA_REAL_LOCK:
            if clave in _CACHE_HISTORICA_REAL:
                _CACHE_HISTORICA_REAL.move_to_end(clave)  # LRU: refresca su posición
                return jsonify(_CACHE_HISTORICA_REAL[clave])

        region = ee.Geometry.Point([lon, lat]).buffer(radius_km * 1000)

        if escenario in ESCENARIOS_JRC_ANIO:
            anio = ESCENARIOS_JRC_ANIO[escenario]
            img_anio = ee.Image(f'JRC/GSW1_4/YearlyHistory/{anio}').select('waterClass')
            agua = img_anio.gte(2).And(img_anio.lte(3)).selfMask().clip(region)
            fuente = f'JRC Global Surface Water — histórico {anio} (observado, no simulado)'
        else:
            fecha_inicio, fecha_fin = ESCENARIOS_SENTINEL1[escenario]
            ...
            if coleccion.size().getInfo() == 0:
                return jsonify({
                    'error': (
                        f'Sin escenas Sentinel-1 disponibles para {escenario} '
                        f'({fecha_inicio} a {fecha_fin}) en esta zona.'
                    ),
                }), 404
            mediana = coleccion.median()
            agua = mediana.lt(-17).selfMask().clip(region)  # umbral típico agua abierta, ajustar si hay ruido
            fuente = f'Sentinel-1 SAR (radar, {fecha_inicio} a {fecha_fin}) — observado, no simulado'

        vis = {'palette': ['#4a90d9']}
        map_id = agua.getMapId(vis)

        respuesta = {
            'tile_url': map_id['tile_fetcher'].url_format,
            'tipo': 'inundacion_historica_real',
            'escenario': escenario,
            'fuente': fuente,
            'vis': vis,
            'motor': 'satelital_observado',  # NO 'simulado' — distinción a propósito para la UI
        }

        with _CACHE_HISTORICA_REAL_LOCK:
            _CACHE_HISTORICA_REAL[clave] = respuesta
            _CACHE_HISTORICA_REAL.move_to_end(clave)
            while len(_CACHE_HISTORICA_REAL) > _CACHE_HISTORICA_REAL_MAX:
                _CACHE_HISTORICA_REAL.popitem(last=False)  # descarta el menos usado recientemente
...
def inundacion_animacion():
    ...
    try:
        ...
        candidatas, zona_conectada, costo_acumulado, conectividad_ok, info_motor = (
            _simular_inundacion(hand, agua_fuente, region, radius_km, umbral_m, lat=lat, lon=lon)
        )
        ...
        # RENDIMIENTO — los N fotogramas se generaban antes en un for secuencial:
        # cada .getMapId() es un viaje de red independiente a Earth Engine (no
        # calcula píxeles ahí, solo registra el cómputo), y con 10-15 de esos
        # uno atrás del otro, la espera se sumaba entera (de ahí el "puede
        # tardar ~20-30s" del frontend). Como son independientes entre sí, se
        # paralelizan con un thread pool — misma lógica que subir --threads en
        # gunicorn: es espera de red, no cómputo de CPU, así que varios hilos
        # a la vez ayudan mucho. executor.map conserva el orden de salida
        # (fotogramas queda en el mismo orden 1..frames que antes).
        def _generar_fotograma(i):
            if i == frames:
                # Último fotograma: extensión REAL completa, no el recorte
                # del percentil 95 usado arriba solo para repartir los
                # fotogramas intermedios. Sin esto, el 5% más alejado del
                # agua nunca se pintaba — la animación terminaba "a medio
                # inundar" incluso en el 100%.
                frame_img = candidatas
            else:
            ...
            return {
                'orden'        : i,
                'distancia_m'  : round(max_costo_m, 1) if i == frames else round(max_costo_m * (i / frames), 1),
...
def _calcular_conectividad_hidraulica_punto(lat, lon, radius_km, devolver_rasters=False):
    ...
    devolver_rasters=True (opcional, default False para no romper a ningún
    consumidor existente que haga jsonify() directo de este dict) agrega una
    clave '_rasters' con las imágenes de Earth Engine YA CALCULADAS en esta
    misma corrida (zona_conectada, costo_acumulado, region) para que otro
    endpoint (/iipdi_red_hidraulica) las reutilice en vez de volver a pedirle
    todo a Earth Engine desde cero. El llamador debe hacer .pop('_rasters')
    antes de devolver este dict como JSON.
    """
    region, elevacion, agua_fuente = _preparar_dem_y_agua(lat, lon, radius_km)
...
# ──────────────────────────────────────────────────────────────────────────
# Endpoint: /iipdi_red_hidraulica
# ------------------------------------------------------------------------
# Complementa a /iipdi_punto para la nueva presentación visual del
# Inspector IIPDI: en vez de mostrar solo el % de conectividad hidráulica,
# el frontend quiere resaltar en el mapa QUÉ cuerpos de agua concretos
# (lagunas, esteros, cañadas, arroyos, ríos) son los que justifican ese
# número.
#
# IMPORTANTE — este endpoint NO cambia el algoritmo del IIPDI ni de
# conectividad hidráulica: llama a _calcular_conectividad_hidraulica_punto()
# tal cual existe (con devolver_rasters=True) y reutiliza exactamente las
# mismas imágenes de Earth Engine que esa función ya arma para el índice
# (zona_conectada, costo_acumulado) — un único cómputo de EE, no dos. Lo
# único nuevo acá es: tomar los cuerpos de agua vectoriales candidatos
# (hidrografia_vectorial.buscar_cuerpos_en_radio) y filtrarlos contra esa
# máscara para quedarnos solo con los que la red de drenaje real conecta
# con el punto — no simplemente "los que están cerca".
# ──────────────────────────────────────────────────────────────────────────

def _nivel_conexion_de_score(score):
    if score >= 0.75:
        return 'Muy alta'
    if score >= 0.4:
        return 'Media'
...
@app.route('/iipdi_red_hidraulica')
def iipdi_red_hidraulica():
    """
    Cuerpos de agua vectoriales realmente conectados (por la red de
    drenaje) al punto consultado por el Inspector IIPDI.

    Reglas de cuántos/cuáles se devuelven en `cuerpos_conectados`, según el
    índice de conectividad hidráulica del punto (mismo valor que ya
    devuelve /iipdi_punto en modulos.conectividad_hidraulica.indice_conectividad):
        0%        -> lista vacía ("Sin conectividad hidráulica superficial")
        1%-30%    -> solo el más cercano de los conectados
        30%-60%   -> los de conexión Media o superior
        >60%      -> todos los conectados encontrados (tope: 20 candidatos evaluados)
    ...
    if not _hidro_vectorial_disponible:
        return jsonify({
            'error': ('Módulo hidrografia_vectorial no disponible en el servidor. '
                      'Subí hidrografia_vectorial.py junto a app.py y volvé a desplegar.'),
        }), 503
    ...
    try:
        ...
        conectividad = _calcular_conectividad_hidraulica_punto(lat, lon, radius_km, devolver_rasters=True)
        rasters = conectividad.pop('_rasters', None)
        indice = conectividad['indice_conectividad']

        respuesta_base = {
            'indice_conectividad': indice,
            'cuerpos_conectados': [],
            'cuerpo_mas_cercano': None,
            'area_influencia': None,
        }

        # Regla 0%: nada que resaltar (puede ser porque no hay agua_fuente
        # visible en el radio, o porque el punto no está conectado a ella).
        if indice <= 0 or not rasters:
            respuesta_base['nivel'] = 'Sin conexión superficial'
            return jsonify(respuesta_base)

        zona_conectada = rasters['zona_conectada']
        costo_acumulado = rasters['costo_acumulado']

        candidatos = hidro_vectorial.buscar_cuerpos_en_radio(lat, lon, radio_km=radius_km, max_resultados=20)

        conectados = []
        if candidatos:
            # Un único reduceRegions para TODOS los candidatos (en vez de N
            # llamadas): barato y evita saturar la cuota de Earth Engine.
            puntos_fc = ee.FeatureCollection([
                ee.Feature(ee.Geometry.Point([c['lon'], c['lat']]), {'idx': i})
                for i, c in enumerate(candidatos)
            ])
            muestras = ee.Image.cat([
                zona_conectada.rename('conectado'),
                costo_acumulado.rename('costo'),
            ]).reduceRegions(
                collection=puntos_fc, reducer=ee.Reducer.first(), scale=90, tileScale=4,
            ).getInfo()
            ...
            for feat in muestras.get('features', []):
                ...
                idx = props.get('idx')
                # Si 'conectado' es None, el pixel está fuera de zona_conectada
                # (enmascarado) -> ese cuerpo NO pertenece a la red, aunque
                # esté geográficamente cerca. Se descarta acá.
                if props.get('conectado') is not None and idx is not None:
                    costos_por_idx[idx] = props.get('costo')

            for i, c in enumerate(candidatos):
                if i not in costos_por_idx:
                    continue
                costo_m = costos_por_idx[i]
                score = _score_exponencial_decreciente(costo_m, CONECTIVIDAD_ESCALA_DIST_M) if costo_m is not None else 0.0
                conectados.append({
                    **c,
                    'distancia_hidraulica_m': round(costo_m, 1) if costo_m is not None else None,
                    'nivel_conexion': _nivel_conexion_de_score(score),
                    'score_conexion': round(score, 3),
                })

        conectados.sort(key=lambda c: c['distancia_m'])

        # Reglas de cuántos mostrar, según el índice global del punto.
        if not conectados:
            conectados_a_mostrar = []
            nivel_txt = 'Sin conexión superficial'
        elif indice <= 30:
            conectados_a_mostrar = conectados[:1]
            nivel_txt = 'Baja'
        elif indice <= 60:
            conectados_a_mostrar = [c for c in conectados if c['nivel_conexion'] in ('Muy alta', 'Media')] or conectados[:1]
            nivel_txt = 'Media'
        else:
            conectados_a_mostrar = conectados
            nivel_txt = 'Alta'

        # Área de influencia: vectorizar zona_conectada. Accesorio: si falla
        # o es muy pesada, no debe tumbar el resto de la respuesta.
        area_influencia = None
        try:
            vectores = zona_conectada.reduceToVectors(
                geometry=rasters['region'], scale=90, geometryType='polygon',
                eightConnected=True, maxPixels=1e8, bestEffort=True, tileScale=4,
            )
            geom_disuelta = vectores.geometry(maxError=30)
            geom_geojson = geom_disuelta.simplify(60).getInfo()
            area_km2 = geom_disuelta.area(maxError=30).divide(1e6).getInfo()
            area_influencia = {'geojson': geom_geojson, 'area_km2': round(area_km2, 2)}
        except ee.EEException:
            area_influencia = None
        ...
        return jsonify({
            'indice_conectividad': indice,
            'nivel': nivel_txt,
            'cuerpos_conectados': conectados_a_mostrar,
            'cuerpo_mas_cercano': conectados_a_mostrar[0] if conectados_a_mostrar else None,
...
# ──────────────────────────────────────────────────────────────────────────
# Endpoint: /expandir_cuerpo_agua
# ------------------------------------------------------------------------
# "Simular Desborde" del Inspector de Riesgo Hídrico: a diferencia de
# /inundacion_tiles (raster genérico de "zona baja" por HAND, sin relación
# con la forma real de ningún cuerpo puntual), esto expande la GEOMETRÍA
# REAL de UN cuerpo de agua específico (la que ya devuelve
# /iipdi_red_hidraulica en cuerpos_conectados[].geometria) en la dirección
# en la que el agua realmente escurriría si se desborda.
#
# Reutiliza el mismo FloodSimulationEngine que /inundacion_tiles y
# /inundacion_animacion (pendiente, sesgo de dirección de flujo,
# acumulación de flujo, resistencia, perfil regional) — la única diferencia
# es la fuente de agua: en vez de "todo pixel JRC>50% de la región" (agua
# genérica), se rasteriza SOLO el polígono del cuerpo elegido, para que la
# expansión salga literalmente de su forma real y no de cualquier agua
# cercana. El resultado se vectoriza (no es un tile raster) para poder
# dibujarse como un polígono más sobre el mapa, igual que el resto de la
# capa de cuerpos de agua.
#
# `margen_m` es la sobre-elevación a simular (metros por encima del HAND
# del propio cuerpo) — el frontend ya la calcula combinando IPCA del
# cuerpo con el HAND del punto analizado (ver calcularSimulacionDesborde en
# GeoSentinel_v2.html). Si no llega, se usa un valor moderado por defecto.
#
# Si el resultado alcanza/roza otro cuerpo, el frontend decide si mostrarlo
# como "conectado" comparando contra dataRed.cuerpos_conectados (ya
# disponible del lado del cliente) — este endpoint no decide eso, solo
# devuelve la geometría expandida.
# ──────────────────────────────────────────────────────────────────────────
EXPANSION_MARGEN_DEFECTO_M = 1.5
EXPANSION_MARGEN_MIN_M = 0.2
EXPANSION_MARGEN_MAX_M = 6.0


@app.route('/expandir_cuerpo_agua')
def expandir_cuerpo_agua():
    if not _ee_ready:
        return jsonify({'error': f'Earth Engine no inicializado: {_ee_error}'}), 503
    ...
    try:
        ...
        margen_raw = request.args.get('margen_m', EXPANSION_MARGEN_DEFECTO_M)
        try:
            margen_m = float(margen_raw)
        except (TypeError, ValueError):
            margen_m = EXPANSION_MARGEN_DEFECTO_M
        margen_m = max(EXPANSION_MARGEN_MIN_M, min(margen_m, EXPANSION_MARGEN_MAX_M))

        geometria_raw = request.args.get('geometria')
        if not geometria_raw:
            return jsonify({'error': 'Falta el parámetro "geometria" (GeoJSON del cuerpo de agua a expandir).'}), 400
        try:
            geom_dict = json.loads(geometria_raw)
        except (TypeError, ValueError, json.JSONDecodeError):
            return jsonify({'error': '"geometria" no es un GeoJSON válido.'}), 400

    except (TypeError, ValueError):
        return jsonify({'error': 'Parámetros lat/lon requeridos y deben ser numéricos.'}), 400

    try:
        region = ee.Geometry.Point([lon, lat]).buffer(radius_km * 1000)
        cuerpo_geom = ee.Geometry(geom_dict)

        # Fuente de agua = SOLO este cuerpo (no todo el JRC de la región),
        # así el motor expande a partir de su forma real.
        agua_fuente_especifica = (
            ee.Image(0).byte()
              .paint(ee.FeatureCollection([ee.Feature(cuerpo_geom)]), 1)
              .clip(region)
        )

        _, hand, _ = _preparar_hand_y_agua(lat, lon, radius_km, region=region)

        candidatas, zona_conectada, costo_acumulado, conectividad_ok, info_motor = _simular_inundacion(
            hand, agua_fuente_especifica, region, radius_km, margen_m,
        )

        # Unión con el polígono original: si algún borde de la máscara
        # rasterizada (90 m) no calza perfecto con el polígono vectorial
        # real, esto evita que la forma "original" quede mordida en el
        # resultado final.
        zona_final = zona_conectada.unmask(0).Or(agua_fuente_especifica).selfMask()

        vectores = zona_final.reduceToVectors(
            geometry=region, scale=90, geometryType='polygon',
            eightConnected=True, maxPixels=1e8, bestEffort=True, tileScale=4,
            reducer=ee.Reducer.countEvery(),
        )
        geom_disuelta = vectores.geometry(maxError=30)
        expansion_geojson = geom_disuelta.simplify(45).getInfo()
        area_km2 = geom_disuelta.area(maxError=30).divide(1e6).getInfo()

        return jsonify({
            'ok': True,
            'conectividad_ok': conectividad_ok,
            'margen_aplicado_m': round(margen_m, 2),
            'expansion': expansion_geojson,
            'area_km2': round(area_km2, 2),
            'motor': info_motor,
        })

    except ee.EEException as exc:
        return jsonify({'error': f'Error de Earth Engine: {exc}'}), 500
    except Exception as exc:  # noqa: BLE001
        return jsonify({'error': str(exc)}), 500
...
# ------------------------------------------------------------------------
# Para el Inspector de Suelos INTA y el Motor de Aptitud Productiva: en vez
# de consultar siempre la capa nacional 1:500.000, prueba primero las capas
# regionales de mejor resolución (Corrientes 1:50.000 por departamento,
# Córdoba y Entre Ríos 1:100.000) y solo cae al 1:500.000 si el punto queda
# fuera de toda cobertura regional conocida. Ver suelos_inta_regional.py
# para el detalle de qué capas están confirmadas y de dónde salió cada bbox.
# ──────────────────────────────────────────────────────────────────────────
@app.route('/suelo_mejor_resolucion')
def suelo_mejor_resolucion():
    if not _suelos_regional_disponible:
        return jsonify({
            'error': ('Módulo suelos_inta_regional no disponible en el servidor. '
                      'Subí suelos_inta_regional.py junto a app.py y volvé a desplegar.'),
        }), 503
    try:
        lat = float(request.args.get('lat'))
        lon = float(request.args.get('lon'))
    ...
    resultado = suelos_inta_regional.consultar_mejor_capa(lat, lon)
    return jsonify(resultado)


# ──────────────────────────────────────────────────────────────────────────
# Endpoint: /suelo_capa_aptitud
# ------------------------------------------------------------------------
# Para el overlay visual coloreado de Aptitud Productiva (polígonos en el
# área visible del mapa, no un punto). Usa el mismo catálogo regional que
# /suelo_mejor_resolucion, pero trae varias features dentro de un bbox y
# las devuelve con propiedades ya normalizadas (mismos nombres de campo
# sin importar de qué provincia/esquema haya salido cada polígono), para
# que el frontend pueda clasificar el color de forma uniforme.
# Ver suelos_inta_regional.capa_visual_bbox() para el detalle.
# ──────────────────────────────────────────────────────────────────────────
@app.route('/suelo_capa_aptitud')
def suelo_capa_aptitud():
    if not _suelos_regional_disponible:
        return jsonify({
            'error': ('Módulo suelos_inta_regional no disponible en el servidor. '
                      'Subí suelos_inta_regional.py junto a app.py y volvé a desplegar.'),
    ...
    try:
        ...
        return jsonify({'error': 'Parámetros west/south/east/north requeridos y deben ser numéricos.'}), 400

    resultado = suelos_inta_regional.capa_visual_bbox((west, south, east, north))
    return jsonify(resultado)


# ──────────────────────────────────────────────────────────────────────────
# Endpoint: /humedal_punto
# ------------------------------------------------------------------------
# Probabilidad de humedal INTA (rásters por subregión, autoalojados en
# Hugging Face) para un punto. Complementa a hidrografia_vectorial: sirve
# para detectar esteros/bañados/cuerpos de agua chicos que la capa
# vectorial de cuerpos de agua todavía no tiene mapeados como polígono.
# Ver humedales_inta.py para el detalle de umbrales y diseño.
# ──────────────────────────────────────────────────────────────────────────
@app.route('/humedal_punto')
def humedal_punto():
    if not _humedales_disponible:
        return jsonify({
            'error': ('Módulo humedales_inta no disponible en el servidor. '
                      'Subí humedales_inta.py junto a app.py, agregá "rasterio" '
                      'a requirements.txt, y volvé a desplegar.'),
        }), 503
    try:
        lat = float(request.args.get('lat'))
        lon = float(request.args.get('lon'))
    ...
    resultado = humedales_inta.consultar_probabilidad_humedal(lat, lon)
    return jsonify(resultado)


# ──────────────────────────────────────────────────────────────────────────
# Endpoint: /terreno_punto
# ------------------------------------------------------------------------
# Fase 1 del Hydrologic Decision Engine (sin GEE): HAND, pendiente y TWI
# desde MERIT Hydro autoalojado. Ver terreno_merit_hydro.py.
# ──────────────────────────────────────────────────────────────────────────
@app.route('/terreno_punto')
def terreno_punto():
    if not _terreno_disponible:
        return jsonify({
            'error': ('Módulo terreno_merit_hydro no disponible en el servidor. '
                      'Subí terreno_merit_hydro.py junto a app.py, agregá "rasterio" '
                      'y "numpy" a requirements.txt, y volvé a desplegar.'),
        }), 503
    try:
        lat = float(request.args.get('lat'))
        lon = float(request.args.get('lon'))
    ...
    resultado = terreno_merit_hydro.consultar_terreno(lat, lon)
    return jsonify(resultado)


# ──────────────────────────────────────────────────────────────────────────
# Endpoint: /inundacion_local
# ------------------------------------------------------------------------
# Reemplaza a /inundacion_tiles (tipo=zona_baja/anegamiento) para el
# Simulador de Inundación del frontend — CERO Earth Engine en el camino
# caliente. HAND se calcula una sola vez por zona (Copernicus GLO-30 +
# pysheds, ver local_flood_engine.py) y queda cacheado en disco; consultas
# siguientes a la misma zona son lectura de disco, milisegundos.
#
# A diferencia de /inundacion_tiles (que devuelve una tile_url — pirámide
# de zoom que el navegador va pidiendo pedazo a pedazo, cada uno calculado
# on-demand por Earth Engine), esto devuelve UNA sola imagen PNG en base64
# ya recortada al área de análisis + sus bounds geográficos, pensada para
# L.imageOverlay en Leaflet (no un tile layer). El área de análisis es
# fija (~60 km alrededor del punto, ver RADIO_ANALISIS_KM en
# local_flood_engine.py) — el radius_km del simulador (pensado para
# /inundacion_tiles, hasta 800 km) no aplica acá a propósito: 60 km ya
# excede el tope de propagación hidráulica real que tenía el motor GEE
# (MAX_DIST_CONECTIVIDAD_M = 50 km).
#
# Parámetros:
#   lat, lon            centro del área (requeridos)
#   umbral_m             metros de subida sobre el cauce normal (HAND) —
#                         mismo significado que en /inundacion_tiles.
#   mm_lluvia             alternativa a umbral_m: si no se manda umbral_m,
#                         se convierte desde mm de lluvia (Curve Number,
#                         ver local_flood_engine.mm_a_umbral_hand).
#   forzar_recalculo      '1'/'true' para ignorar el caché en disco y
#                         recalcular el HAND de esa zona desde cero.
# ──────────────────────────────────────────────────────────────────────────
_TIPOS_INUNDACION_LOCAL = (
    'zona_baja', 'anegamiento',           # basados en HAND + umbral
    'elevacion', 'pendiente',             # directo del DEM
    'zona_media', 'zona_alta',            # bandas de elevación
    'depresiones',                        # Fill Sinks (pysheds)
    'riesgo', 'critico',                  # elevación+pendiente (sin SMAP, ver nota)
)


@app.route('/inundacion_local')
def inundacion_local():
    """
    Reemplazo local (sin GEE) de /inundacion_tiles para TODOS sus tipos.
    A diferencia de /inundacion_tiles (pirámide de tiles calculada
    on-demand por Earth Engine), acá se devuelve una sola imagen PNG en
    base64 ya recortada al área de análisis (~60 km, ver RADIO_ANALISIS_KM
    en local_flood_engine.py) + sus bounds, pensada para L.imageOverlay.

    Parámetros:
        tipo         zona_baja (default) | anegamiento | elevacion |
                     pendiente | zona_media | zona_alta | depresiones |
                     riesgo | critico
        lat, lon     centro del área (requeridos)
        umbral_m     SOLO zona_baja/anegamiento: metros de subida sobre
                     el cauce normal (HAND).
        mm_lluvia    alternativa a umbral_m (SOLO zona_baja/anegamiento):
                     se convierte con local_flood_engine.mm_a_umbral_hand.
        forzar_recalculo   '1'/'true' para ignorar el caché en disco.

    Nota sobre 'riesgo'/'critico': el script GEE original pondera
    elevación 40% + pendiente 30% + humedad de suelo SMAP 30%. Acá no hay
    SMAP (es un dato de la NASA que llegaba vía GEE) — el peso se
    redistribuyó entre elevación y pendiente. Ver la nota larga junto a
    calcular_riesgo_compuesto() en local_flood_engine.py.
    """
    if not _flood_local_disponible_check():
        return jsonify({
            'error': ('Módulo local_flood_engine no disponible en el servidor '
                      '(falta "pysheds"/"rasterio"/"Pillow" en requirements.txt, '
                      'o local_flood_engine.py no está desplegado junto a app.py).'),
            'detalle_import': _flood_local_error_detalle(),
        }), 503
    ...
    tipo = request.args.get('tipo', 'zona_baja')
    if tipo not in _TIPOS_INUNDACION_LOCAL:
        return jsonify({
            'error': f"tipo '{tipo}' no reconocido. Válidos: {', '.join(_TIPOS_INUNDACION_LOCAL)}",
        }), 400

    umbral_m = None
    if tipo in ('zona_baja', 'anegamiento'):
        umbral_m = request.args.get('umbral_m', type=float)
        if umbral_m is None or umbral_m <= 0:
            mm_lluvia = request.args.get('mm_lluvia', type=float) or 0.0
            umbral_m = local_flood_engine.mm_a_umbral_hand(mm_lluvia)
            if umbral_m <= 0:
                return jsonify({
                    'error': ('Con esos mm de lluvia no hay escorrentía efectiva (por debajo '
                              'de la abstracción inicial del suelo) — no se generaría ninguna '
                              'mancha. Mandá "umbral_m" directo si querés forzar una subida.'),
                    'umbral_m': 0.0,
                }), 200

    forzar = request.args.get('forzar_recalculo', '').strip().lower() in ('1', 'true', 'si', 'sí')

    try:
        datos = local_flood_engine.obtener_hand_local(lat, lon, forzar_recalculo=forzar)
    except RuntimeError as exc:
        return jsonify({'error': str(exc)}), 502
    except Exception as exc:  # noqa: BLE001
        return jsonify({'error': f'Error inesperado calculando el terreno local: {exc}'}), 500

    try:
        if tipo in ('zona_baja', 'anegamiento'):
            imagen_b64 = local_flood_engine.generar_imagen_png_base64(datos['hand'], umbral_m, datos['nodata'])
            vis = {'palette': ['#bcd2ee']}
            extra = {}
        else:
            imagen_b64, vis, extra = local_flood_engine.renderizar_capa(tipo, datos)
        bounds = local_flood_engine.bounds_geograficos(datos['transform'], datos['hand'].shape)
    except Exception as exc:  # noqa: BLE001
        return jsonify({'error': f'Error generando la imagen: {exc}'}), 500

    # FIX: el frontend mostraba "Área inundada / % del área / Volumen est."
    # calculados con una fórmula inventada (sin relación con lo que
    # realmente dibuja esta capa), lo que daba números sin sentido (ej.
    # "46,3 ha" junto con "100% del área" en un círculo de 282.000 ha). Acá
    # SÍ tenemos el raster HAND real en memoria (`datos['hand']`), así que
    # se cuenta de verdad cuántos píxeles quedan inundados (HAND <= umbral)
    # y se convierte a hectáreas usando el tamaño real de cada píxel
    # (derivado de bounds/shape — Copernicus GLO-30 es ~30 m, pero el
    # ancho en metros de un grado de longitud cambia con la latitud, así
    # que se calcula por punto en vez de asumir 30 m fijo).
    extra_area = {}
    if tipo in ('zona_baja', 'anegamiento') and umbral_m is not None:
        try:
            import numpy as np
            hand_arr = datos['hand']
            nodata = datos['nodata']
            if nodata is None:
                valido = np.ones_like(hand_arr, dtype=bool)
            elif isinstance(nodata, float) and math.isnan(nodata):
                valido = ~np.isnan(hand_arr)
            else:
                valido = hand_arr != nodata
            inundado = valido & (hand_arr <= umbral_m)

            (sur, oeste), (norte, este) = bounds
            filas, columnas = hand_arr.shape
            lat_centro_rad = math.radians((sur + norte) / 2.0)
            m_por_grado_lat = 111320.0
            m_por_grado_lon = 111320.0 * math.cos(lat_centro_rad)
            ancho_px_m = abs(((este - oeste) / columnas) * m_por_grado_lon)
            alto_px_m = abs(((norte - sur) / filas) * m_por_grado_lat)
            area_px_m2 = ancho_px_m * alto_px_m

            pixeles_inundados = int(inundado.sum())
            pixeles_validos = int(valido.sum())

            # Volumen: suma de la "lámina de agua" (umbral - hand) en cada
            # píxel inundado × área del píxel — es una aproximación (asume
            # terreno plano dentro de cada píxel de 30 m), no una medición
            # real de caudal ni de volumen embalsado.
            lamina = np.where(inundado, umbral_m - hand_arr, 0.0)
            volumen_m3 = float(lamina.sum()) * area_px_m2

            extra_area = {
                'area_inundada_ha': round(pixeles_inundados * area_px_m2 / 10000.0, 1),
                'pct_area_inundada': round((pixeles_inundados / pixeles_validos) * 100, 1) if pixeles_validos else 0.0,
                'volumen_estimado_m3': round(volumen_m3, 0),
            }
        except Exception as exc:  # noqa: BLE001
            # Si algo falla acá, no se rompe la capa (que ya se generó bien
            # arriba) — simplemente no viajan los 3 números extra y el
            # frontend los muestra como "no disponible".
            print(f'[inundacion_local] No se pudo calcular área real: {exc}')
    ...
    respuesta = {
        ...
        'bounds': bounds,
        'vis': vis,
        'de_cache': datos.get('de_cache', False),
        'tiempo_seg': datos.get('tiempo_seg'),
        'motor': 'local_hand_pysheds_copernicus30',
        **extra,
        **extra_area,
    }
    if umbral_m is not None:
        respuesta['umbral_m'] = round(umbral_m, 3)
    return jsonify(respuesta)


# ──────────────────────────────────────────────────────────────────────────
# Endpoint: /inundacion_dinamica
# ------------------------------------------------------------------------
# Simulación DINÁMICA (LISFLOOD-FP, binario C++ compilado) — el agua se
# propaga en el tiempo según una tormenta de diseño, en vez del umbral
# HAND estático de /inundacion_local. Radio chico a propósito (ver
# RADIO_ANALISIS_KM en lisflood_engine.py) para que corra en segundos.
# Corre como subproceso — no landlab (esa versión tiraba el servidor por
# memoria: ~150-180MB solo de importar el paquete). Sin Earth Engine.
# ──────────────────────────────────────────────────────────────────────────
def _lisflood_disponible_check():
    """Igual razón que _flood_local_disponible_check(): 'disponible' acá
    depende de si el binario `lisflood` está compilado y accesible, no
    solo de que el archivo lisflood_engine.py exista."""
    try:
        mod = lisflood_engine._cargar()
        return bool(mod._MOTOR_DINAMICO_DISPONIBLE)
    except Exception:
        return False


def _cuenca_disponible_check():
    """Mismo patrón que _lisflood_disponible_check() — la disponibilidad
    real depende de si pysheds/shapely se pudieron importar DENTRO del
    módulo, no solo de que el archivo exista."""
    try:
        mod = cuenca_engine._cargar()
        return bool(mod._MOTOR_CUENCA_DISPONIBLE)
    except Exception:
        return False


# ──────────────────────────────────────────────────────────────────────────
# Endpoint: /cuenca_hidrologica
# ------------------------------------------------------------------------
# Delimita la cuenca hidrológica COMPLETA aguas arriba de un punto que el
# usuario marca en el mapa (ej. donde un arroyo cruza su lote). Mismo
# motor pysheds que local_flood_engine.py (HAND), sin GEE. A diferencia
# del HAND/simulador de inundación, acá el área de salida puede ser
# mucho más grande que el radio de análisis pedido — es la cuenca
# entera, no un recorte fijo.
# ──────────────────────────────────────────────────────────────────────────
@app.route('/cuenca_hidrologica')
def cuenca_hidrologica():
    """
    Parámetros:
        lat, lon    punto de salida (outlet) marcado por el usuario —
                    requeridos. No hace falta que caiga exacto sobre el
                    cauce: se engancha automáticamente al cauce más
                    cercano dentro del área descargada.
        radio_km    opcional, radio del área descargada para buscar la
                    cuenca (default 40 km — ver cuenca_engine.py). Si la
                    respuesta viene con truncada=true, la cuenca real es
                    más grande que este radio; probar de nuevo con un
                    radio mayor.
    """
    if not _cuenca_disponible_check():
        return jsonify({
            'error': ('Módulo cuenca_engine no disponible en el servidor '
                      '(falta "pysheds"/"shapely" en requirements.txt, o el '
                      'archivo no está desplegado junto a app.py).'),
        }), 503
    ...
    radio_km = request.args.get('radio_km', type=float)

    try:
        resultado = cuenca_engine.delimitar_cuenca(lat, lon, radio_km=radio_km)
    except ValueError as exc:
        return jsonify({'error': str(exc)}), 422
    except RuntimeError as exc:
        return jsonify({'error': str(exc)}), 502
    except Exception as exc:  # noqa: BLE001
        return jsonify({'error': f'Error inesperado delimitando la cuenca: {exc}'}), 500

    respuesta = dict(resultado)
    respuesta['ok'] = True
    respuesta['motor'] = 'local_cuenca_pysheds_copernicus30'
    if resultado['truncada']:
        respuesta['advertencia'] = (
            f'La cuenca delimitada toca el borde del área analizada '
            f'({resultado["radio_km"]} km de radio) — la cuenca real probablemente '
            f'sea más grande. Repetí la consulta con un radio_km mayor si necesitás '
            f'el área completa.'
        )
    return jsonify(respuesta)


@app.route('/inundacion_dinamica')
def inundacion_dinamica():
    """
    Parámetros:
        lat, lon        centro del área (requeridos)
        escenario       'enso_actual' (default) — usa el ONI actual (NOAA,
                         vía enso_oni) para elegir la tormenta de diseño —
                         o 'manual', junto con mm_totales/horas_lluvia.
        mm_totales       SOLO escenario=manual: mm totales de la tormenta.
        horas_lluvia     SOLO escenario=manual: duración de la lluvia (h).
        horas_total      opcional, duración total simulada (lluvia +
                         escurrimiento). Default 6h.
        radio_km         opcional, default 2 km (ver lisflood_engine.py:
                         medido en pruebas, 3km/30m tardaba ~99s; a más
                         radio, riesgo real de pasar el timeout del request).
    """
    if not _lisflood_disponible_check():
        return jsonify({
            'error': ('Módulo lisflood_engine no disponible en el servidor '
                      '(binario "lisflood" no compilado — ver lisflood_src/ '
                      'y el Build Command de Render).'),
        }), 503
    ...
    escenario = request.args.get('escenario', 'enso_actual')
    horas_total = request.args.get('horas_total', type=float) or 6.0
    radio_km = request.args.get('radio_km', type=float)

    tormenta_info = None
    if escenario == 'manual':
        mm_totales = request.args.get('mm_totales', type=float)
        horas_lluvia = request.args.get('horas_lluvia', type=float)
        if not mm_totales or not horas_lluvia:
            return jsonify({'error': 'escenario=manual requiere mm_totales y horas_lluvia.'}), 400
    elif escenario == 'fase_manual':
        # Igual que enso_actual, pero con una fase/intensidad ELEGIDA a mano
        # (no consulta el ONI en vivo de NOAA) — para escenarios de
        # referencia fijos como "El Niño fuerte", sin depender de qué esté
        # pasando hoy en el Pacífico.
        if not _enso_disponible:
            return jsonify({
                'error': ('Módulo enso_oni no disponible en el servidor — no se puede usar '
                          'escenario=fase_manual. Desplegá enso_oni.py.'),
            }), 503
        fase = request.args.get('fase', 'El Niño')
        intensidad = request.args.get('intensidad') or None
        tormenta_info = enso_oni.tormenta_diseno_desde_oni(fase, intensidad)
        mm_totales = tormenta_info['mm_totales']
        horas_lluvia = tormenta_info['horas_lluvia']
        horas_total = tormenta_info['horas_total']
        tormenta_info['fase'] = fase
        tormenta_info['intensidad'] = intensidad
    elif escenario == 'enso_actual':
        if not _enso_disponible:
            return jsonify({
                'error': ('Módulo enso_oni no disponible en el servidor — no se puede usar '
                          'escenario=enso_actual. Usá escenario=manual con mm_totales/horas_lluvia, '
                          'o desplegá enso_oni.py.'),
            }), 503
        oni = enso_oni.consultar_oni()
        if 'oni' not in oni:
            return jsonify({'error': f"No se pudo consultar el ONI actual: {oni.get('error')}"}), 502
        tormenta_info = enso_oni.tormenta_diseno_desde_oni(oni['fase'], oni.get('intensidad'))
        mm_totales = tormenta_info['mm_totales']
        horas_lluvia = tormenta_info['horas_lluvia']
        horas_total = tormenta_info['horas_total']
        tormenta_info['oni'] = oni
    else:
        return jsonify({'error': "escenario debe ser 'enso_actual', 'fase_manual' o 'manual'."}), 400

    try:
        resultado = lisflood_engine.simular_crecida_dinamica(
            lat, lon, mm_totales, horas_lluvia, horas_total, radio_km=radio_km,
        )
    except RuntimeError as exc:
        return jsonify({'error': str(exc)}), 502
    except Exception as exc:  # noqa: BLE001
        return jsonify({'error': f'Error inesperado en la simulación dinámica: {exc}'}), 500

    imagen_b64 = lisflood_engine.generar_imagen_profundidad_base64(
        resultado['profundidad_m'], resultado['es_inundada'],
    )
    ...
    respuesta = {
        ...
        'vis': {'min': 0.0, 'max': 1.5, 'palette': lisflood_engine.PALETA_PROFUNDIDAD},
        'area_inundada_ha': resultado['area_inundada_ha'],
        'pasos': resultado['pasos'],
        'tiempo_seg_simulado': resultado['tiempo_seg_simulado'],
        'tiempo_seg_real': resultado['tiempo_seg_real'],
        'grilla': resultado['grilla'],
        'radio_km': resultado['radio_km'],
        'resolucion_m': resultado['resolucion_m'],
        'truncado_por_max_pasos': resultado['truncado_por_max_pasos'],
        'mm_totales': mm_totales,
        'horas_lluvia': horas_lluvia,
        'horas_total': horas_total,
        'escenario': escenario,
        'motor': 'local_lisflood_fp_copernicus30',
    }
    if tormenta_info is not None:
        respuesta['tormenta'] = tormenta_info
    return jsonify(respuesta)



# ──────────────────────────────────────────────────────────────────────────
# Endpoint: /memoria_hidrica_punto
# ------------------------------------------------------------------------
# Fase 2 del Hydrologic Decision Engine (sin GEE): los 6 indicadores JRC
# Global Surface Water + los 3 indicadores derivados (memoria hidrológica,
# persistencia, tendencia). Ver jrc_memoria_hidrica.py.
# ──────────────────────────────────────────────────────────────────────────
@app.route('/memoria_hidrica_punto')
def memoria_hidrica_punto():
    if not _jrc_disponible:
        return jsonify({
            'error': ('Módulo jrc_memoria_hidrica no disponible en el servidor. '
                      'Subí jrc_memoria_hidrica.py junto a app.py y volvé a desplegar.'),
        }), 503
    try:
        lat = float(request.args.get('lat'))
        lon = float(request.args.get('lon'))
    ...
    resultado = jrc_memoria_hidrica.consultar_memoria_hidrica(lat, lon)
    return jsonify(resultado)


# ──────────────────────────────────────────────────────────────────────────
# Endpoint: /enso_actual
# ------------------------------------------------------------------------
# Fase ENSO actual (El Niño / La Niña / Neutral) vía ONI de NOAA CPC. Usado
# por el Planificador (scoreENSO) y por el simulador de expansión histórica
# (JRC + ENSO) del Inspector de Riesgo Hídrico. Ver enso_oni.py.
# ──────────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────────
# Endpoint: /envoltura_historica_cuerpo
# ------------------------------------------------------------------------
# Envoltura histórica del comportamiento del agua alrededor de un punto:
# combina occurrence + recurrence (JRC GSW v1.5, 2024) leídos por ventana
# desde COGs propios en Hugging Face, clasifica en 3 niveles (Permanente /
# Estacional confiable / Esporádico) y devuelve los contornos como GeoJSON.
# No usa GEE ni fórmula ponderada — reglas explícitas. Ver jrc_raster_regional.py.
# ──────────────────────────────────────────────────────────────────────────
@app.route('/envoltura_historica_cuerpo')
def envoltura_historica_cuerpo():
    if not _jrc_raster_disponible:
        return jsonify({
            'error': ('Módulo jrc_raster_regional no disponible en el servidor. '
                      'Subí jrc_raster_regional.py junto a app.py, agregá "rasterio" '
                      'y "shapely" a requirements.txt, y volvé a desplegar.'),
        }), 503
    try:
        lat = float(request.args.get('lat'))
        lon = float(request.args.get('lon'))
        buffer_km = float(request.args.get('buffer_km', 5))
    except (TypeError, ValueError):
        return jsonify({'error': 'Parámetros lat/lon requeridos y deben ser numéricos.'}), 400

    resultado = jrc_raster_regional.envoltura_historica(lat, lon, buffer_km)
    return jsonify(resultado)


# ──────────────────────────────────────────────────────────────────────────
# Endpoint: /estacionalidad_agua
# ------------------------------------------------------------------------
# Confiabilidad estacional del agua alrededor de un punto: usa SEASONALITY
# (JRC GSW v1.5, 2024), conteo de meses/año con agua leído por ventana desde
# un COG propio en Hugging Face. Clasifica en 4 niveles (Permanente / Casi
# permanente / Estacional / Esporádico) y devuelve los contornos como
# GeoJSON. No usa GEE ni fórmula ponderada — reglas explícitas.
#
# ADVERTENCIA (ver jrc_raster_regional.py): seasonality es un conteo de
# meses/año con agua, NO un calendario — no permite saber en qué meses del
# año hubo agua, solo qué tan confiable es la fuente en general.
# ──────────────────────────────────────────────────────────────────────────
@app.route('/estacionalidad_agua')
def estacionalidad_agua():
    if not _jrc_raster_disponible:
        return jsonify({
            'error': ('Módulo jrc_raster_regional no disponible en el servidor. '
                      'Subí jrc_raster_regional.py junto a app.py, agregá "rasterio" '
    ...
    resultado = jrc_raster_regional.estacionalidad_agua(lat, lon, buffer_km)
    return jsonify(resultado)


# ──────────────────────────────────────────────────────────────────────────
# Endpoint: /humedal_vs_anomalia
# ------------------------------------------------------------------------
# Combo 5: distingue "humedal natural (expansión esperable)" de "posible
# inundación anómala" cruzando TRANSITIONS (categoría de cambio 1984→2024,
# JRC GSW v1.5 2024) con OCCURRENCE (contexto histórico). Ambos se leen por
# ventana desde COGs propios en Hugging Face — no usa GEE. Reglas explícitas
# en jrc_raster_regional._clasificar_humedal, no fórmula ponderada.
# ──────────────────────────────────────────────────────────────────────────
@app.route('/humedal_vs_anomalia')
def humedal_vs_anomalia():
    if not _jrc_raster_disponible:
        return jsonify({
            'error': ('Módulo jrc_raster_regional no disponible en el servidor. '
                      'Subí jrc_raster_regional.py junto a app.py, agregá "rasterio" '
    ...
    try:
        resultado = jrc_raster_regional.humedal_vs_anomalia(lat, lon, buffer_km)
...
# ──────────────────────────────────────────────────────────────────────────
# Endpoint: /transiciones_completas
# ------------------------------------------------------------------------
# A diferencia de /humedal_vs_anomalia (que agrupa transitions en 5 niveles
# cruzando con occurrence), este endpoint devuelve las 10 categorías
# OFICIALES de TRANSITIONS (JRC GSW v1.5 2024) sin agrupar, para un
# diagnóstico más fino de qué le pasó a cada sector entre 1984 y 2024.
# Reglas explícitas en jrc_raster_regional.transiciones_completas, no
# fórmula ponderada.
# ──────────────────────────────────────────────────────────────────────────
@app.route('/transiciones_completas')
def transiciones_completas():
    if not _jrc_raster_disponible:
        return jsonify({
            'error': ('Módulo jrc_raster_regional no disponible en el servidor. '
                      'Subí jrc_raster_regional.py junto a app.py, agregá "rasterio" '
    ...
    try:
        resultado = jrc_raster_regional.transiciones_completas(lat, lon, buffer_km)
    except Exception as e:
        import traceback
        return jsonify({
            'ok': False,
...
# ──────────────────────────────────────────────────────────────────────────
# Endpoint: /degradacion_aguada
# ------------------------------------------------------------------------
# Alerta temprana de degradación (o mejora) de una fuente de agua: usa
# CHANGE_ABS (diferencia de occurrence entre épocas 1984-1999 y 2000-2024,
# JRC GSW), leído por ventana desde un COG propio en Hugging Face. A
# diferencia de los otros endpoints de este módulo (que describen un
# ESTADO), este mide TENDENCIA. Reglas explícitas en
# jrc_raster_regional._clasificar_cambio, no fórmula ponderada.
# ──────────────────────────────────────────────────────────────────────────
@app.route('/degradacion_aguada')
def degradacion_aguada():
    if not _jrc_raster_disponible:
        return jsonify({
            'error': ('Módulo jrc_raster_regional no disponible en el servidor. '
                      'Subí jrc_raster_regional.py junto a app.py, agregá "rasterio" '
    ...
    try:
        resultado = jrc_raster_regional.degradacion_aguada(lat, lon, buffer_km)
    except Exception as e:
        import traceback
        return jsonify({
            'ok': False,
...
# ──────────────────────────────────────────────────────────────────────────
# Endpoint: /tendencia_confiable_agua
# ------------------------------------------------------------------------
# Filtra ruido de la "Tendencia del agua": cruza CHANGE_ABS (magnitud del
# cambio entre épocas 1984-1999 y 2000-2024, JRC GSW) con RECURRENCE
# (qué tan consistente fue el comportamiento del agua año a año), para
# distinguir una retracción/expansión real y sostenida de un salto puntual
# que no es representativo. Reglas explícitas en
# jrc_raster_regional._clasificar_tendencia_confiable, no fórmula ponderada.
# ──────────────────────────────────────────────────────────────────────────
@app.route('/tendencia_confiable_agua')
def tendencia_confiable_agua():
    if not _jrc_raster_disponible:
        return jsonify({
            'error': ('Módulo jrc_raster_regional no disponible en el servidor. '
                      'Subí jrc_raster_regional.py junto a app.py, agregá "rasterio" '
    ...
    try:
        resultado = jrc_raster_regional.tendencia_confiable_agua(lat, lon, buffer_km)
    except Exception as e:
        import traceback
        return jsonify({
            'ok': False,
...
# ──────────────────────────────────────────────────────────────────────────
# Endpoint: /riesgo_oculto_extent
# ------------------------------------------------------------------------
# Chequeo de sitio antes de construir: cruza EXTENT (máxima superficie de
# agua alguna vez ocupada, binario) con OCCURRENCE, para detectar lugares
# que hoy se ven secos casi siempre pero que sí se mojaron en algún evento
# extremo de los últimos 40 años — el caso más peligroso porque parece
# seguro a simple vista. Reglas explícitas en
# jrc_raster_regional._clasificar_extent, no fórmula ponderada.
# ──────────────────────────────────────────────────────────────────────────
@app.route('/riesgo_oculto_extent')
def riesgo_oculto_extent():
    if not _jrc_raster_disponible:
        return jsonify({
            'error': ('Módulo jrc_raster_regional no disponible en el servidor. '
                      'Subí jrc_raster_regional.py junto a app.py, agregá "rasterio" '
    ...
    try:
        resultado = jrc_raster_regional.riesgo_oculto_extent(lat, lon, buffer_km)
    except Exception as e:
        import traceback
        return jsonify({
            'ok': False,
