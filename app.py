...
def vegetacion():
    ...
    try:
        ...
        hoy = ee.Date(ee.Date(None).format())
        ...
        return jsonify({'ndvi_promedio': round(valor, 3) if valor is not None else None})
...
def riesgo_incendio_tiles():
    ...
    try:
        ...
        hoy = ee.Date(ee.Date(None).format())  # fecha actual del servidor de EE
