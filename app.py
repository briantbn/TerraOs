...
# ──────────────────────────────────────────────────────────────────────────
# HAND calculado LOCAL (Copernicus GLO-30 + pysheds), sin Earth Engine, para
# /inundacion_local_png — reemplaza el cuello de botella de 30 min de
# zona_baja/anegamiento (GEE) por segundos en la primera consulta de una
# zona y milisegundos en las siguientes (caché en disco). Mismo patrón
# defensivo: si falta el archivo o "pysheds"/"rasterio" en requirements.txt,
# solo se desactiva /inundacion_local_png — zona_baja/anegamiento (el
# camino GEE actual) sigue funcionando igual que siempre, sin cambios.
try:
    import local_flood_engine
    _flood_local_disponible = local_flood_engine._MOTOR_LOCAL_DISPONIBLE
    if not _flood_local_disponible:
        print(f'⚠️ local_flood_engine importado pero sin dependencias '
              f'({local_flood_engine._error_import}). Agregá "pysheds" y '
              f'"rasterio" a requirements.txt. /inundacion_local_png '
              f'devolverá error hasta que se resuelva.')
except ImportError as _exc_flood_local:
    local_flood_engine = None
    _flood_local_disponible = False
    print(f'⚠️ Módulo local_flood_engine no disponible ({_exc_flood_local}). '
          f'Asegurate de que local_flood_engine.py esté en el mismo repo/'
          f'directorio que app.py y se haya desplegado junto con él. '
          f'/inundacion_local_png devolverá error hasta que se resuelva.')
...
@app.route('/inundacion_local_png')
def inundacion_local_png():
    """
    Simulación de inundación RÁPIDA, sin Earth Engine — HAND calculado
    localmente (Copernicus GLO-30 + pysheds, ver local_flood_engine.py).

    Pensada para el mismo caso de uso que zona_baja/anegamiento
    (/inundacion_tiles), pero devolviendo directo una imagen PNG (con
    sus bounds geográficos) en vez de un template de teselas XYZ — no
    hace falta pirámide de zoom para "mostrar la mancha de esta zona
    ya", que es el problema real que tarda 30 min hoy.

    Parámetros:
        lat, lon    punto de análisis (obligatorios)
        mm_lluvia   mm de lluvia a simular. Se convierte a un umbral de
                    HAND vía Curve Number (ver mm_a_umbral_hand) — a
                    más mm, más celdas se pintan, mismo comportamiento
                    que el sitio de referencia del INTA (SAEPGa).
        umbral_m    alternativa directa a mm_lluvia: umbral de HAND en
                    metros ya calculado (si se manda, se ignora mm_lluvia).
        curve_number  opcional, default 75 (suelo mixto). Pasar uno más
                    bajo (suelo arenoso, más infiltración) o más alto
                    (suelo arcilloso/urbano, más escorrentía) si ya se
                    conoce del punto (ej. desde suelos_inta_regional).

    Respuesta: { imagen_base64 (PNG), bounds, umbral_efectivo_m,
                 de_cache, tiempo_seg }. `imagen_base64` se sirve como
    L.imageOverlay('data:image/png;base64,' + imagen_base64, bounds)
    en el frontend.
    """
    if not _flood_local_disponible:
        return jsonify({
            'error': 'Módulo local_flood_engine no disponible en el servidor. '
                      'Subí local_flood_engine.py junto a app.py, agregá '
                      '"pysheds" y "rasterio" a requirements.txt, y volvé a desplegar.'
        }), 503
    try:
        lat = float(request.args['lat'])
        lon = float(request.args['lon'])
    except (KeyError, ValueError):
        return jsonify({'error': 'Faltan o son inválidos los parámetros lat/lon.'}), 400

    curve_number = float(request.args.get('curve_number', local_flood_engine.CURVE_NUMBER_DEFECTO))

    if 'umbral_m' in request.args:
        try:
            umbral_m = float(request.args['umbral_m'])
        except ValueError:
            return jsonify({'error': 'umbral_m inválido.'}), 400
    else:
        try:
            mm_lluvia = float(request.args.get('mm_lluvia', 0))
        except ValueError:
            return jsonify({'error': 'mm_lluvia inválido.'}), 400
        umbral_m = local_flood_engine.mm_a_umbral_hand(mm_lluvia, curve_number=curve_number)

    try:
        datos = local_flood_engine.obtener_hand_local(lat, lon)
    except RuntimeError as e:
        return jsonify({'error': str(e)}), 502

    imagen_base64 = local_flood_engine.generar_imagen_png_base64(
        datos['hand'], umbral_m, datos['nodata'],
    )
    ...
    return jsonify({
        ...
        'bounds': bounds,
        'umbral_efectivo_m': round(umbral_m, 3),
        'de_cache': datos['de_cache'],
        'tiempo_seg': datos['tiempo_seg'],
    })
...
# ──────────────────────────────────────────────────────────────────────────
# Endpoint: /suelo_mejor_resolucion
# ------------------------------------------------------------------------
