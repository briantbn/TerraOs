...
def _flood_local_disponible_check():
    ...
    try:
        ...
        return bool(mod._MOTOR_LOCAL_DISPONIBLE)
    except Exception:
        return False
...
# /inundacion_animacion usa cumulativeCost() para calcular el "orden de
# llegada" del agua — a diferencia de las capas estáticas (que solo
# clasifican cada celda de forma independiente), cumulativeCost recorre la
# grilla completa como un grafo, y su costo crece muy rápido con el área.
# A 90m de resolución, un radio de 80km ya son ~3 millones de celdas —
# suficiente para que el cálculo tarde más de lo que tolera el proxy del
# hosting antes de cortar la conexión ("Failed to fetch" en el navegador,
# sin llegar siquiera a devolver un error). Se topea la animación a un
# radio bastante más chico que el resto de los endpoints de inundación.
RADIO_KM_MAX_ANIMACION = 35
...
def gee_tiles():
    ...
    try:
        ...
        if band == 'dNBR':
            ...
            # FIX: mismo problema que la rama de abajo (band != dNBR) — sin
            # clip, EE rechaza tiles a zoom alejado por exceso de píxeles.
            # Acá es aún más crítico: dNBR computa DOS composiciones
            # (pre/post) y una resta pixel a pixel, el doble de trabajo por
            # tile que una capa simple.
            map_id_dnbr = img_dnbr.clip(region).getMapId(vis_dnbr)
        ...
        # FIX: se había sacado .clip(region) para evitar parches
        # transparentes en los bordes (ver comentario abajo) — pero eso
        # dejaba el mosaico SIN RECORTAR (~660×660 km, resolución nativa
        # 10-30m). A zoom alejado, cada tile del mapa necesita agregar una
        # cantidad enorme de píxeles nativos sobre esa área sin recortar,
        # y Earth Engine rechaza el tile con HTTP 400 por exceder su
        # límite de cómputo por tile ("La capa se generó pero el servidor
        # de Earth Engine rechazó los tiles").
        # El bbox ancho de filterBounds() (arriba) ya resuelve el problema
        # original de parches por franjas orbitales faltantes, así que
        # recortar de nuevo a `region` (el círculo de radius_km) no trae
        # ese bug de vuelta — solo evita pedirle a EE que sirva tiles de
        # un área mucho más grande de la que realmente hace falta mostrar.
...
def riesgo_incendio_tiles():
    ...
    try:
        ...
        # ── Índice combinado ──────────────────────────────────────────
        # FIX: el comentario decía "sin .clip(region): mismo motivo que en
        # /gee_tiles" — pero ese motivo (parches por franjas orbitales
        # faltantes) ya se soluciona con el bbox ancho en NDVI/CHIRPS, no
        # con dejar de recortar acá. Sin clip, a zoom alejado EE puede
        # rechazar el tile por exceso de píxeles a resolución nativa sobre
        # el radio de 150km por defecto. Se recorta de nuevo, mismo fix
        # aplicado en /gee_tiles y /inundacion_tiles.
...
def inundacion_tiles():
    ...
    try:
        ...
        # FIX: el propio docstring de esta función ya advertía de este bug
        # ("un radio grande puede provocar 400 de Earth Engine al cargar
        # los tiles") pero nunca se había aplicado el recorte. Igual que en
        # /gee_tiles y /inundacion_animacion: sin .clip(region), a zoom
        # alejado EE puede rechazar el tile por exceso de píxeles —
        # especialmente en 'zona_baja'/'anegamiento', donde `imagen` puede
        # venir sin recortar del motor nuevo (flood_simulation_engine.py).
        map_id = imagen.clip(region).getMapId(vis)
        respuesta = {
            'tile_url': map_id['tile_fetcher'].url_format,
            'tipo'    : tipo,
            'vis'     : vis,
...
# año calendario que mejor representa cada escenario JRC. Los 3 botones
# reales del Simulador de Inundación son 1998 (8.39m), 1992 (8.63m) y 1983
# (9.03m) — 1983 queda deliberadamente afuera de este diccionario: JRC GSW
# YearlyHistory arranca en 1984, así que ese año no se puede mostrar con
# este método (el frontend lo sabe — ESCENARIOS_SIMULADOR_INUNDACION no le
# pone `anioJRC` a ese botón — y sigue usando el simulador HAND genérico
# para él en vez de llamar a este endpoint).
ESCENARIOS_JRC_ANIO = {
    '1998': 1998,
    '1992': 1992,
...
def inundacion_historica_real():
    """
    Parámetros:
        escenario   '1998' | '2013' | 'abril_2026' (requerido)
    ...
    try:
        ...
        if escenario in ESCENARIOS_JRC_ANIO:
            ...
            coleccion = (ee.ImageCollection('COPERNICUS/S1_GRD')
                         ...
                         .select('VH'))
            # FIX rendimiento: antes había un coleccion.size().getInfo() acá para
            # devolver 404 si no había escenas. Ese chequeo obliga a Earth Engine a
            # contar sincrónicamente las imágenes en un radio de hasta 200km y 30
            # días — con Sentinel-1 eso puede tardar varios segundos y, si el
            # servidor tiene pocos workers, bloquea el resto de la app mientras
            # tanto (por eso se sentía lento TODO el simulador, no solo este botón).
            # getMapId() no necesita ese chequeo: si la colección está vacía,
            # median() da una imagen totalmente enmascarada y el tile sale
            # transparente — el frontend simplemente no ve nada pintado, sin
            # pagar el costo de ese conteo previo.
...
def inundacion_animacion():
    ...
    try:
        lat       = float(request.args.get('lat',  -27.48))
        lon       = float(request.args.get('lon',  -58.83))
        radius_km_pedido = min(float(request.args.get('radius_km', 200)), RADIUS_KM_MAX_INUNDACION)
        # FIX: cumulativeCost() escala muy mal con el área — se topea acá,
        # más chico que el resto de los endpoints de inundación (ver nota
        # junto a RADIO_KM_MAX_ANIMACION), para evitar que el proxy del
        # hosting corte la conexión antes de que Earth Engine termine
        # ("Failed to fetch" sin ninguna respuesta).
        radius_km = min(radius_km_pedido, RADIO_KM_MAX_ANIMACION)
        radio_recortado = radius_km < radius_km_pedido
        ...
        for i in range(1, frames + 1):
            ...
            # FIX: mismo problema que en /gee_tiles — sin .clip(region), a
            # zoom alejado EE puede rechazar el tile con HTTP 400 por
            # exceso de píxeles. `candidatas` puede venir del motor nuevo
        ...
        # FIX: los fotogramas de arriba usan el percentil 95 de distancia
        # como "100%" (a propósito, para no pagar el costo de un max()
        # exacto — ver nota arriba). Eso significa que el 5% del área más
        # lejana del río NUNCA aparecía en ningún fotograma, aunque sí
        # está presente en la capa estática "Zona baja" (mismo punto/
        # radio/umbral) — la animación se sentía "incompleta" comparada
        # con esa capa. Se agrega UN fotograma final con la extensión
        # COMPLETA real (sin el recorte al percentil 95), para que la
        # animación siempre termine mostrando exactamente lo mismo que la
        # capa estática, sin pagar el costo de usar max() en los pasos
        # intermedios.
        frame_final_img = (zona_conectada if conectividad_ok else candidatas.selfMask()).clip(region)
        map_id_final = frame_final_img.getMapId({'palette': ['#1e40af']})
        fotogramas.append({
            'orden'                 : frames + 1,
            'distancia_m'           : None,
            'porcentaje'            : 100.0,
            'tile_url'              : map_id_final['tile_fetcher'].url_format,
            'es_extension_completa' : True,
        })
        ...
        respuesta = {
            ...
            'radius_km'       : radius_km,
            'radio_recortado' : radio_recortado,
            'frames'          : len(fotogramas),
            'distancia_max_m' : round(max_costo_m, 1),
            'fotogramas'      : fotogramas,
            'nota'            : (
                (f'Radio recortado de {radius_km_pedido:.0f}km a {radius_km:.0f}km para esta '
                 f'animación (cumulativeCost es costoso y un radio grande puede tardar '
                 f'demasiado) — la capa estática sí soporta el radio completo. '
                 if radio_recortado else '') +
