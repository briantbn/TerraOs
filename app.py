...
def _band_sentinel(col, band):
    # FIX: en vez de .median() (que deja huecos transparentes si en el rango
    # de fechas elegido no hubo, para alguna zona, ninguna imagen entera por
    # debajo del umbral de nubes), ordenamos por nubosidad ascendente y
    # armamos un mosaico: cada pixel se completa con la imagen MENOS nublada
    # que tenga dato válido ahí. Esto elimina los "cortes" rectos en el mapa.
...
def riesgo_incendio_tiles():
    ...
    try:
        ...
        ndvi = s2.sort('CLOUDY_PIXEL_PERCENTAGE').mosaic().normalizedDifference(['B8', 'B4'])
