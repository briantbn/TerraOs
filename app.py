...
def _region_poligono_desde_query():
    """
    Si el request trae ?poligono=[[lon,lat],[lon,lat],...] (el campo que el
    usuario delimitó con el botón "Delimitar" del mapa principal, ver
    campoPoligono en el frontend), devuelve un ee.Geometry.Polygon con ese
    anillo — se cierra automáticamente si el primer y último punto no
    coinciden. Si no viene el parámetro o es inválido (menos de 3 puntos,
    JSON roto, etc.), devuelve None y el llamador sigue con el radio como
    hasta ahora.

    OJO: esto NO reemplaza la región usada para el cómputo (agua conocida,
    conectividad hidráulica, etc. siguen buscándose en el buffer de
    radius_km alrededor de lat/lon, porque el río real puede estar fuera
    del campo) — el polígono solo se usa para RECORTAR la imagen final
    antes de generarle la tile_url, así el usuario ve la simulación
    limitada a su campo aunque el cálculo interno haya mirado más lejos.
    """
    raw = request.args.get('poligono')
    if not raw:
        return None
    try:
        ...
        if not isinstance(coords, list) or len(coords) < 3:
            return None
        anillo = [[float(p[0]), float(p[1])] for p in coords]
...
def inundacion_tiles():
    ...
    try:
        ...
        # Campo delimitado por el usuario (botón "Delimitar"): si viene, la
        # imagen final se recorta a ese polígono exacto — ver
        # _region_poligono_desde_query() para el porqué de no tocar acá el
        # radio/región de cómputo.
        region_poligono = _region_poligono_desde_query()
        ...
        if region_poligono is not None:
            imagen = imagen.clip(region_poligono)
        ...
        respuesta = {
            ...
            'tipo'    : tipo,
            'vis'     : vis,
            'limitado_a_campo': region_poligono is not None,
        }
        if tipo == 'zona_baja':
...
def inundacion_animacion():
    ...
    try:
        ...
        # Ver nota en _region_poligono_desde_query(): el cómputo sigue
        # buscando agua/conectividad en el buffer de radius_km, solo se
        # recorta cada fotograma al campo delimitado antes de tesearlo.
        region_poligono = _region_poligono_desde_query()
        ...
        for i in range(1, frames + 1):
            ...
            if region_poligono is not None:
                frame_img = frame_img.clip(region_poligono)
...
# área visible del mapa, no un punto). A diferencia de /suelo_mejor_resolucion
# (un punto para el Inspector), esto trae varias features dentro de un bbox
# ya normalizadas al esquema común, eligiendo la MEJOR fuente disponible
# para esa zona (misma cascada de escalas). Reemplaza al fetch directo que
# el frontend le hacía antes al WFS nacional 1:500.000 (con un proxy CORS
# externo) — ver suelos_inta_regional.capa_visual_bbox() para el detalle.
# ──────────────────────────────────────────────────────────────────────────
...
def inundacion_local():
    ...
    # Campo delimitado por el usuario (botón "Delimitar"): mismo parámetro
    # `poligono` que usa el camino GEE (/inundacion_tiles) — ver
    # _region_poligono_desde_query(). Acá no hay ee.Geometry, se rasteriza
    # directo sobre la grilla del HAND local con mascara_poligono().
    poligono_coords = None
    _raw_poligono = request.args.get('poligono')
    if _raw_poligono:
        try:
            _coords = json.loads(_raw_poligono)
            if isinstance(_coords, list) and len(_coords) >= 3:
                poligono_coords = _coords
        except Exception:  # noqa: BLE001
            poligono_coords = None
    ...
    mascara_campo = local_flood_engine.mascara_poligono(datos['hand'].shape, datos['transform'], poligono_coords)

    try:
        if tipo in ('zona_baja', 'anegamiento'):
            imagen_b64 = local_flood_engine.generar_imagen_png_base64(datos['hand'], umbral_m, datos['nodata'],
                                                                        mascara_extra=mascara_campo)
            vis = {'palette': ['#bcd2ee']}
            extra = {}
        else:
            imagen_b64, vis, extra = local_flood_engine.renderizar_capa(tipo, datos, mascara_extra=mascara_campo)
    ...
    respuesta = {
        ...
        'limitado_a_campo': mascara_campo is not None,
        **extra,
    }
    if umbral_m is not None:
...
if __name__ == '__main__':
    ...
    # threaded=True: sin esto, mientras corre una simulación larga (ej.
    # /inundacion_dinamica, que puede tardar 2-3 min en terreno plano) el
    # servidor puede quedar sin atender otras requests -- incluyendo el
    # health-check de Render -- lo que en el peor caso hace que Render
    # reinicie la instancia a mitad de la simulación y tire la conexión
    # (eso se ve del lado del navegador como "Failed to fetch").
