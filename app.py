...
def copernicus_info():
    ...
    try:
        ...
        catalog_url = 'https://catalogue.dataspace.copernicus.eu/stac/collections/sentinel-2-l2a/items'
        ...
        if r.status_code != 200:
            return jsonify({'disponible': False, 'mensaje': 'Error consultando el catálogo de Copernicus.'}), 502
        ...
        if not candidatas:
            return jsonify({
                'disponible': False,
                'mensaje': 'No hay imágenes Sentinel-2 con esa nubosidad para la fecha elegida.'
            })

        mejor = min(candidatas, key=lambda it: it['properties'].get('eo:cloud_cover', 100))
