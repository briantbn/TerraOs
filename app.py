...
try:
    import suelos_inta_regional
    _suelos_regional_disponible = True
except ImportError as _exc_suelos:
    suelos_inta_regional = None
    _suelos_regional_disponible = False
    print(f'⚠️ Módulo suelos_inta_regional no disponible ({_exc_suelos}). '
          f'Asegurate de que suelos_inta_regional.py esté en el mismo repo/'
          f'directorio que app.py y se haya desplegado junto con él. '
          f'/suelo_mejor_resolucion devolverá error hasta que se resuelva.')
...
try:
    import humedales_inta
    _humedales_disponible = True
except ImportError as _exc_humedales:
    humedales_inta = None
    _humedales_disponible = False
    print(f'⚠️ Módulo humedales_inta no disponible ({_exc_humedales}). '
          f'Revisá que humedales_inta.py esté desplegado junto a app.py y '
          f'que "rasterio" esté en requirements.txt. '
          f'/humedal_punto devolverá error hasta que se resuelva.')
...
try:
    import terreno_merit_hydro
    _terreno_disponible = True
except ImportError as _exc_terreno:
    terreno_merit_hydro = None
    _terreno_disponible = False
    print(f'⚠️ Módulo terreno_merit_hydro no disponible ({_exc_terreno}). '
          f'Revisá que terreno_merit_hydro.py esté desplegado junto a app.py '
          f'y que "rasterio"/"numpy" estén en requirements.txt. '
          f'/terreno_punto devolverá error hasta que se resuelva.')
...
try:
    import local_flood_engine
    _flood_local_disponible = bool(local_flood_engine._MOTOR_LOCAL_DISPONIBLE)
    if not _flood_local_disponible:
        print(f'⚠️ local_flood_engine importado pero el motor no está disponible '
              f'({local_flood_engine._error_import}). Revisá que "pysheds", '
              f'"rasterio" y "Pillow" estén en requirements.txt. '
              f'/inundacion_local devolverá error hasta que se resuelva.')
except Exception as _exc_flood_local:
    local_flood_engine = None
    _flood_local_disponible = False
    print(f'⚠️ Módulo local_flood_engine no disponible ({_exc_flood_local}). '
          f'Revisá que local_flood_engine.py esté desplegado junto a app.py '
          f'y que "pysheds"/"rasterio"/"Pillow"/"numpy" estén en requirements.txt. '
          f'/inundacion_local devolverá error hasta que se resuelva.')
...
try:
    import jrc_raster_regional
    _jrc_raster_disponible = True
except ImportError as _exc_jrc_raster:
    jrc_raster_regional = None
    _jrc_raster_disponible = False
    print(f'⚠️ Módulo jrc_raster_regional no disponible ({_exc_jrc_raster}). '
          f'Revisá que jrc_raster_regional.py esté desplegado junto a app.py y '
          f'que "rasterio"/"shapely" estén en requirements.txt. '
          f'/envoltura_historica_cuerpo, /estacionalidad_agua, '
          f'/humedal_vs_anomalia, /degradacion_aguada y /riesgo_oculto_extent '
          f'devolverán error hasta que se resuelva.')

# ──────────────────────────────────────────────────────────────────────────
# MÓDULO OPCIONAL: jrc_memoria_hidrica
# ──────────────────────────────────────────────────────────────────────────
try:
    import jrc_memoria_hidrica
    _jrc_disponible = True
except ImportError as _exc_jrc:
    jrc_memoria_hidrica = None
    _jrc_disponible = False
    print(f'⚠️ Módulo jrc_memoria_hidrica no disponible ({_exc_jrc}). '
          f'Revisá que jrc_memoria_hidrica.py esté desplegado junto a app.py '
          f'y que "rasterio" esté en requirements.txt. '
          f'/memoria_hidrica_punto devolverá error hasta que se resuelva.')
...
# ──────────────────────────────────────────────────────────────────────────
@app.route('/debug_motor_local')
def debug_motor_local():
    """
    Diagnóstico instantáneo del motor local de inundación — no descarga
    ningún DEM ni corre pysheds, solo lee la configuración YA cargada en
    memoria del proceso. Pensado para chequear un deploy sin necesitar el
    Shell de Render (que es una feature paga, no está en el plan Free).
    """
    if local_flood_engine is None:
        return jsonify({
            'modulo_importado': False,
            'error': 'local_flood_engine no se pudo importar en absoluto (revisar que el .py esté en el repo).',
        })
    return jsonify({
        'modulo_importado': True,
        'motor_disponible': bool(local_flood_engine._MOTOR_LOCAL_DISPONIBLE),
        'error_import': local_flood_engine._error_import,
        'radio_analisis_km': local_flood_engine.RADIO_ANALISIS_KM,
        'cache_grid_deg': local_flood_engine.CACHE_GRID_DEG,
        'umbral_acumulacion_cauce': local_flood_engine.UMBRAL_ACUMULACION_CAUCE,
        'numba_disable_jit': os.environ.get('NUMBA_DISABLE_JIT', '(no seteado)'),
        'gdal_http_timeout': os.environ.get('GDAL_HTTP_TIMEOUT', '(no seteado)'),
    })
...
def inundacion_local():
    ...
    try:
        ...
        return jsonify({'error': f'Error inesperado calculando HAND local: {exc}'}), 500
