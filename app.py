...
# Tope específico para tipo='depresiones' (Fill Sinks sobre DEM corregido
# por dosel/edificios): pipeline mucho más pesado, deja de poder servirse
# como tiles en vivo por encima de este radio (ver nota junto a su uso).
RADIUS_KM_MAX_DEPRESIONES = 50
...
def _dem_composite_correccion_dosel(elevacion, region):
    ...
    proxy de "esto es bosque"), esta versión usa:
      ...
      4. Corrección específica para EDIFICIOS — HÍBRIDA (v3.1): el CHM de
         ...
         (inpainting, una pasada, 100 m), en vez de dejar sin corregir. Se
    ...
    #      circundante (una pasada, 100 m — versión simplificada de
    #      "Interpolación de alturas.js", que usaba dos pasadas 90m+150m;
    #      acá se redujo a una sola para poder servirse como tiles en vivo
    #      sin pegar contra los límites de cómputo de Earth Engine). La
    #      diferencia entre la elevación original y ese terreno interpolado
    #      es la corrección a aplicar en esos píxeles.
    ...
    # Una sola pasada (antes eran dos encadenadas, 90m + 150m): mismo
    # objetivo (rellenar el hueco urbano con el entorno), mucho más barato
    # de servir como tiles en vivo. 100m alcanza para manzanas/edificios
    # aislados; construcciones urbanas muy extensas quedan algo más
    # promediadas, pero siguen siendo mejor que dejarlas sin corregir.
    terreno_interpolado = dem_sin_edificios.focal_mean(
...
def inundacion_tiles():
    ...
    try:
        ...
        if tipo == 'depresiones':
            # Pipeline mucho más pesado (dosel + edificios + fill-sinks
            # encadenados) — a 100km+ de radio Earth Engine devuelve 429/400
            # al servir los tiles. Es además un análisis inherentemente
            # local (una depresión de interés no se mira útilmente a escala
            # de 100km), así que un radio chico no resta valor real.
            radius_km = min(radius_km, RADIUS_KM_MAX_DEPRESIONES)
