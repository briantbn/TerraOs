...
# Distancia (en grados) para "enganchar" el punto de salida al píxel de
# mayor acumulación de flujo más cercano -- absorbe el error de unos
# metros/cientos de metros al marcar manualmente el punto más bajo del
# lote. ~1 km: bastante generoso para no fallar en zonas de relieve muy
# plano (llanura), donde el cauce definido puede quedar algo más lejos del
# punto marcado que en zonas de relieve marcado.
SNAP_DIST_GRADOS = 0.01
...
def _correr_paso(nombre, archivo_esperado, workdir, mensajes, funcion, *args, **kwargs):
    """
    WhiteboxTools no lanza excepciones en Python: devuelve un código de
    salida (0 = éxito) y, además, puede imprimir mensajes de diagnóstico
    por su propio callback -- los capturamos en `mensajes` en vez de
    tirarlos (antes se perdían con verbose=False), porque son justo lo que
    explica un fallo silencioso (ej. "no se encontró un cauce cerca del
    punto de enganche").

    Además de chequear el código de salida, verificamos que el archivo de
    salida esperado REALMENTE exista -- un código 0 no siempre implica que
    el archivo se haya escrito (visto en la práctica con `watershed`).
    """
    def _callback(mensaje):
        mensajes.append(mensaje)

    codigo = funcion(*args, callback=_callback, **kwargs)
    existe = os.path.exists(os.path.join(workdir, archivo_esperado))

    if codigo != 0 or not existe:
        detalle = ' | '.join(mensajes[-8:]) if mensajes else '(sin mensajes de WhiteboxTools)'
        raise RuntimeError(
            f'WhiteboxTools falló en el paso "{nombre}" (código {codigo}, '
            f'archivo generado: {existe}). Mensajes: {detalle}'
        )
...
def delimitar_cuenca_aportante(lat, lon, buffer_km=15.0):
    ...
    try:
        ...
        wbt.verbose = False  # los mensajes igual se capturan vía callback, no se pierden

        mensajes = []

        # Mismo pipeline D8 estándar que el script de referencia:
        _correr_paso('suavizado', 'dem_smooth.tif', workdir, mensajes,
                     wbt.fast_almost_gaussian_filter,
                     'dem.tif', 'dem_smooth.tif', sigma=1.8)
        _correr_paso('relleno de depresiones', 'dem_breach.tif', workdir, mensajes,
                     wbt.breach_depressions,
                     'dem_smooth.tif', 'dem_breach.tif')
        _correr_paso('dirección de flujo (D8)', 'fdr.tif', workdir, mensajes,
                     wbt.d8_pointer,
                     'dem_breach.tif', 'fdr.tif')
        _correr_paso('acumulación de flujo (D8)', 'fac.tif', workdir, mensajes,
                     wbt.d8_flow_accumulation,
                     'dem_breach.tif', 'fac.tif', 'cells')
        _correr_paso('enganche del punto de salida', 'snap.shp', workdir, mensajes,
                     wbt.snap_pour_points,
                     'outlet.shp', 'fac.tif', 'snap.shp', SNAP_DIST_GRADOS)
        _correr_paso('delimitación de cuenca', 'watershed.tif', workdir, mensajes,
                     wbt.watershed,
