...
#
# Endpoint usado: /api/area/csv/[MAP_KEY]/[SENSOR]/[BBOX]/[DIAS]/[FECHA]
#   - BBOX de Argentina en formato west,south,east,north (documentado así
#     explícitamente para /area/csv/, con la forma de 5 segmentos incluyendo
#     fecha — a diferencia de /country/csv/, que en las pruebas devolvió
#     'Invalid API call.' al agregarle fecha; parece no soportar ese
#     parámetro pese a estar documentado con day_range).
...
#     bloques de a 5 — la documentación pública decía 1..10, pero NASA
#     devuelve 'Invalid day range. Expects [1..5].' en la práctica; 5 es
#     el límite real confirmado.
...
# /api/country/csv/ solo está documentado con 4 segmentos (sin fecha) —
# agregarle fecha devolvía 'Invalid API call.' (probado). /api/area/csv/
# SÍ está documentado con la forma de 5 segmentos, [.../DIA_RANGO/FECHA],
# así que usamos un bounding box de Argentina en vez del código de país.
# west,south,east,north — cubre continente + Tierra del Fuego, con margen.
BBOX_ARGENTINA = '-73.6,-55.3,-53.5,-21.7'
...
def descargar_firms_argentina(fecha_inicio, fecha_fin, map_key, sensor='MODIS_SP',
                               dias_por_bloque=5, pausa_seg=1.0, cache_dir=CACHE_DIR):
    ...
    while fecha_actual <= fecha_fin:
        ...
        url = (f'{BASE_API_FIRMS}/area/csv/{map_key}/{sensor}/{BBOX_ARGENTINA}/'
               f'{dias_por_bloque}/{etiqueta}')
        try:
            r = requests.get(url, timeout=60)
        except requests.RequestException as exc:
            print(f'[FIRMS] {etiqueta}: falló la conexión ({exc}) — se omite ese bloque.')
            fecha_actual += pd.Timedelta(days=dias_por_bloque)
            continue

        # OJO: no usamos r.raise_for_status() acá a propósito — si tiraba
        # antes de este punto, nunca llegábamos a ver r.text, que es
        # justamente donde NASA manda el mensaje de error real (ej. "map_key
        # not valid" o el motivo puntual del 400) en vez de solo el código
        # HTTP. Mejor mirar SIEMPRE el cuerpo primero.
        primera_linea = r.text.split('\n', 1)[0]
        if r.status_code != 200 or 'latitude' not in primera_linea.lower():
            print(f'[FIRMS] {etiqueta}: HTTP {r.status_code} — {r.text[:300]!r} — se omite.')
            fecha_actual += pd.Timedelta(days=dias_por_bloque)
            continue
...
def main():
    ...
    ap.add_argument('--dias-por-bloque', type=int, default=5,
                     help='Tamaño de cada consulta a la API (días). NASA confirmó el límite '
                          'real en 5 (mensaje de error: "Invalid day range. Expects [1..5]"), '
                          'pese a que la documentación pública decía 1..10.')
