...
# NOAA (como muchos sitios .gov) suele filtrar por User-Agent y devolver 403
# al User-Agent por defecto de requests ("python-requests/x.y.z"). Sin este
# header, la descarga podía fallar en el servidor aunque el archivo esté
# perfectamente disponible (confirmado bajándolo directo: sí responde).
_HEADERS_NOAA = {
...
def consultar_oni():
    ...
    try:
        r = requests.get(_ONI_URL, headers=_HEADERS_NOAA, timeout=15)
        r.raise_for_status()
