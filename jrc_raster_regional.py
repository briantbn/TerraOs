...
# Info de display para las 10 categorías OFICIALES de transitions, sin
# simplificar a grupos (a diferencia de _clasificar_humedal / NIVEL_INFO_HUMEDAL,
# que las agrupa en 5 niveles cruzando con occurrence). Esta es la lectura
# "cruda" del JRC: qué le pasó exactamente a cada píxel entre 1984 y 2024.
NIVEL_INFO_TRANSICIONES = {
    1:  {"nombre": "Permanente estable",         "color": "#0b4f8a", "desc": "Tenía agua permanente en 1984 y la sigue teniendo en 2024: sin cambio de fondo."},
    2:  {"nombre": "Nuevo permanente",           "color": "#15803d", "desc": "No tenía agua permanente antes y ahora sí: consolidación fuerte de una fuente nueva."},
    3:  {"nombre": "Permanente perdido",         "color": "#7f1d1d", "desc": "Tenía agua permanente y la perdió por completo: la pérdida más severa que registra este dataset."},
    4:  {"nombre": "Estacional estable",         "color": "#5fa3d6", "desc": "Tenía un comportamiento estacional en 1984 y lo sigue teniendo en 2024: sin cambio de fondo."},
    5:  {"nombre": "Nuevo estacional",           "color": "#4ade80", "desc": "No tenía agua estacional antes y ahora sí: una fuente nueva, pero todavía no permanente."},
    6:  {"nombre": "Estacional perdido",         "color": "#dc2626", "desc": "Tenía un comportamiento estacional y lo perdió: dejó de tener agua de forma predecible."},
    7:  {"nombre": "Estacional → Permanente",    "color": "#16a34a", "desc": "Pasó de estacional a permanente: se está consolidando, cada vez más confiable."},
    8:  {"nombre": "Permanente → Estacional",    "color": "#f59e0b", "desc": "Pasó de permanente a estacional: se está degradando, aunque todavía no desapareció del todo."},
    9:  {"nombre": "Efímero (antes permanente)", "color": "#9333ea", "desc": "Era permanente y ahora oscila sin consolidarse: comportamiento inestable, a vigilar de cerca."},
    10: {"nombre": "Efímero (antes estacional)", "color": "#c084fc", "desc": "Era estacional y ahora oscila sin consolidarse: comportamiento inestable."},
}
...
# Niveles del combo CHANGE_ABS + RECURRENCE ("tendencia confiable"): a
# diferencia de _clasificar_cambio (que solo mira la magnitud del cambio
# entre épocas), este combo cruza esa tendencia con recurrence (qué tan
# consistente fue el comportamiento del agua año a año) para distinguir una
# tendencia real y sostenida de un salto puntual que no es representativo
# — mismo umbral de recurrence >= 50 que ya se usa en _clasificar (nivel
# "Estacional confiable"), para no inventar un corte nuevo.
NIVEL_INFO_TENDENCIA_CONFIABLE = {
    1: {"nombre": "Retracción confiable",     "color": "#7f1d1d", "desc": "Perdió occurrence entre épocas Y el comportamiento histórico es consistente (recurrence ≥ 50%): es una tendencia real y sostenida, no ruido."},
    2: {"nombre": "Retracción posible ruido", "color": "#f59e0b", "desc": "Perdió occurrence entre épocas, pero la recurrence histórica es baja (<50%): el comportamiento del agua ahí fue errático, así que esta caída podría ser un evento puntual, no una tendencia consolidada."},
    3: {"nombre": "Expansión confiable",      "color": "#15803d", "desc": "Ganó occurrence entre épocas Y el comportamiento histórico es consistente (recurrence ≥ 50%): es una tendencia real y sostenida, no ruido."},
    4: {"nombre": "Expansión posible ruido",  "color": "#86efac", "desc": "Ganó occurrence entre épocas, pero la recurrence histórica es baja (<50%): el comportamiento del agua ahí fue errático, así que esta expansión podría ser un evento puntual, no una tendencia consolidada."},
}
...
def transiciones_completas(lat, lon, buffer_km=5.0):
    """
    Devuelve un GeoJSON FeatureCollection con las 10 categorías OFICIALES
    de TRANSITIONS (JRC GSW v1.5 2024) sin agrupar, a diferencia de
    humedal_vs_anomalia (que las simplifica a 5 niveles cruzando con
    occurrence). Da el diagnóstico más fino posible: qué le pasó
    exactamente a cada sector entre 1984 y 2024 (por ejemplo, distingue
    "se está consolidando" de "ya es permanente hace rato", algo que
    humedal_vs_anomalia no puede mostrar porque agrupa ambos casos). Cada
    Feature: properties = {nivel, nombre, color, desc}.
    """
    bbox = _bbox_desde_punto(lat, lon, buffer_km)
    ...
    valido = ~np.isin(transitions, NODATA_VALORES) & np.isin(transitions, range(1, 11))
    clase = np.where(valido, transitions, 0).astype(np.uint8)
    ...
    return {
        "ok": True,
        "fuente": "JRC Global Surface Water v1.5 (2024) — transitions (10 categorías oficiales)",
        "buffer_km": buffer_km,
        "geojson": {"type": "FeatureCollection", "features": features},
    }
...
def _clasificar_tendencia_confiable(change_abs, rec):
    """
    Reglas explícitas, cruzando change_abs (magnitud del cambio entre
    épocas) con recurrence (consistencia histórica año a año):
      1 = Retracción confiable    -> perdió >= 20 puntos Y recurrence >= 50
      2 = Retracción posible ruido -> perdió >= 20 puntos Y recurrence < 50
      3 = Expansión confiable      -> ganó >= 20 puntos Y recurrence >= 50
      4 = Expansión posible ruido  -> ganó >= 20 puntos Y recurrence < 50
      0 = Sin cambio relevante (|change_abs| < 20, igual que "Estable" en
          _clasificar_cambio) o sin dato
    No reclasifica la magnitud (severa/moderada/fuerte): eso ya lo hace
    _clasificar_cambio / degradacion_aguada. Este combo responde una
    pregunta distinta: de los cambios detectados, ¿cuáles son confiables?
    """
    valido = ~np.isin(change_abs, NODATA_CHANGE) & ~np.isin(rec, NODATA_VALORES)

    retraccion = valido & (change_abs <= -20)
    expansion = valido & (change_abs >= 20)
    recurrence_alta = rec >= 50

    clase = np.zeros(change_abs.shape, dtype=np.uint8)
    clase[retraccion & recurrence_alta] = 1
    clase[retraccion & ~recurrence_alta] = 2
    clase[expansion & recurrence_alta] = 3
    clase[expansion & ~recurrence_alta] = 4
    return clase


def tendencia_confiable_agua(lat, lon, buffer_km=5.0):
    """
    Devuelve un GeoJSON FeatureCollection cruzando CHANGE_ABS (tendencia
    entre épocas) con RECURRENCE (consistencia histórica), para filtrar
    ruido de la "Tendencia del agua" (degradacion_aguada): distingue una
    retracción o expansión real y sostenida de un salto puntual que no es
    representativo del comportamiento habitual de ese cuerpo de agua. Cada
    Feature: properties = {nivel, nombre, color, desc}.
    """
    bbox = _bbox_desde_punto(lat, lon, buffer_km)
    ...
    try:
        ...
        return {"ok": False, "error": f"no se pudo leer el raster JRC (change_abs/recurrence): {e}"}

    if change_abs is None or rec is None or change_abs.size == 0:
        return {"ok": False, "error": "sin datos JRC para este punto (fuera de cobertura del raster nacional)"}

    if change_abs.shape != rec.shape:
        h = min(change_abs.shape[0], rec.shape[0])
        w = min(change_abs.shape[1], rec.shape[1])
        change_abs, rec = change_abs[:h, :w], rec[:h, :w]

    clase = _clasificar_tendencia_confiable(change_abs, rec)
    ...
    for nivel in (1, 2, 3, 4):
        ...
        if not geoms:
            continue
        geom_unida = unary_union(geoms).simplify(0.0003, preserve_topology=True)
        info = NIVEL_INFO_TENDENCIA_CONFIABLE[nivel]
        features.append({
            "type": "Feature",
            "geometry": mapping(geom_unida),
            "properties": {"nivel": nivel, **info},
    ...
    return {
        "ok": True,
        "fuente": "JRC Global Surface Water (2024) — change_abs + recurrence",
        "buffer_km": buffer_km,
        "geojson": {"type": "FeatureCollection", "features": features},
    }
