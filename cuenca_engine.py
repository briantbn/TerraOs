...
def delimitar_cuenca(lat, lon, radio_km=None):
    ...
    # De acá solo hacen falta acumulacion y fdir — hand y dem_sin_dep se
    # calculan igual (son parte del mismo pipeline de pysheds) pero nunca
    # se usan en esta función. Se descartan enseguida para no cargar con
    # dos arrays completos de más durante todo el resto del cálculo.
    hand, acumulacion, fdir, dem_sin_dep = local_flood_engine._calcular_hand(
        dem, transform, crs, nodata,
    )
    del hand, dem_sin_dep
    ...
    # Ya no hacen falta los arrays/rasters grandes — liberarlos acá ayuda
    # a que el proceso le devuelva esa memoria al SO antes del próximo
    # pedido (mismo motivo que en local_flood_engine._calcular_hand).
    del (dem, acumulacion, fdir, grid, fdir_raster, acc_raster, mascara_cauce,
         catch, catch_arr, catch_view, catch_view_int, polys)
    gc.collect()
