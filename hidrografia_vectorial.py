# ============================================================
#  hidrografia_vectorial.py
# ------------------------------------------------------------
#  Módulo independiente y reutilizable (por IIPDI, simulación de
#  inundaciones, riesgo hídrico, expansión de cuerpos de agua, etc.)
#
#  Responsabilidad única: dado un punto (lat, lon), decir qué
#  cuerpo de agua vectorial (río/arroyo/laguna/estero/...) tiene
#  más cerca, de qué tipo es y su coeficiente de influencia.
#
#  Fuente de datos: capas GeoJSON por provincia alojadas en
#  Hugging Face (https://huggingface.co/datasets/Briant97/GeoSentinel).
#  Son públicas: no requieren token para descargarse.
#
#  DISEÑO PARA ESCALA:
#  - NO descarga las 19 capas al arrancar el backend (serían
#    cientos de MB en RAM). Descarga solo la provincia del punto
#    consultado, la reproyecta una vez y la cachea en memoria de
#    proceso para consultas siguientes.
#  - Usa un índice espacial (STRtree de Shapely) por provincia,
#    así buscar "el más cercano" no recorre miles de features
#    secuencialmente.
#
#  Requiere: pip install shapely pyproj requests
# ============================================================

import re
import json
import os
import threading
from collections import OrderedDict
import requests
from shapely.geometry import shape, Point, mapping
from shapely.strtree import STRtree
from shapely.ops import nearest_points
from pyproj import Transformer

HF_BASE_URL = 'https://huggingface.co/datasets/Briant97/GeoSentinel/resolve/main/'

# ------------------------------------------------------------
# Mapeo provincia -> archivo + bounding box aproximado
# (min_lon, min_lat, max_lon, max_lat). Bboxes aproximados: alcanzan
# para elegir el archivo correcto, no para límites catastrales.
# Se revisa en orden: los específicos ANTES que "NOA" (que agrupa
# varias provincias), para que La Rioja no caiga en el archivo NOA.
# ------------------------------------------------------------
PROVINCIAS = [
    ('La Rioja',      'Cuerpos de agua de La Rioja.geojson',  (-69.0, -31.8, -66.0, -28.0)),
    ('NOA',           'Cuerpos de agua NOA.geojson',          (-69.2, -30.5, -61.5, -21.8)),
    ('Córdoba',       'Cuerpos de agua Córdoba.geojson',      (-65.6, -35.0, -61.8, -29.5)),
    ('Buenos Aires',  'Cuerpos de agua de Bs As.geojson',     (-63.4, -41.0, -56.6, -33.3)),
    ('Chaco',         'Chaco.geojson',                        (-63.0, -27.6, -58.6, -24.0)),
    ('Formosa',       'Cuerpos de agua de Formosa.geojson',   (-62.5, -26.7, -57.5, -22.5)),
    ('Misiones',      'Cuerpos de agua de Msiones.geojson',   (-56.0, -28.2, -53.6, -25.3)),
    ('Corrientes',    'agua_corrientes.geojson',              (-59.7, -30.3, -55.6, -27.0)),
    ('Entre Ríos',    'Entre Rios.geojson',                   (-60.9, -34.0, -57.8, -30.0)),
    ('Santa Fe',      'Santa Fe.geojson',                     (-63.4, -34.5, -59.7, -28.0)),
    ('La Pampa',      'La Pampa.geojson',                     (-68.3, -39.0, -63.3, -35.0)),
    ('Mendoza',       'Mendoza_combined.geojson',             (-70.6, -37.6, -66.5, -32.0)),
    ('San Juan',      'San Juan.geojson',                     (-70.8, -32.6, -66.5, -28.5)),
    ('San Luis',      'San Luis.geojson',                     (-67.2, -36.3, -64.0, -32.0)),
    ('Neuquén',       'Neuquén.geojson',                      (-71.8, -41.0, -68.0, -36.0)),
    ('Río Negro',     'Río Negro.geojson',                    (-71.8, -42.0, -62.0, -38.0)),
    ('Chubut',        'Chubut.geojson',                       (-72.0, -46.3, -63.5, -42.0)),
    ('Santa Cruz',    'Santa Cruz.geojson',                   (-73.5, -52.5, -64.9, -46.0)),
]

# Coeficientes por tipo de cuerpo de agua (configurable: valores
# iniciales del prompt + los tipos reales que aparecieron en los
# datos). Clave en minúsculas y sin acentos para matchear tolerante.
COEFICIENTES_TIPO_AGUA = {
    'rio': 1.00,
    'estero': 0.95,
    'embalse': 0.90,
    'banado': 0.85,
    'laguna': 0.80,
    'canada': 0.75,
    'arroyo': 0.70,
    'valle aluvial': 0.65,
    'carrizal': 0.60,
    # Equivalentes en inglés/OpenStreetMap (ej. Chaco.geojson: tag "water=river")
    'river': 1.00,
    'reservoir': 0.90,
    'lagoon': 0.80,
    'oxbow': 0.80,
    'lake': 0.80,
    'pond': 0.75,
    'marsh': 0.85,
    'swamp': 0.85,
    'bog': 0.85,
    'wet_meadow': 0.75,
    'stream': 0.70,
    'canal': 0.70,
    'basin': 0.60,
}
COEFICIENTE_DEFECTO = 0.50  # tipo desconocido / no clasificado

_CACHE_LOCK = threading.Lock()

# CON LÍMITE (LRU): antes esto era un dict común sin tope — cada
# provincia consultada (capa vectorial COMPLETA de ríos/arroyos/lagunas)
# se quedaba en RAM para siempre. Con 18 provincias posibles y varios
# endpoints activos que pegan acá (/cuerpo_agua_cercano, /iipdi_*), una
# sesión que consulta puntos en unas pocas provincias distintas ya
# alcanzaba para acumular memoria sin límite — la misma causa de OOM que
# ya se encontró y arregló en local_flood_engine.py y
# suelos_inta_regional.py. Ahora solo se mantienen las últimas
# _CACHE_MAX provincias en memoria; el resto se relee del caché en disco
# (rápido, sin red) la próxima vez que haga falta.
_CACHE_MAX = 4
_CACHE_INDICE = OrderedDict()   # nombre_archivo -> {'tree': STRtree, 'props': [...], 'geoms': [...]}

CACHE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'cache_hidrografia')
os.makedirs(CACHE_DIR, exist_ok=True)


def _cache_set(clave, valor):
    _CACHE_INDICE[clave] = valor
    _CACHE_INDICE.move_to_end(clave)
    while len(_CACHE_INDICE) > _CACHE_MAX:
        _CACHE_INDICE.popitem(last=False)


def _cache_get(clave):
    if clave not in _CACHE_INDICE:
        return None, False
    _CACHE_INDICE.move_to_end(clave)
    return _CACHE_INDICE[clave], True


def _ruta_cache_disco(archivo):
    seguro = ''.join(c if c.isalnum() else '_' for c in archivo)
    return os.path.join(CACHE_DIR, f'{seguro}.geojson')


def _quitar_acentos(texto):
    reemplazos = str.maketrans('áéíóúñÁÉÍÓÚÑ', 'aeiounAEIOUN')
    return texto.translate(reemplazos)


def _normalizar_tipo(tipo_raw):
    if not tipo_raw:
        return 'desconocido'
    return _quitar_acentos(str(tipo_raw)).strip().lower()


def coeficiente_de_tipo(tipo_raw):
    return COEFICIENTES_TIPO_AGUA.get(_normalizar_tipo(tipo_raw), COEFICIENTE_DEFECTO)


def _buscar_propiedad_tolerante(props, palabras_clave):
    """
    Busca en las properties de una feature cualquier clave cuyo nombre
    (normalizado: sin acentos, minúsculas) contenga alguna de las
    palabras clave dadas. Necesario porque cada provincia vino de una
    fuente GIS distinta y no todas usan exactamente 'tipo'/'nombre'
    como nombre de columna (ej. 'Tipo_Cuerpo', 'NOMBRE_CA', 'DENOMINA').
    Devuelve el primer valor no vacío que encuentre, o None.
    """
    for clave, valor in props.items():
        clave_norm = _quitar_acentos(str(clave)).strip().lower()
        if any(palabra in clave_norm for palabra in palabras_clave) and valor:
            return valor
    return None


def _archivos_candidatos_para_punto(lat, lon):
    """
    Devuelve TODOS los archivos cuyo bbox contiene el punto (no solo el
    primero). Es necesario porque provincias limítrofes (ej. Chaco y
    Corrientes, separadas por el río Paraná) tienen bboxes aproximados
    que se superponen cerca del límite real. Elegir "el primero que
    matchea" daba resultados incorrectos en esas zonas; en cambio, se
    consultan todos los candidatos y se compara la distancia real al
    cuerpo de agua más cercano en cada uno (ver buscar_cuerpo_mas_cercano).
    """
    return [archivo for _nombre, archivo, bbox in PROVINCIAS
            if bbox[0] <= lon <= bbox[2] and bbox[1] <= lat <= bbox[3]]


def _extraer_epsg(geojson_dict):
    """Lee el EPSG del bloque 'crs' del GeoJSON, si existe. Si no
    tiene 'crs', el estándar GeoJSON asume WGS84 (EPSG:4326)."""
    crs = geojson_dict.get('crs')
    if not crs:
        return 4326
    nombre = crs.get('properties', {}).get('name', '')
    m = re.search(r'EPSG::?(\d+)', nombre)
    return int(m.group(1)) if m else 4326


def _construir_indice(archivo):
    """Descarga (si no está cacheado en disco/memoria), reproyecta a
    WGS84 si hace falta, y arma el índice espacial de un archivo de
    provincia."""
    with _CACHE_LOCK:
        valor, encontrado = _cache_get(archivo)
        if encontrado:
            return valor

    ruta_disco = _ruta_cache_disco(archivo)
    if os.path.exists(ruta_disco):
        # Ya se había descargado en un proceso anterior: leer del disco
        # es cuestión de milisegundos, sin depender de la red.
        with open(ruta_disco, 'r', encoding='utf-8') as f:
            data = json.load(f)
    else:
        url = HF_BASE_URL + requests.utils.quote(archivo)
        resp = requests.get(url, timeout=60)
        resp.raise_for_status()
        data = resp.json()
        try:
            with open(ruta_disco, 'w', encoding='utf-8') as f:
                json.dump(data, f)
        except Exception:
            pass  # si no se pudo escribir a disco, no es grave — sigue con lo que ya tiene en memoria

    epsg_origen = _extraer_epsg(data)
    transformer = None
    if epsg_origen != 4326:
        transformer = Transformer.from_crs(f'EPSG:{epsg_origen}', 'EPSG:4326', always_xy=True)

    geoms, props = [], []
    for feat in data.get('features', []):
        geom_raw = feat.get('geometry')
        if not geom_raw:
            continue
        try:
            geom = shape(geom_raw)
            if transformer:
                geom = _reproyectar_geom(geom, transformer)
            geoms.append(geom)
            props.append(feat.get('properties', {}) or {})
        except Exception:
            continue  # feature corrupta: se ignora, no se cae todo el índice

    tree = STRtree(geoms) if geoms else None
    resultado = {'tree': tree, 'props': props, 'geoms': geoms}
    with _CACHE_LOCK:
        _cache_set(archivo, resultado)
    return resultado


def _reproyectar_geom(geom, transformer):
    from shapely.ops import transform as shp_transform
    return shp_transform(lambda x, y, z=None: transformer.transform(x, y), geom)


def buscar_cuerpo_mas_cercano(lat, lon, radio_km=20):
    """
    Devuelve el cuerpo de agua vectorial más cercano al punto, comparando
    entre TODOS los archivos de provincia cuyo bbox contiene el punto
    (relevante en zonas de frontera entre provincias). Devuelve None si
    no hay ningún archivo candidato o no hay features dentro del radio.

    Retorna: { tipo, nombre, distancia_m, coeficiente, provincia_archivo,
               superficie_ha, lat, lon } — lat/lon son del punto más cercano
    SOBRE la geometría del cuerpo de agua (no un centroide), útil para
    trazar líneas y calcular rumbos exactos desde otro punto.
    """
    archivos = _archivos_candidatos_para_punto(lat, lon)
    if not archivos:
        return None

    punto = Point(lon, lat)
    radio_grados = radio_km / 111.0

    mejor_global = None
    mejor_dist_grados = None

    for archivo in archivos:
        try:
            indice = _construir_indice(archivo)
        except Exception:
            continue  # un archivo caído/con error no debe tumbar la consulta entera
        if not indice['tree']:
            continue

        candidatos_idx = indice['tree'].query(punto.buffer(radio_grados))
        for idx in candidatos_idx:
            geom = indice['geoms'][idx]
            d = punto.distance(geom)
            if mejor_dist_grados is None or d < mejor_dist_grados:
                mejor_dist_grados = d
                props = indice['props'][idx]
                mejor_global = {'props': props, 'archivo': archivo, 'geom': geom}

    if mejor_global is None:
        return None

    distancia_m = mejor_dist_grados * 111_000  # aproximación válida a esta escala local
    props = mejor_global['props']

    # Punto más cercano SOBRE la geometría (no el centroide): lo necesita el
    # frontend para trazar la línea al cuerpo de agua y calcular el rumbo
    # real desde el foco de incendio (feature "cuerpo de agua + barrera").
    _, punto_en_geom = nearest_points(punto, mejor_global['geom'])
    punto_cercano_lon, punto_cercano_lat = punto_en_geom.x, punto_en_geom.y

    tipo_raw = (props.get('tipo') or props.get('TIPO') or
                props.get('water') or props.get('wetland') or  # esquema OpenStreetMap
                _buscar_propiedad_tolerante(props, ['tipo']))
    nombre = (props.get('nombre') or props.get('NOMBRE') or props.get('CUENCA') or
              props.get('name') or props.get('loc_name') or  # esquema OpenStreetMap
              _buscar_propiedad_tolerante(props, ['nombre', 'denomina']) or 'Sin nombre')
    superficie_ha = props.get('superficie_ha') or _buscar_propiedad_tolerante(props, ['superficie', 'area', 'sup_ha'])

    return {
        'tipo': tipo_raw or 'desconocido',
        'nombre': nombre,
        'distancia_m': round(distancia_m, 1),
        'coeficiente': coeficiente_de_tipo(tipo_raw),
        'provincia_archivo': mejor_global['archivo'],
        'superficie_ha': superficie_ha,
        'lat': round(punto_cercano_lat, 6),
        'lon': round(punto_cercano_lon, 6),
    }


def _propiedades_cuerpo(props):
    """Extrae (tipo, nombre, superficie_ha) de las properties de una feature,
    con la misma búsqueda tolerante que usa buscar_cuerpo_mas_cercano."""
    tipo_raw = (props.get('tipo') or props.get('TIPO') or
                props.get('water') or props.get('wetland') or
                _buscar_propiedad_tolerante(props, ['tipo']))
    nombre = (props.get('nombre') or props.get('NOMBRE') or props.get('CUENCA') or
              props.get('name') or props.get('loc_name') or
              _buscar_propiedad_tolerante(props, ['nombre', 'denomina']) or 'Sin nombre')
    superficie_ha = props.get('superficie_ha') or _buscar_propiedad_tolerante(props, ['superficie', 'area', 'sup_ha'])
    return tipo_raw, nombre, superficie_ha


def buscar_cuerpos_en_radio(lat, lon, radio_km=15, max_resultados=20):
    """
    A diferencia de buscar_cuerpo_mas_cercano() (que devuelve UN solo
    resultado), esta función devuelve TODOS los cuerpos de agua vectoriales
    dentro del radio, ordenados por distancia, cada uno con su geometría
    (simplificada, para no mandar polígonos enormes al frontend).

    Pensada para /iipdi_red_hidraulica: da la lista de "candidatos por
    cercanía" que después el backend filtra contra la máscara raster de
    conectividad real (zona_conectada), para quedarse solo con los que
    de verdad pertenecen a la red de drenaje del punto — no simplemente
    los más cercanos en línea recta.

    Retorna una lista (puede ser vacía) de dicts con los mismos campos que
    buscar_cuerpo_mas_cercano() más 'geometria' (GeoJSON, geometry only,
    ya simplificada con shapely.simplify).
    """
    archivos = _archivos_candidatos_para_punto(lat, lon)
    if not archivos:
        return []

    punto = Point(lon, lat)
    radio_grados = radio_km / 111.0
    tolerancia_simplify = max(radio_grados / 400.0, 0.00005)

    resultados = []
    vistos = set()  # (archivo, idx) para no duplicar si dos provincias se solapan en el bbox

    for archivo in archivos:
        try:
            indice = _construir_indice(archivo)
        except Exception:
            continue
        if not indice['tree']:
            continue

        candidatos_idx = indice['tree'].query(punto.buffer(radio_grados))
        for idx in candidatos_idx:
            clave = (archivo, int(idx))
            if clave in vistos:
                continue
            vistos.add(clave)

            geom = indice['geoms'][idx]
            d = punto.distance(geom)
            if d > radio_grados:
                continue  # el STRtree consulta por bbox del buffer; puede traer algo apenas afuera

            props = indice['props'][idx]
            distancia_m = d * 111_000
            _, punto_en_geom = nearest_points(punto, geom)
            tipo_raw, nombre, superficie_ha = _propiedades_cuerpo(props)

            try:
                geom_simplificada = geom.simplify(tolerancia_simplify, preserve_topology=True)
                geometria_geojson = mapping(geom_simplificada)
            except Exception:
                geometria_geojson = None  # geometría rara/corrupta: se informa el cuerpo igual, sin polígono

            resultados.append({
                'tipo': tipo_raw or 'desconocido',
                'nombre': nombre,
                'distancia_m': round(distancia_m, 1),
                'coeficiente': coeficiente_de_tipo(tipo_raw),
                'provincia_archivo': archivo,
                'superficie_ha': superficie_ha,
                'lat': round(punto_en_geom.y, 6),
                'lon': round(punto_en_geom.x, 6),
                'geometria': geometria_geojson,
            })

    resultados.sort(key=lambda r: r['distancia_m'])
    return resultados[:max_resultados]
