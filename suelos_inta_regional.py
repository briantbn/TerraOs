...
CAPAS_GEOJSON = [
    ...
    # Santa Fe (subido a HF, reemplaza el WFS en vivo geo-nodo10 que estaba
    # en CAPAS_REGIONALES — mismo motivo que Córdoba: no depender de que el
    # geoservidor de INTA esté arriba en el momento de la consulta.
    # ⚠️ NOMBRE DE ARCHIVO SIN CONFIRMAR: armado según el patrón del commit
    # de Córdoba (título del commit = nombre exacto del archivo). Si no
    # resuelve (descarga falla), no rompe nada — solo no se usa esta capa y
    # sigue la cascada normal. Confirmar la URL real copiando "Copy download
    # link" en la página del archivo en Hugging Face y pasarla para corregir.
    {'provincia': 'Santa Fe', 'depto': None, 'escala': 50_000,
     'url': 'https://huggingface.co/datasets/Briant97/GeoSentinel/resolve/main/Provincia%20de%20Santa%20Fe%20%281_50000%29%20%E2%80%94%20suelos_santa_fe.geojson',
...
CAPAS_REGIONALES = [
    ...
    # Santa Fe: ya NO usa WFS en vivo acá — se mudó a CAPAS_GEOJSON (archivo
    # estático en Hugging Face), mismo criterio que Córdoba, para no depender
    # de que geo-nodo10.inta.gob.ar esté arriba en el momento de la consulta.
    ...
    # TODO: todavía no existe un geojson 1:50.000 de Entre Ríos (confirmado
    # con el usuario, agosto 2026) — por eso sigue acá como WFS en vivo a
    # 1:100.000. Cuando se suba ese archivo a Hugging Face, mover esta
    # entrada a CAPAS_GEOJSON con escala 50_000, mismo patrón que Córdoba y
    # Santa Fe arriba, para dejar de depender de que geo-nodo03.inta.gob.ar
    # esté arriba en el momento de la consulta (esa es la causa más probable
    # de que a veces caiga al nacional 1:500.000 en vez de usar esto).
