...
def normalizar_propiedades(props):
    ...
    regional) a un esquema común, buscando por palabras clave
    tolerantes. Los campos que no se puedan mapear quedan en None.
    'propiedades_crudas' siempre incluye el original completo, por si
    el frontend necesita mostrar algo puntual de una fuente
    específica que este mapeo no contempló.
    """
    resultado = {campo: None for campo in _CAMPOS_BUSCADOS}
    for clave_original, valor in (props or {}).items():
        if valor in (None, ''):
            continue
    ...
    resultado['propiedades_crudas'] = props or {}
    return resultado
