...
def _familia_capa(nombre):
    """Agrupa los tiles que son pedazos de una misma capa lógica partida en
    grilla — ej. "Córdoba (-66_-35)", "Córdoba (-66_-34)", ... todos caen en
    la familia "Córdoba". Una capa sin tiles (ej. "Nacional — Cordoba") queda
    en su propia familia de un solo elemento. Se usa SOLO en capa_visual_bbox,
    para poder unir features de varios tiles contiguos sin mezclar capas de
    distinta escala/fuente entre sí."""
...
def capa_visual_bbox(bbox):
    ...
    Usa SIEMPRE una sola FAMILIA de capa por pedido (la de mejor escala que
    tenga algo dentro del bbox), para no mezclar visualmente polígonos de
    distinta escala/confiabilidad en una misma vista -- pero si esa familia
    está partida en varios tiles de grilla (ej. Córdoba, 18 tiles), une las
    features de TODOS los tiles que intersecten el bbox, no solo el primero.
    Antes, una vista que cruzaba varios tiles de Córdoba solo mostraba el
    primero que matcheaba, dejando el resto de la pantalla vacío aunque sí
    había datos ahí."""
    west, south, east, north = bbox
    ...
    # Agrupar las capas candidatas (que geográficamente pueden cubrir este
    # bbox) por familia, preservando el orden de SUELO_CAPAS -- que ya está
    # ordenado por prioridad: mejor escala/confiabilidad primero.
    familias_orden = []
    familias = {}
    for capa in SUELO_CAPAS:
        if not _capa_puede_cubrir_bbox(capa, bbox):
            continue  # fuera del bbox de esta capa: ni se descarga ni se carga en memoria
        familia = _familia_capa(capa['nombre'])
        if familia not in familias:
            familias[familia] = []
            familias_orden.append(familia)
        familias[familia].append(capa)

    for familia in familias_orden:
        features = []
        capa_referencia = None  # para 'confiabilidad'/'advertencia_escala' de la respuesta

        for capa in familias[familia]:
            datos = _cargar_capa(capa)
            if not datos or not datos['tree']:
            ...
            capa_referencia = capa_referencia or capa
            tile_aporto = 0
            for i in indices:
                geom = datos['geoms'][i]
            ...
            if len(features) >= _MAX_FEATURES_BBOX:
                break  # ya llegamos al tope combinando tiles de esta familia

        if not features:
            continue  # esta familia no aportó nada: probamos la siguiente (ej. Nacional)

        print(
            f'🗺️ [suelos_inta_regional] capa_visual_bbox {bbox} -> familia "{familia}" '
            f'({len(features)} features de {len(familias[familia])} tile(s) candidato(s)). '
            f'Intentos previos: {intentos or "ninguno"}'
        )
        return {
            'type': 'FeatureCollection',
            'fuente': familia,
            'confiabilidad': capa_referencia['confiabilidad'],
            'advertencia_escala': _advertencia_escala(capa_referencia),
            'features': features,
        }
