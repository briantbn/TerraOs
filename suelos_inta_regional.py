...
def _precargar_en_segundo_plano():
    """Descarga/parsea todas las cartas en un hilo aparte apenas arranca
    el proceso, en vez de esperar a que un usuario haga el primer pedido.
    Así la descarga lenta (si hace falta, la primera vez) le pasa al
    servidor arrancando, no a un usuario esperando una respuesta que
    puede llegar a tardar más que el timeout del worker de gunicorn."""
