...
# NOTA (bajado de 60 a 50 km): a 30m de resolución, 60km de radio arma
# una grilla de ~4000x4000 px — con los ~7 arrays float32/float64 que
# _calcular_hand() mantiene vivos a la vez, eso pasaba los 512MB del
# plan de Render y tiraba la instancia (OOM). 50km coincide exacto con
# el límite de conectividad que ya usaba el motor GEE, así que no se
# pierde alcance real, solo el margen extra que causaba el problema.
RADIO_ANALISIS_KM = 50.0
...
# ANTES: dict común, sin límite — cada zona nueva consultada (~5-6 arrays
# completos, 300-400MB) se quedaba ahí para siempre mientras el proceso
# viviera. Esto era la causa principal de los OOM en Render (con solo un
# par de zonas distintas consultadas después de un arranque ya se pasaba
# de los 512MB). El caché en DISCO (CACHE_DIR) sigue evitando recalcular
# — leer de disco ya es "milisegundos" (ver docstring de
# obtener_hand_local) — así que no hace falta guardar más de un puñado
# de zonas en RAM: lo que se saca de acá simplemente se relee de disco
# la próxima vez, sin volver a correr pysheds.
#
# CORRECCIÓN (bajado de 3 a 1): con RADIO_ANALISIS_KM=50 a 30m de
# resolución, cada zona cachea 6 bandas (hand, acumulacion, elevacion,
# pendiente, profundidad_depresion, es_depresion) de ~3333x3333 px ->
# ~223 MB POR ZONA. Con el límite en 3, la propia caché en RAM podía
# llegar a ~668 MB ella sola -- YA por encima del techo de 512MB del
# plan free de Render, antes de sumar el resto del proceso (Flask,
# Earth Engine, rasterio/pysheds cargados, etc.). Con el límite en 1,
# el tope de esta caché baja a ~223 MB, dejando margen real para el
# resto de la app. El caso que se pierde (rebotar entre 2-3 zonas
# distintas dentro del mismo segundo) es marginal frente al riesgo de
# OOM; para todo lo demás la caché en disco sigue sirviendo lecturas
# casi instantáneas.
_CACHE_MEMORIA_MAX = 1
_CACHE_MEMORIA = OrderedDict()  # clave -> {'hand': ndarray, 'transform': Affine, 'crs': CRS, 'nodata': float}


def _cache_memoria_set(clave, valor):
    """Guarda en la caché de RAM con límite fijo (LRU): si ya está llena,
    descarta la entrada menos usada recientemente antes de agregar la
    nueva — nunca deja crecer el dict sin límite."""
    _CACHE_MEMORIA[clave] = valor
    _CACHE_MEMORIA.move_to_end(clave)
    while len(_CACHE_MEMORIA) > _CACHE_MEMORIA_MAX:
        _CACHE_MEMORIA.popitem(last=False)


def _cache_memoria_get(clave):
    """Lee de la caché de RAM y la marca como recién usada (para LRU)."""
    valor = _CACHE_MEMORIA.get(clave)
...
def _calcular_hand(dem, transform, crs, nodata):
    ...
    # pysheds arma accumulation() internamente en float64 (el doble de lo
    # que necesitamos: son conteos de celdas aguas arriba, no requieren
    # esa precisión). La bajamos a float32 apenas sale — con una grilla
    # de ~4000x4000 esto solo ya ahorra ~64MB por pedido.
    acumulacion = np.asarray(grid.accumulation(fdir)).astype('float32')

    mascara_cauce = acumulacion > UMBRAL_ACUMULACION_CAUCE
    hand = grid.compute_hand(fdir, dem_final, mascara_cauce)

    resultado = (
        np.asarray(hand).astype('float32'),
        acumulacion,
        np.asarray(fdir),
        np.asarray(dem_sin_depresiones).astype('float32'),
    )

    # grid/dem_final/mascara_cauce ya no se usan — liberarlos acá (en vez
    # de esperar a que salgan de scope al terminar la función) ayuda a
    # que el proceso le devuelva esa memoria al SO antes del próximo
    # pedido, en vez de arrastrarla al siguiente request del mismo worker.
    del grid, dem_final, mascara_cauce
    gc.collect()
...
def obtener_hand_local(lat, lon, forzar_recalculo=False):
    ...
    with _CACHE_LOCK:
        en_memoria = None if forzar_recalculo else _cache_memoria_get(clave)
        if en_memoria is not None:
            resultado = dict(en_memoria)
    ...
    if not forzar_recalculo:
        ...
        if desde_disco is not None:
            with _CACHE_LOCK:
                _cache_memoria_set(clave, desde_disco)
    ...
    with _CACHE_LOCK:
        _cache_memoria_set(clave, resultado)
...
def mascara_poligono(shape, transform, poligono_lonlat):
    """
    Array booleano (mismo `shape` que el HAND local) en True para los
    píxeles que caen DENTRO del polígono que el usuario delimitó con el
    botón "Delimitar" del frontend (poligono_lonlat: lista de [lon, lat]).
    Se usa para recortar la imagen final de /inundacion_local a ese campo
    exacto — mismo criterio que _region_poligono_desde_query() en app.py
    para el camino GEE (/inundacion_tiles): el HAND se sigue calculando
    sobre toda el área de análisis normal (~60 km), esto solo decide qué
    píxeles se pintan en la imagen.

    Devuelve None si poligono_lonlat es None/inválido (menos de 3 puntos)
    o si algo falla al rasterizar — en ambos casos el llamador sigue sin
    recorte, como hasta ahora.
    """
    if not poligono_lonlat or len(poligono_lonlat) < 3:
        return None
    try:
        from rasterio import features
        anillo = [[float(p[0]), float(p[1])] for p in poligono_lonlat]
        if anillo[0] != anillo[-1]:
            anillo.append(anillo[0])
        geom = {'type': 'Polygon', 'coordinates': [anillo]}
        # invert=True => True DENTRO de la geometría (por defecto
        # geometry_mask da True AFUERA, pensado para enmascarar/tapar).
        return features.geometry_mask([geom], out_shape=shape, transform=transform, invert=True)
...
def generar_imagen_mascara_color_base64(mascara, color_hex, alpha=_ALPHA_MASCARA, mascara_extra=None):
    ...
    mascara_extra: recorte opcional al campo delimitado (ver mascara_poligono())."""
    from PIL import Image
    import io
    import base64

    if mascara_extra is not None:
        mascara = mascara & mascara_extra
...
def renderizar_capa(tipo, resultado, mascara_extra=None):
    """
    ...
    mascara_extra: recorte opcional al campo delimitado por el usuario
    (ver mascara_poligono() en app.py / /inundacion_local) — se combina
    con la máscara propia de cada tipo, no reemplaza nada del cálculo.
    ...
    if tipo == 'elevacion':
        img = generar_imagen_gradiente_base64(elevacion, 30, 140, PALETA_ELEVACION, mascara_extra=mascara_extra)
        return img, {'min': 30, 'max': 140, 'palette': PALETA_ELEVACION}, {}

    if tipo == 'pendiente':
        img = generar_imagen_gradiente_base64(pendiente, 0, 3, PALETA_PENDIENTE, mascara_extra=mascara_extra)
        return img, {'min': 0, 'max': 3, 'palette': PALETA_PENDIENTE}, {}

    if tipo == 'zona_media':
        mascara = (elevacion >= 55) & (elevacion < 90)
        img = generar_imagen_mascara_color_base64(mascara, COLOR_ZONA_MEDIA, mascara_extra=mascara_extra)
        return img, {'palette': [COLOR_ZONA_MEDIA]}, {}

    if tipo == 'zona_alta':
        mascara = elevacion >= 90
        img = generar_imagen_mascara_color_base64(mascara, COLOR_ZONA_ALTA, mascara_extra=mascara_extra)
    ...
    if tipo in ('riesgo', 'critico'):
        ...
        if tipo == 'riesgo':
            img = generar_imagen_gradiente_base64(riesgo, 0.2, 0.8, PALETA_RIESGO, mascara_extra=mascara_extra)
            return img, {'min': 0.2, 'max': 0.8, 'palette': PALETA_RIESGO}, {'nota': nota_smap}
        mascara = riesgo > UMBRAL_CRITICO
        img = generar_imagen_mascara_color_base64(mascara, COLOR_CRITICO, mascara_extra=mascara_extra)
        return img, {'palette': [COLOR_CRITICO]}, {'nota': nota_smap}

    raise ValueError(f"tipo '{tipo}' no soportado por renderizar_capa()")


def generar_imagen_png_base64(hand, umbral_m, nodata, mascara_extra=None):
    """
    ...
    mascara_extra: booleano opcional (mismo shape que hand) — celdas en
    False se fuerzan a transparente además de la máscara de inundación.
    Se usa para recortar al campo delimitado por el usuario (ver
    mascara_poligono()); no cambia el cálculo del HAND, solo qué se pinta.
    """
    from PIL import Image
    import io
    import base64
