...
try:
    ...
    # SHIM DE COMPATIBILIDAD: pysheds 0.5 (internamente, en
    # sgrid.py:_d8_accumulation) todavía llama a np.in1d — función que
    # NumPy ELIMINÓ en la versión 2.0 (requirements.txt fija
    # numpy==2.4.4). Confirmado corriendo el pipeline a mano: sin este
    # shim, accumulation()/compute_hand() truena con
    # "module 'numpy' has no attribute 'in1d'". np.isin es el
    # reemplazo directo (mismo comportamiento para el caso 1D que usa
    # pysheds acá), así que alcanza con reexponer el nombre viejo.
    if not hasattr(np, 'in1d'):
        np.in1d = np.isin

    # Sin esto, GDAL puede quedarse esperando indefinidamente una
    # respuesta HTTP colgada del bucket de Copernicus (/vsicurl/ no
    # tiene timeout por defecto) — el --timeout de gunicorn mata igual
    # el worker eventualmente, pero recién a los 180s y con el peor
    # resultado posible (SIGKILL a mitad de una lectura de red, en vez
    # de un error de Python controlado y mensaje claro). Con esto, una
    # conexión/lectura colgada corta sola en pocos segundos y cae en
    # el except de _descargar_dem() como cualquier otro fallo de red.
    ...
    # Evita que GDAL intente listar el directorio remoto completo (algunos
    # servidores S3 lo permiten y es lento/pesado) — no hace falta acá,
    # ya sabemos el nombre exacto del archivo por tile.
    os.environ.setdefault('GDAL_DISABLE_READDIR_ON_OPEN', 'EMPTY_DIR')
    # Techo al caché de bloques de GDAL en RAM (además de los arrays de
    # numpy que ya administra Python) — otro margen de seguridad para no
    # sumarle presión de memoria extra al proceso en Render.
...
# a 800 km). La propagación de inundación real ya estaba topeada a 50 km
# en el motor GEE actual (MAX_DIST_CONECTIVIDAD_M), así que 60 km daba
# margen de sobra — PERO a 30 m de resolución eso es ~4000×4000 píxeles,
# y pysheds necesita varios arrays de ese tamaño en memoria a la vez
# (DEM, DEM rellenado, dirección de flujo, acumulación, HAND). En Render
# Free (512 MB) eso funde el proceso (SIGKILL / OOM). 20 km reduce el
# área ~9 veces (≈1300×1300 píxeles) y sigue sobrando para un campo
# típico. Si en el futuro se sube de plan (más RAM), este valor se
# puede volver a subir sin tocar nada más del pipeline.
RADIO_ANALISIS_KM = 20.0
...
def _calcular_hand(dem, transform, crs, nodata):
    ...
    pysheds necesita un archivo/raster real para armar su Grid — pero
    OJO: `Grid.from_raster()` / `grid.read_raster()` (pysheds 0.5)
    esperan un Raster de pysheds o un PATH (str), NO un dataset de
    rasterio ya abierto (eso tira 'data must be a Raster or str',
    confirmado corriendo el pipeline a mano). La solución: escribir el
    mosaico a un GeoTIFF EN MEMORIA (rasterio MemoryFile, sin tocar
    disco) y pasarle a pysheds el *path virtual* de GDAL
    (`memfile.name`, tipo '/vsimem/...') — internamente pysheds hace
    `rasterio.open(ese_path)` él solo, así que necesita el string, no
    el objeto ya abierto.
    """
    perfil = {
    ...
    with MemoryFile() as memfile:
        with memfile.open(**perfil) as dataset:
            dataset.write(dem, 1)
        ruta_virtual = memfile.name  # ej. '/vsimem/xxxxxxxx' — string, no el dataset
        grid = Grid.from_raster(ruta_virtual)
        dem_grid = grid.read_raster(ruta_virtual)
